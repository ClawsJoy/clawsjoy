...
class ContentWorkflow:
    ...
    def run_full_workflow(
        self, topic: str, style: str = "科普", platform: str = "youtube"
    ) -> Dict:
        results = {}

        # Step 1: 采集
        print(f"📡 采集: {topic}")
        collect_result = content_collector.execute(
            {"action": "search", "keyword": topic}
        )
        results["collect"] = collect_result

        # Step 2: 生成脚本
        print(f"✍️ 生成脚本: {topic}")
        script_result = script_writer.execute(
            {"action": "generate", "topic": topic, "style": style}
        )
        ...
        publish_result = video_publisher.execute(
            {"action": "publish", "platform": platform}
        )
