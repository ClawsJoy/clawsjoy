...
class RSSTopicCrawlerSkill:
    ...
    def execute(self, params):
            ...
            for feed_url in self.RSS_FEEDS.get(source, []):
                try:
                    ...
                    for entry in feed.entries[:15]:
                        ...
                        # 过滤：长度>3，不是停用词
                        keywords = [
                            w for w in words if len(w) > 3 and w not in self.STOP_WORDS
                        ]
        ...
        # 优先采集有价值的关键词
        valuable_found = {
            k: v for k, v in word_count.items() if k in self.VALUABLE_WORDS
        }
        other_found = {
            k: v
            for k, v in word_count.items()
            if k not in self.VALUABLE_WORDS and v >= 2
        }
