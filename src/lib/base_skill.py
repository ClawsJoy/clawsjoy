...
class BaseAtomicSkill(ABC):
    ...
    def log_execution(self, params: Dict, result: Dict):
        ...
        with open(log_file, 'a') as f:
            f.write(f"{datetime.datetime.now()} | {self.name} | {json.dumps(params)[:100]} | {result.get('success')}\n")
...
class BaseWorkflowSkill(BaseAtomicSkill):
    ...
    def _call_atomic_skill(self, name: str, params: Dict) -> Dict:
        """调用原子技能"""
        try:
            module = __import__(f"src.skills.atomic.{self._get_skill_path(name)}", fromlist=["skill"])
