...
class SkillSecurityScanner:
        ...
        for pattern, info in self.DANGEROUS_PATTERNS.items():
            if re.search(pattern, content, re.IGNORECASE):
                result["issues"].append(
                    {"pattern": pattern, "level": info["level"], "message": info["msg"]}
                )
    ...
    def validate_manifest(self, manifest_path: str) -> Dict:
        ...
        result = {"file": str(path), "valid": True, "issues": []}
    ...
    def generate_report(self) -> Dict:
        ...
        report = {
            ...
            "safe_count": sum(
                1 for r in self.scan_results.values() if r.get("is_safe")
            ),
            "unsafe_count": sum(
                1 for r in self.scan_results.values() if not r.get("is_safe")
            ),
