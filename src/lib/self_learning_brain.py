...
class SelfLearningBrain:
    ...
    def _reason(self, input_text):
        ...
        skills_desc = "\n".join(
            [
                f"- {name}: {getattr(skill, 'description', name)}"
...
只输出 JSON。"""
    ...
    def _execute_plan(self, plan):
        """执行计划"""
        from src.api.gateway import _legacy_skills, _skills, _workflows
        ...
        for step in plan.get("steps", []):
            ...
            if skill_name in _skills:
                ...
                result = _legacy_skills[skill_name]["loader"].execute(
                    skill_name, params
                )
                results.append({"skill": skill_name, "result": result})
            else:
                results.append(
                    {
                        "skill": skill_name,
                        "result": {"success": False, "error": "技能不存在"},
                    }
                )
    ...
    def _learn(self, input_text, plan, result):
        ...
        if result.get("success"):
            ...
            memory.remember(
                json.dumps(
                    {"pattern": pattern, "plan": plan, "success": True, "count": 1}
                ),
