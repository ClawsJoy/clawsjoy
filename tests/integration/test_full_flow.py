...
class TestFullFlow:
    ...
    def test_chat_greeting(self, client):
        """测试问候对话"""
        response = client.post('/api/v5/enhanced/chat',
    ...
    def test_chat_calculate(self, client):
        """测试计算"""
        response = client.post('/api/v5/enhanced/chat',
            json={'message': '15加27', 'user_id': 'test_user'})
        assert response.status_code == 200
        data = json.loads(response.data)
        assert 'response' in data
    ...
    def test_memory_chain(self, client):
        ...
        # 第1轮：告诉名字
        response1 = client.post('/api/v5/enhanced/chat',
            json={'message': '我叫李华', 'user_id': user_id})
        assert response1.status_code == 200
        
        # 第2轮：询问名字
        response2 = client.post('/api/v5/enhanced/chat',
        ...
        # 第3轮：告诉偏好
        response3 = client.post('/api/v5/enhanced/chat',
            json={'message': '我喜欢喝咖啡', 'user_id': user_id})
        assert response3.status_code == 200
        
        # 第4轮：询问偏好
        response4 = client.post('/api/v5/enhanced/chat',
            json={'message': '我喜欢什么', 'user_id': user_id})
        assert response4.status_code == 200
    
    def test_learning_endpoint(self, client):
        """测试学习端点"""
        response = client.post('/api/learning/record',
            json={'fact': '测试学习', 'success': True})
        assert response.status_code == 200
        data = json.loads(response.data)
    ...
    def test_vector_search(self, client):
        """测试向量搜索"""
        response = client.post('/api/vector/search',
            json={'query': '代码', 'top_k': 5})
        assert response.status_code == 200
        data = json.loads(response.data)
        assert data.get('success') == True
...
class TestPerformance:
    ...
    def test_concurrent_requests(self, client):
        ...
        results = []
        def make_request():
            resp = client.post('/api/v5/enhanced/chat',
