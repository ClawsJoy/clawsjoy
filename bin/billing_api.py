...
def record_usage(tenant_id: str, task_type: str, quantity: int = 1):
    ...
    c.execute("UPDATE tenant_balance SET balance = balance - ?, updated_at = CURRENT_TIMESTAMP WHERE tenant_id = ?",
              (total, tenant_id))
    if c.rowcount == 0:
        c.execute("INSERT INTO tenant_balance (tenant_id, balance) VALUES (?, ?)", (tenant_id, -total))
...
class BillingHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == '/api/billing/balance':
            ...
            self.send_json({"tenant_id": tenant_id, "usage": get_usage(tenant_id, days)})
    ...
    def do_POST(self):
        ...
        if self.path == '/api/billing/record':
            ...
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute("UPDATE tenant_balance SET balance = balance + ? WHERE tenant_id = ?", (amount, tenant_id))
            if c.rowcount == 0:
                c.execute("INSERT INTO tenant_balance (tenant_id, balance) VALUES (?, ?)", (tenant_id, amount))
            conn.commit()
            conn.close()
            self.send_json({"success": True, "tenant_id": tenant_id, "new_balance": get_balance(tenant_id)})
...
def get_plans():
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT id, name, price, quota_chat, quota_code, quota_promo, description FROM plans")
    rows = c.fetchall()
    conn.close()
    return [{"id": r[0], "name": r[1], "price": r[2], "quota": {"chat": r[3], "code": r[4], "promo": r[5]}, "description": r[6]} for r in rows]

if __name__ == "__main__":
    init_db()
    port = PORT_BILLING
    LOGGER.info("Billing API started: http://localhost:%s", port)
