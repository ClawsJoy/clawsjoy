    ...
    def do_POST(self):
        if self.path == '/api/promo/make':
            ...
            cmd = [
                ...
                '-vf', f"drawtext=text='{city} {style}':fontcolor=white:fontsize=60:x=(w-text_w)/2:y=(h-text_h)/2",
                '-c:v', 'libx264', '-y', video_file
            ]
            
            result = subprocess.run(cmd, capture_output=True)
            
            if result.returncode == 0 and os.path.exists(video_file):
                self.send_json({'success': True, 'video_url': f'/videos/{os.path.basename(video_file)}'})
            else:
                self.send_json({'success': False, 'error': result.stderr.decode()[:200]})
        else:
            self.send_error(404)
