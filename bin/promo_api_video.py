...
def make_promo(city, style):
    ...
    try:
        ...
        subprocess.run(
            f"/root/clawsjoy/bin/spider_unsplash '{city} {style}' 3",
            shell=True,
            timeout=30,
        )
        time.sleep(2)
        ...
        if output_file.exists() and output_file.stat().st_size > 10000:
            ...
            print("⚠️ 生成占位视频")
            subprocess.run(
                f"ffmpeg -f lavfi -i color=c=#667eea:s=1280x720:d=5 -vf 'drawtext=text={city}{style}宣传片:fontcolor=white:fontsize=48' -y {output_file}",
                shell=True,
            )
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == "/api/promo/make":
            ...
            # 返回视频URL（使用最新视频）
            videos = sorted(
                OUTPUT_DIR.glob("*.mp4"), key=os.path.getmtime, reverse=True
            )
