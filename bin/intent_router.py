...
class IntentHandler(BaseHTTPRequestHandler):
    ...
    def _detect_promo(self, msg):
        ...
        if has_action and has_target:
            # 提取城市
            city = '香港'
            cities = ['香港', '北京', '上海', '深圳', '广州', '杭州', '成都', '重庆', '武汉', '西安']
            for c in cities:
                if c in msg:
                    city = c
                    break
            # 提取风格
            style = '科技'
            if '复古' in msg: style = '复古'
            elif '温馨' in msg: style = '温馨'
            elif '大气' in msg: style = '大气'
        ...
        try:
            ...
            if result:
                ...
                resp = requests.post(f"{TASK_API}/api/task/promo",
                    json={'city': result['city'], 'style': result['style'], 'tenant_id': '1'},
                    timeout=60)
                if resp.status_code == 200:
                    video_data = resp.json()
                    if video_data.get('success'):
                        self.send_json({'type': 'promo', 'video_url': video_data.get('video_url')})
                        return
            ...
            if result:
                resp = requests.get(f"{TASK_API}/api/library/list", params={'tenant_id': '1', 'limit': 5})
                if resp.status_code == 200:
