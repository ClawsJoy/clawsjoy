#!/usr/bin/env python3
"""Stable Task API - 多图轮播+字幕，内置资料库（未剥离）"""
...
def _call_library_skill(action, params):
    """调用资料库 Skill，失败时返回 None 表示降级"""
    try:
        skill_dir = "/home/flybo/clawsjoy/skills/customer-profile-manager"
        if not os.path.exists(skill_dir):
            return None
        spec = importlib.util.spec_from_file_location("lib_skill", os.path.join(skill_dir, "main.py"))
