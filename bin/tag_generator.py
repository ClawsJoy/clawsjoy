...
def get_image_tags(image_path, candidates=None):
    if CLIP_AVAILABLE:
        if candidates is None:
            candidates = ["风景","人物","建筑","科技","美食","动物","夜景","文化","艺术","自然"]
        _load_model()
        if _model is not None:
            try:
                image = Image.open(image_path).convert("RGB")
                inputs = _processor(text=candidates, images=image, return_tensors="pt", padding=True)
...
def async_tag_file(file_path, file_id, tenant_id):
    ...
    if tags:
        ...
        conn.commit()
        conn.close()
        print(f"🏷️ 文件 {file_id} 标签: {tags} (来源: {'CLIP' if CLIP_AVAILABLE else '回退'})")
