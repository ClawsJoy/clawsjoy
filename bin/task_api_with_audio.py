...
class TaskHandler(BaseHTTPRequestHandler):
    ...
    def _handle_promo(self):
        try:
            ...
            has_audio = generate_audio_tts(script, audio_path)
            video_temp = f"/home/flybo/clawsjoy/web/videos/{city}_{style}_{int(time.time())}_temp.mp4"
            video_final = f"/home/flybo/clawsjoy/web/videos/{city}_{style}_{int(time.time())}.mp4"
            duration = 15.0
            if len(final_images) == 1:
                img = final_images[0]
                cmd = f"ffmpeg -y -loop 1 -i '{img}' -c:v libx264 -t {duration} -pix_fmt yuv420p -vf 'scale=1920:1080:force_original_aspect_ratio=1,pad=1920:1080:(ow-iw)/2:(oh-ih)/2' {video_temp}"
                ...
                cmd = f"ffmpeg -y -f concat -safe 0 -i {concat_file} -c:v libx264 -pix_fmt yuv420p -vf 'scale=1920:1080:force_original_aspect_ratio=1,pad=1920:1080:(ow-iw)/2:(oh-ih)/2' -t {duration} {video_temp}"
                subprocess.run(cmd, shell=True, stderr=subprocess.DEVNULL, timeout=60)
                os.remove(concat_file)
            if has_audio and os.path.exists(audio_path):
                subtitle_text = script[:50].replace("'", "\\'")
                cmd = f"ffmpeg -i {video_temp} -i {audio_path} -c:v copy -c:a aac -shortest -vf \"drawtext=text='{subtitle_text}':fontcolor=white:fontsize=24:x=(w-text_w)/2:y=h-100\" -y {video_final}"
                subprocess.run(cmd, shell=True, stderr=subprocess.DEVNULL, timeout=60)
                os.remove(audio_path)
                os.remove(video_temp)
            else:
                os.rename(video_temp, video_final)
            self._update_task(task_id, 'completed', video_final)
            if os.path.exists(video_final):
                self.send_json({'success': True, 'video_url': f"/videos/{os.path.basename(video_final)}", 'message': f'{city}{style}宣传片已生成', 'source': 'library' if personal else 'external', 'images_used': len(final_images)})
            else:
