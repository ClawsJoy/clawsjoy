...
class SyntheticGenerator:
    ...
    def _load_templates(self) -> Dict:
        return {
            'code': {
                'templates': ["写一个{function}函数", "帮我写{code_type}代码", "实现{algorithm}算法"],
                'variables': {
            ...
            'weather': {
                'templates': ["{city}天气怎么样", "今天{city}气温多少", "{city}会下雨吗"],
                'variables': {'city': ['北京', '上海', '广州', '深圳', '杭州']}
            },
            'translate': {
                'templates': ["把{text}翻译成{target_lang}", "翻译{text}到{target_lang}"],
                'variables': {
    ...
    def generate_all(self, samples_per_intent: int = 20) -> Dict[str, List[str]]:
        ...
        engine_logger.get().info(f"   ✅ 生成 {sum(len(v) for v in result.values())} 条合成数据")
        return result
    ...
    def get_stats(self) -> Dict:
        return {"intents": len(self.templates), "generated": sum(len(v) for v in self.generated_data.values())}
