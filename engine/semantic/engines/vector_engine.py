...
class VectorEngine(BaseEngine):
    ...
    def _init_collection(self):
        ...
        try:
            ...
            self._collection = vector_knowledge_center._get_or_create_collection("intent_vectors")
            print(f"✅ 向量引擎已初始化")
        ...
        try:
            ...
            if results and results.get('distances') and results['distances'][0]:
                ...
                if results.get('metadatas') and results['metadatas'][0]:
                    ...
                    return intent, confidence, {"distance": distance, "source": "vector"}
        except Exception as e:
            pass
    ...
    def learn(self, text: str, intent: str, confidence: float) -> Dict:
        ...
        try:
            doc_id = hashlib.md5(f"{intent}_{text}_{time.time()}".encode()).hexdigest()[:16]
            self._collection.add(
                ids=[doc_id],
                documents=[text],
                metadatas=[{"intent": intent, "source": "learned", "confidence": confidence}]
            )
        ...
        try:
            ...
            for intent_name, intent_config in intents.items():
                for keyword in intent_config.get('keywords', []):
                    doc_id = hashlib.md5(f"config_{intent_name}_{keyword}".encode()).hexdigest()[:16]
