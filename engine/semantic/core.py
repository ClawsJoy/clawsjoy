...
class IntentResult:
    ...
    def __init__(self, intent: str, confidence: float, entities: dict = None,
                 source: str = "unknown", engine: str = "", raw_text: str = "",
                 latency_ms: float = 0, metadata: dict = None):
        self.intent = intent
...
class SemanticEngine:
    ...
    def _init_engines(self):
        ...
        for module_name, class_name in engine_classes:
            try:
                module = __import__(f"engine.semantic.engines.{module_name}_engine", 
                                   fromlist=[class_name])
                engine_class = getattr(module, class_name)
                engine = engine_class()
                self._engines.append(engine)
                self._circuit_breakers[engine.name] = {"failures": 0, "open": False}
                engine_logger.get().info(f"   ✅ {engine.name} (priority={engine.priority})")
    ...
    def understand(self, text: str) -> IntentResult:
        ...
        return IntentResult("unknown", 0.0, source="none", latency_ms=(time.time() - start_time) * 1000)
    ...
    def get_stats(self) -> Dict:
        success_rate = self._success_requests / self._total_requests if self._total_requests > 0 else 0
        return {
            "status": "active",
            "version": "8.0.0",
