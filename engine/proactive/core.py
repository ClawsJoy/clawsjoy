...
class ProactiveEngine:
    ...
    def process(self, input_data: Any = None, **kwargs) -> Any:
        ...
        if isinstance(input_data, dict):
            ...
            if action == 'record':
                return self.record_behavior(input_data.get('user_id', 'default'), input_data.get('action_type', 'unknown'))
            return self.suggest_proactive_action(input_data.get('user_id', 'default'), input_data.get('context', {}))
    ...
    def record_behavior(self, user_id: str, action: str, context: Dict = None) -> Dict:
        self.user_patterns[user_id].append({
            "action": action, "context": context or {}, "timestamp": datetime.now().isoformat()
        })
    ...
    def suggest_proactive_action(self, user_id: str, current_context: Dict) -> Dict:
        ...
        if 6 <= hour <= 10:
            return {"action": "morning_greeting", "reason": "早晨问候", "priority": "medium"}
        elif 12 <= hour <= 14:
            return {"action": "lunch_reminder", "reason": "午餐提醒", "priority": "medium"}
        elif 18 <= hour <= 22:
            return {"action": "evening_summary", "reason": "晚间总结", "priority": "medium"}
