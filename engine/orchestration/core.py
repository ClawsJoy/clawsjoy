    ...
    def process(self, input_data: Any = None, **kwargs) -> Any:
        ...
        if isinstance(input_data, dict):
            ...
            if action == 'orchestrate':
                return self.orchestrate(input_data.get('task', ''), input_data.get('agents'))
            elif action == 'workflow':
                return self.execute_workflow(input_data.get('name', ''), input_data.get('context'))
        return self.get_stats()
    
    def get_stats(self) -> Dict:
        return {"workflows": len(self.workflows), "running_tasks": len(self.running_tasks), "status": "active"}
