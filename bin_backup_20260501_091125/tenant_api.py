    ...
    def do_GET(self):
        ...
        if parsed.path == '/api/tenants':
            ...
            for d in TENANTS_DIR.glob("tenant_*"):
                ...
                tenants.append({"id": tenant_id, "name": config.get("name", f"租户{tenant_id}")})
    ...
    def do_DELETE(self):
        ...
        if self.path.startswith('/api/tenants/'):
            ...
            if tenant_dir.exists():
                ...
                self.send_json({"success": True, "message": f"Tenant {tenant_id} deleted"})
            else:
                self.send_json({"error": "tenant not found"}, 404)
        else:
            self.send_error(404)
...
if __name__ == "__main__":
    port = PORT_TENANT
    LOGGER.info("Tenant API started: http://localhost:%s", port)
