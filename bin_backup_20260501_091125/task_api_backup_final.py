...
class TaskHandler(BaseHTTPRequestHandler):
    ...
    def _handle_promo(self):
        try:
            ...
            # 修改：只用城市名检索，确保匹配您的图片
            tags = [city]
            print(f"🔍 DEBUG: 检索标签 {tags}", flush=True)
            personal = get_library_images_by_tags(tenant_id, tags, limit=6)
            print(f"🔍 DEBUG: 找到 {len(personal)} 张个人图片", flush=True)

            # 临时强制使用个人图片（如果存在），忽略 source 参数
            if personal:
                final_images = personal[:6]
                print(f"🎬 使用个人图片: {final_images[0]}", flush=True)
            else:
                # 没有个人图片才走外部
                final_images = fetch_external_images(city, style, 6)
                print(f"⚠️ 无个人图片，使用外部素材", flush=True)
    ...
    # 其他方法（_handle_library_upload, _handle_library_list, _handle_library_file_request, do_GET, do_POST 等）保持不变
    # 为了完整性，这里只保留必要的部分，实际使用时需要将之前版本中的完整内容复制过来。
    # 但由于长度限制，请执行下面的命令生成完整文件。

if __name__ == '__main__':
    init_db()
    port = 8084
    print(f"📋 Task API started: http://localhost:{port}")
    print("  POST /api/task/promo - 提交宣传片任务（强制优先个人图片）")
