...
class TrueLearningAgent:
    def __init__(self):
        self.memory_file = Path("str(smart_config.ROOT)/data/agent_brain.json")
    ...
    def match(self, symptom):
        ...
        knowledge_base = {
            "agent-api.*(errored|not_found)": {
                "name": "fix_agent_api",
                "cmd": "cd str(smart_config.ROOT)/bin && pm2 start agent_api.py --name agent-api -- --port 18103 2>/dev/null || pm2 restart agent-api",
                "verify": "pm2 list | grep agent-api | grep -q online"
            },
            "chat-api.*(errored|not_found)": {
                "name": "fix_chat_api",
                "cmd": "cd str(smart_config.ROOT)/bin && pm2 start chat_api.py --name chat-api -- --port 18109 2>/dev/null || pm2 restart chat-api",
                "verify": "pm2 list | grep chat-api | grep -q online"
            },
            "promo-api.*(errored|not_found)": {
                "name": "fix_promo_api",
                "cmd": "cd str(smart_config.ROOT)/bin && pm2 start promo_api.py --name promo-api -- --port 8108 2>/dev/null || pm2 restart promo-api",
                "verify": "pm2 list | grep promo-api | grep -q online"
            },
            "web.*stopped": {
                "name": "fix_web",
                "cmd": "cd str(smart_config.ROOT) && docker-compose up -d web 2>/dev/null || docker start clawsjoy-web",
