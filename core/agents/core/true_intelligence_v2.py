...
class TrueIntelligenceV2:
    ...
    def _handle_api_down(self, data):
        ...
        print(f"🚨 API 宕机: {data}")
        # 尝试重启
        subprocess.Popen("cd str(smart_config.ROOT) && python3 agent_gateway_web.py &", shell=True)
    ...
    def _handle_disk_full(self, data):
        """磁盘满处理"""
        print(f"💾 磁盘空间不足: {data}")
        subprocess.run("find str(smart_config.ROOT)/logs -name '*.log' -mtime +3 -delete", shell=True)
...
输出改进建议。"""
    ...
    def _check_ollama_latency(self):
        try:
            start = time.time()
            requests.get("http://smart_config.HOST:str(smart_config.get_port("ollama"))/api/tags", timeout=5)
