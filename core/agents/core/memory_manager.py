...
class MemoryManager:
    def __init__(self, tenant_id="tenant_1"):
        ...
        self.qdrant_url = "http://smart_config.HOST:6333"
        self.collection = f"mem_{tenant_id}"
        self.db_path = Path(f"config.ROOT/tenants/{tenant_id}/memory.db")
        self.json_path = Path(f"config.ROOT/tenants/{tenant_id}/memory.json")
        self._init()
