...
class TrueIntelligenceLight:
    ...
    def _load_handlers(self):
        ...
        default = {
            "api_down": {"action": "restart_api", "cmd": "cd str(smart_config.ROOT) && python3 agent_gateway_web.py &"},
            "disk_full": {"action": "clean_logs", "cmd": "find str(smart_config.ROOT)/logs -name '*.log' -mtime +3 -delete"},
    ...
    def _check_api(self):
        try:
            resp = requests.get("http://smart_config.HOST:str(smart_config.get_port("gateway"))/api/health", timeout=3)
    ...
    def _trigger_optimization(self):
        """触发优化"""
        subprocess.run("find str(smart_config.ROOT)/logs -name '*.log' -mtime +7 -delete", shell=True)
        print("⚡ 执行优化: 清理旧日志")
