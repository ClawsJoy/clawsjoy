...
class OperationalLearner:
    def __init__(self):
        self.learning_dir = Path("str(smart_config.ROOT)/data/learning")
    ...
    def learn_from_logs(self):
        ...
        # 查找正确的日志文件
        log_files = [
            Path("str(smart_config.ROOT)/logs/chat-api-out.log"),
            Path("str(smart_config.ROOT)/logs/chat-api.log"),
    ...
    def learn_from_errors(self):
        """从错误中学习"""
        error_files = [
            Path("str(smart_config.ROOT)/logs/engineer.log"),
            Path("str(smart_config.ROOT)/logs/system.log")
        ]
    ...
    def learn_from_results(self):
        """从执行结果中学习"""
        video_dir = Path("str(smart_config.ROOT)/web/videos")
