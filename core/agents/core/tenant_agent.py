...
class TenantAgent:
    def __init__(self, tenant_id="tenant_1"):
        ...
        self.config_file = Path(f"str(smart_config.ROOT)/tenants/{tenant_id}/agent_config.json")
        self._load_config()
    ...
    def _video(self, user_input):
        ...
        try:
            resp = requests.post("http://smart_config.HOST:8108/api/promo/make", json={"topic": topic, "duration": 30}, timeout=120)
    ...
    def _upload(self):
        try:
            result = subprocess.run(["python3", "str(smart_config.ROOT)/agents/youtube_uploader.py", "--tenant", self.tenant_id], capture_output=True, text=True, timeout=300)
    ...
    def _chat(self, user_input):
        try:
            resp = requests.post("http://smart_config.HOST:str(smart_config.get_port("ollama"))/api/generate", json={"model": config.LLM["fast_model"], "prompt": user_input, "stream": False, "options": {"num_predict": 500}}, timeout=60)
            if resp.status_code == 200:
...
if __name__ == "__main__":
    ...
    def _url_stats(self):
        ...
        url_file = Path("str(smart_config.ROOT)/data/urls/discovered.json")
        if url_file.exists():
    ...
    def _get_url_stats(self):
        ...
        url_file = Path("str(smart_config.ROOT)/data/urls/discovered.json")
        content_dir = Path("str(smart_config.ROOT)/data/content")
