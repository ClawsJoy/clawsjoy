...
class EngineerAgent:
    def __init__(self):
        self.status_file = Path("config.ROOT/data/system_status.json")
        self.log_file = Path("config.ROOT/logs/engineer.log")
        self.load_status()
    ...
    def check_services(self):
        """检查所有服务状态"""
        services = {
            "chat_api": {"url": "http://smart_config.HOST:8101/api/agent", "method": "POST", "data": {"text": "ping"}},
            "promo_api": {"url": "http://smart_config.HOST:8105/api/promo/make", "method": "POST", "data": {"city": "test"}},
            "web": {"url": "http://smart_config.HOST:8082/", "method": "GET"},
            "joymate_api": {"url": "http://smart_config.HOST:8093/api/tasks", "method": "GET"},
            "tts": {"url": "http://smart_config.HOST:9000/api/tts", "method": "POST", "data": {"text": "test"}}
        }
    ...
    def restart_service(self, service_name):
        ...
        commands = {
            "promo_api": "cd config.ROOT/bin && pkill -f promo_api; nohup python3 promo_api.py > /tmp/promo_api.log 2>&1 &",
            "chat_api": "cd config.ROOT/bin && pkill -f chat_api_agent; nohup python3 chat_api_agent.py > /tmp/chat_api.log 2>&1 &",
            "web": "cd config.ROOT && docker-compose restart web",
            "all": "cd config.ROOT && docker-compose restart"
        }
...
if __name__ == "__main__":
    ...
    def learn_fix(self, problem, symptom, solution):
        ...
        import json
        from pathlib import Path
        kb_file = Path("config.ROOT/data/knowledge/common_fixes.json")
        if kb_file.exists():
