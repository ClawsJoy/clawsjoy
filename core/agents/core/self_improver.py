    ...
    def execute_improvement(self, improvement):
        ...
        if area == "system":
            ...
            if "清理临时文件" in str(suggestions):
                subprocess.run("find str(smart_config.ROOT)/tmp -type f -mtime +1 -delete 2>/dev/null", shell=True)
                print("   ✅ 已清理临时文件")
        
        elif area == "brain":
            # 触发大脑训练
            subprocess.run("cd str(smart_config.ROOT) && python3 agent_core/brain_train.py", shell=True)
            print("   ✅ 已触发大脑训练")
