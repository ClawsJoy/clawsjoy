...
class SimpleLearningAgent:
    def __init__(self):
        self.project_root = Path("str(smart_config.ROOT)")
    ...
    def _rule_based_fix(self, service: str) -> Dict:
        fixes = {
            "agent-api": {
                ...
                "commands": [f"cd str(smart_config.ROOT)/bin && pm2 start agent_api.py --name agent-api -- --port 18103 2>/dev/null || pm2 restart agent-api"],
                "verify": "pm2 list | grep agent-api | grep -q online"
            },
            "chat-api": {
                "analysis": "重启chat-api",
                "commands": [f"cd str(smart_config.ROOT)/bin && pm2 start chat_api.py --name chat-api -- --port 18109 2>/dev/null || pm2 restart chat-api"],
                "verify": "pm2 list | grep chat-api | grep -q online"
            },
            "promo-api": {
                "analysis": "重启promo-api",
                "commands": [f"cd str(smart_config.ROOT)/bin && pm2 start promo_api.py --name promo-api -- --port 8108 2>/dev/null || pm2 restart promo-api"],
                "verify": "pm2 list | grep promo-api | grep -q online"
            },
            "health-api": {
                "analysis": "重启health-api",
                "commands": [f"cd str(smart_config.ROOT)/bin && pm2 start health_api.py --name health-api -- --port 18105 2>/dev/null || pm2 restart health-api"],
