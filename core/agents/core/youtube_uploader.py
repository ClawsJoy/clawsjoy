...
def get_tenant_config(tenant_id):
    """获取租户配置目录"""
    tenant_dir = Path(f"str(smart_config.ROOT)/tenants/{tenant_id}")
...
def upload_video(tenant_id, title=None, privacy="public"):
    ...
    # 获取租户最新的视频
    video_dir = Path(f"str(smart_config.ROOT)/tenants/{tenant_id}/videos")
    video_dir.mkdir(parents=True, exist_ok=True)
    videos = sorted(video_dir.glob("*.mp4"), key=lambda x: x.stat().st_mtime, reverse=True)
    
    if not videos:
        # 尝试全局视频目录
        global_video_dir = Path("str(smart_config.ROOT)/web/videos")
