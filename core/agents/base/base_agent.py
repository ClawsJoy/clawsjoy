...
def get_agent(agent_name: str, user_id: str = "default") -> Optional[BaseAgent]:
    ...
    def learn_experience(self, exp_type: str, content: Dict, confidence: float = 0.5):
        """学习经验"""
        if self.inheritance:
            return self.inheritance.learn(exp_type, content, confidence)
        return None
    
    def inherit_experience(self, parent_exp_id: str, adapter: Dict = None):
        """继承经验"""
        if self.inheritance:
    ...
    def reinforce_experience(self, exp_id: str, success: bool):
        """强化经验"""
        if self.inheritance:
            self.inheritance.reinforce(exp_id, success)
    
    def get_best_experience(self, exp_type: str = None):
        """获取最佳经验"""
        if self.inheritance:
    ...
    def _init_inheritance_config_driven(self):
        """配置驱动的传承系统初始化"""
        import yaml
        from pathlib import Path
    ...
    def save_communication(self, to_agent: str, request: str, response: dict, duration_ms: float = 0):
        """保存 Agent 间通信记录"""
        import json
        from datetime import datetime
        from pathlib import Path
    ...
    def record_communication(self, to_agent: str, request: str, response: dict, duration_ms: float = 0):
        """记录 Agent 间通信（基类方法）"""
        import json
        from datetime import datetime
        from pathlib import Path
    ...
    def send_and_record(self, to_agent: str, message: str) -> dict:
        """发送消息并自动记录"""
        import time
        import requests
