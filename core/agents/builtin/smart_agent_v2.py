...
class SmartAgentV2:
    ...
    def understand(self, user_input: str, session: Dict) -> Dict:
        ...
        # 1. 问候
        if any(g in lower for g in ["你好", "hi", "hello", "嗨", "你是谁", "你干啥", "你叫什么"]):
            return {"task": "greeting", "params": {}}
        
        # 2. 系统介绍
        if any(k in lower for k in ["clawsjoy是什么", "系统是什么", "介绍系统", "clawsjoy", "claws"]):
            return {"task": "system_intro", "params": {}}
        
        # 3. Agent 列表
        if any(k in lower for k in ['有哪些 agent', '列出 agent', 'agent 列表', '什么 agent']):
        ...
        # 5. 指代（它、这个、那个）→ 引用上一话题
        if any(k in lower for k in ['它', '这个', '那个', '他']) and session.get("last_topic"):
        ...
        # 6. 能力询问
        if any(k in lower for k in ['你会什么', '能做什么', '有什么功能', 'capability']):
...
• 配置驱动架构"""
        ...
        if task == "list_agents":
            ...
            return {"success": True, "data": agents, "type": "list", "count": len(agents)}
        ...
        if task == "list_capabilities":
            return {"success": True, "data": self.capabilities, "type": "list", "count": len(self.capabilities)}
        ...
        if task == "list_skills":
            ...
            return {"success": True, "data": skills[:15], "count": len(skills), "type": "list"}
