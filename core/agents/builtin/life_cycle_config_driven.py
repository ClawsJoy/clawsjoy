...
class LifeCycleConfigDriven:
    ...
    def _load_config(self) -> Dict:
        ...
        return {
            "reflection": {"interval": 5, "quality_thresholds": {"excellent": 0.8, "good": 0.5}},
    ...
    def _act(self, perception: Dict) -> Tuple[str, str]:
        ...
        if ptype == "greeting":
            ...
            return (f"你好，{perception.get('value')}！我是 ClawsJoy，很高兴认识你", "self_intro")
        elif ptype == "preference":
            return (f"好的，我记住了（我是 ClawsJoy）", "preference")
        elif ptype == "ask_name":
            return (f"当然记得！你是{name}呀" if name else "我是 ClawsJoy，你还没告诉我名字呢", "query_name")
        elif ptype == "ask_preference":
            return (f"你喜欢{', '.join(prefs)}" if prefs else "我是 ClawsJoy，你还没告诉我你的偏好呢", "query_pref")
        elif ptype == "list_agents":
            return ("我是 ClawsJoy 智能助手。系统有决策Agent、聊天Agent、执行Agent、采集Agent、安全Agent、分析Agent", "list_agents")
        else:
            # 简单回应，不调用 LLM
