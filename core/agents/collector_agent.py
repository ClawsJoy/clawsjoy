...
class CollectorAgent(BaseAgent):
    ...
    def _load_config(self):
        ...
        if config_file.exists():
            with open(config_file, 'r') as f:
                ...
                self.skill_templates = data.get('skill_templates', {})
                self.collection_config = data.get('collection', {})
                self.prefs_config = data.get('user_preferences', {})
        else:
    ...
    def _load_preferences(self) -> Dict:
        """加载用户偏好"""
        storage = self.prefs_config.get('storage', 'data/user_preferences.json')
