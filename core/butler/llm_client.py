...
class SmartLLMClient:
    ...
    def _init_provider(self):
        ...
        if self.provider == "ollama":
            self.api_url = unified_config.get('llm.endpoint', 'http://localhost:11434') + "/api/chat"
    ...
    def is_available(self) -> bool:
        ...
        if self.provider == "ollama":
            try:
                resp = requests.get(f"http://{unified_config.get('llm.endpoint', 'http://localhost:11434')}/api/tags", timeout=5)
