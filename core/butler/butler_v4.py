...
class ButlerV4:
    ...
    def _get_system_prompt(self) -> str:
        ...
        if preferences:
            pref_items = [
                f"- {k}: {v}" for k, v in preferences.items() if k != "butler_name"
            ]
            if pref_items:
                pref_text = f"\n\n用户偏好:\n" + "\n".join(pref_items)
    ...
    def _handle_todo(self, user_input: str) -> Optional[Dict]:
        ...
        if "待办" in user_input or "有什么任务" in user_input:
            ...
            if pending:
                ...
                return {
                    "response": f"您有 {len(pending)} 个待办事项：\n{tasks}",
                    "todos": pending,
                    "type": "todo_list",
                }
