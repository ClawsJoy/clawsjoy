...
class VectorStore:
    ...
    def __init__(self, user_id: str, collection: str = "default"):
        ...
        self.base_path = Path(
            f"{config_helper.get_data_root()}/v5/users/{user_id}/vectors/{collection}"
        )
    ...
    def add(self, content: str, metadata: Dict = None) -> str:
        ...
        self.documents.append(
            VectorDoc(
                id=doc_id, content=content, embedding=embedding, metadata=metadata or {}
            )
        )
    ...
    def search(self, query: str, limit: int = 5) -> List[Dict]:
        ...
        return [
            {"content": doc.content, "score": sim, "metadata": doc.metadata}
            for sim, doc in scores[:limit]
        ]
