...
class ActiveClosedLoop:
    ...
    def learn(self, sense_result: Dict, analysis_result: Dict, decision: Dict, execute_result: Dict):
        ...
        cycle_record = {
            ...
            "timestamp": datetime.now().isoformat(),
            "sense": {"issue_count": len(sense_result.get('quality', {}).get('issues', []))},
...
def self_improve(self) -> Dict:
    ...
    return {"goals": len(goal_setter.goals), "reflections": len(self_reflection.reflections)}
...
def analyze_with_heal(self, sense_result: Dict) -> Dict:
    ...
    if sense_result.get('health_degraded', False):
        ...
        unhealthy = [s for s, st in health.get('details', {}).items() if st != 'healthy']
        for u in unhealthy:
