...
class UnifiedDecisionAgent:
    ...
    def _save_state(self):
        ...
        with open(state_file, 'w') as f:
            json.dump({'history': self.decision_history[-200:]}, f, indent=2, default=str)
    ...
    def decide(self, analysis: Dict, status: Dict) -> Dict:
        if not analysis['has_issues']:
            return {"action": "idle", "target": "none", "level": "safe", "reason": "系统正常"}
        if not status['backend']:
            return {"action": "alert", "target": "start_backend", "level": "warning", 
                    "reason": "后端服务未运行，请启动 agent_gateway_web.py"}
        for issue in analysis['issues']:
            if '技能' in issue:
                return {"action": "fix", "target": "sync_skills", "level": "warning", "reason": issue}
            if '健康' in issue:
                return {"action": "check", "target": "health", "level": "safe", "reason": issue}
        return {"action": "idle", "target": "none", "level": "safe", "reason": "无需处理"}
    ...
    def run_cycle(self) -> Dict:
        print(f"\n🔄 [{self.name}] 工作周期")
        status = self.sense()
        print(f"  📡 后端:{status['backend']}, 技能:{status['skills']}, 健康:{status['health']}")
        ...
        print(f"  ⚡ 结果: {'✅' if result['success'] else '⚠️'} {result.get('result', '')[:50]}")
