...
"""
                    ...
                    registered.append({"name": agent_dir.name, "status": "registered"})
    ...
    def register_new_agent(
        self, agent_name: str, config: Dict, user_id: str = "system"
    ) -> bool:
        """注册新的自定义 Agent"""
        try:
            # 创建工作区目录
...
'''
            ...
            capability_desc = (
                f"角色: {role.get('title', agent_name)} 职责: {', '.join(capabilities)}"
            )

            vector_knowledge_center.add_agent_capability(
                agent_name=agent_name, capability_desc=capability_desc, user_id=user_id
            )
            return True
        except Exception as e:
            print(f"注册失败: {e}")
