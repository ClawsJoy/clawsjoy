...
class FeedbackSystem:
    ...
    def get_stats(self) -> Dict:
        ...
        recent_success = sum(1 for s in data["success"] if datetime.fromisoformat(s["timestamp"]) > week_ago)
        recent_failure = sum(1 for f in data["failure"] if datetime.fromisoformat(f["timestamp"]) > week_ago)
        ...
        return {
            ...
            "avg_response_time": sum(s.get("duration", 0) for s in data["success"][-100:]) / 100 if data["success"] else 0
        }
    ...
    def should_trigger_meeting(self, user_id: str = None) -> Dict:
        ...
        if success_rate < 0.5:
            result = {"should": True, "reason": f"成功率过低: {success_rate*100:.0f}%", "urgency": "high"}
        elif success_rate < 0.7:
            result = {"should": True, "reason": f"成功率偏低: {success_rate*100:.0f}%", "urgency": "medium"}
        ...
        if user_id:
            ...
            if current_score < 40:
                result = {"should": True, "reason": f"用户{user_id}满意度极低: {current_score}", "urgency": "high"}
            elif current_score < 60:
                result = {"should": True, "reason": f"用户{user_id}满意度偏低: {current_score}", "urgency": "medium"}

        recent = stats.get("recent_7d", {})
        failures = recent.get("failure", 0)
        if failures > 10:
            result = {"should": True, "reason": f"最近7天失败次数: {failures}", "urgency": "medium"}
