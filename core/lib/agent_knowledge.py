#!/usr/bin/env python3
"""Agent Knowledge - Agent Knowledge 模块
...
class AgentKnowledgeBase:
    ...
    def __init__(self, knowledge_base_name: str = "agents_knowledge"):
        ...
        self.persist_dir = (
            Path(path_manager.get("data.vector_kb")) / knowledge_base_name
        )
        ...
        self.collections = {
            ...
            "user_instructions": self.client.get_or_create_collection(
                "user_instructions"
            ),
            "learning_patterns": self.client.get_or_create_collection(
                "learning_patterns"
            ),
        }
    ...
    def add_skill_doc(
        self, skill_name: str, description: str, usage: str, example: str = ""
    ):
        """添加技能文档到知识库"""
        content = (
            f"技能: {skill_name}\n描述: {description}\n用法: {usage}\n示例: {example}"
    ...
    def add_learning_pattern(self, pattern: str, context: str, success: bool = True):
        ...
        doc_id = hashlib.md5(
            f"pattern_{datetime.now().isoformat()}".encode()
        ).hexdigest()[:16]
