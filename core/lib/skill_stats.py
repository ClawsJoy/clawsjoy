...
class SkillStats:
    ...
    def _load(self):
        if self.stats_file.exists():
            ...
            self.stats = {"total": 0, "success": 0, "failed": 0, "by_skill": {}, "by_user": {}}
    ...
    def get_recommendations(self, user_id: str, task: str) -> list:
        ...
        sorted_skills = sorted(skills_used.items(), key=lambda x: -x[1].get("success", 0))
        return [s[0] for s in sorted_skills[:5]]
    
    def get_top_skills(self, limit: int = 10) -> list:
        """获取最常用技能"""
        sorted_skills = sorted(self.stats["by_skill"].items(), key=lambda x: -x[1]["total"])
        return [{"name": s[0], "total": s[1]["total"], "success_rate": s[1]["success"]/s[1]["total"] if s[1]["total"]>0 else 0} 
