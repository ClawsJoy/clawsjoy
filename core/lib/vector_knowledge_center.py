    ...
    def _init(self):
        ...
        self.collections = {
            "skills": self.client.get_or_create_collection(
                name="knowledge_skills", metadata={"description": "技能知识库"}
            ),
            "agents": self.client.get_or_create_collection(
                name="knowledge_agents", metadata={"description": "Agent知识库"}
            ),
            "routes": self.client.get_or_create_collection(
                name="knowledge_routes", metadata={"description": "路由知识库"}
            ),
            "memories": self.client.get_or_create_collection(
                name="knowledge_memories", metadata={"description": "记忆知识库"}
            ),
            "documents": self.client.get_or_create_collection(
                name="knowledge_documents", metadata={"description": "文档知识库"}
            ),
        }
...
性格: {metadata.get('personality', 'professional')}
    ...
    def add_memory(self, content: str, user_id: str, category: str = "general") -> str:
        """添加记忆知识"""
        doc_id = hashlib.md5(
            f"memory_{user_id}_{datetime.now().isoformat()}".encode()
        ).hexdigest()[:16]
                        ...
                        results.append(
                            {
                                ...
                                "content": doc[:300],
                                "score": (
                                    1 - search_results["distances"][0][i]
    ...
    def get_stats(self) -> Dict:
        """获取统计信息"""
        return {
            name: collection.count() for name, collection in self.collections.items()
        }
    ...
    def get_member_vector_stats(self) -> Dict:
        ...
        try:
            count = collection.count()
            return {"exists": True, "count": count, "collection_name": "butler_members"}
        except Exception as e:
...
文件: {metadata.get('file', '')}
            ...
            metadatas=[
                {
                    "doc_id": doc_id,
                    "type": (
                        metadata.get("type", "document") if metadata else "document"
                    ),
    ...
    def search_with_cache(
        self, query: str, knowledge_type: str = None, n: int = 10
    ) -> list:
        """带缓存的检索"""
    ...
    def add_agent_capability(
        self, agent_name: str, capability_desc: str, user_id: str = "system"
    ) -> str:
    ...
    def find_best_agent_exclude_self(
        self, query: str, exclude_agent: str, user_id: str = None, top_k: int = 5
    ) -> list:
