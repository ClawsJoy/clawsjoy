...
class ConditionalEventBus(EventBus):
    ...
    def when(self, condition_name: str, check_func: Callable, 
             trigger_event: str, trigger_data: Any = None,
             interval: int = 60):
    ...
    def when_threshold(self, name: str, get_value: Callable,
                    ...
                    try:
                        ...
                        if th['operator'] == '>':
                            if current > th['threshold'] and not th['triggered']:
                                self.emit(th['event'], {'value': current, 'threshold': th['threshold']})
                                th['triggered'] = True
