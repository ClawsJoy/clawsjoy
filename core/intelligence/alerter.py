...
class IntelligentAlerter:
    def __init__(self):
        self.thresholds_file = Path(
            f"{config_helper.get_data_root()}/dynamic_thresholds.json"
        )
    ...
    def adapt_threshold(self, metric_name, recent_values):
        ...
        self.thresholds[metric_name]["warning"] = (
            0.7 * old_warning + 0.3 * dynamic_warning
        )

        print(
            f"📊 自适应阈值: {metric_name} {old_warning:.2f} -> {self.thresholds[metric_name]['warning']:.2f}"
        )
    ...
    def check_and_alert(self, metric_name, current_value):
        ...
        if current_value >= threshold.get("critical", float("inf")):
            return {
                "level": "critical",
                "message": f"{metric_name} 严重异常: {current_value}",
            }
        elif current_value >= threshold.get("warning", float("inf")):
            return {
                "level": "warning",
                "message": f"{metric_name} 告警: {current_value}",
            }
