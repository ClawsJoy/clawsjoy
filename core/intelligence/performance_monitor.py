...
class PerformanceMonitor:
    def __init__(self):
        ...
        self.metrics_file = Path(
            f"{config_helper.get_data_root()}/performance_metrics.json"
        )
    ...
    def get_trend(self):
        ...
        return {
            "cpu_trend": (
                "上升" if cpu_trend > 5 else "下降" if cpu_trend < -5 else "稳定"
            ),
            "memory_trend": (
                "上升" if mem_trend > 5 else "下降" if mem_trend < -5 else "稳定"
            ),
        }
    ...
    def generate_report(self):
        ...
        print(
            f"💾 内存: {metrics['memory']['percent']}% ({metrics['memory']['used_gb']:.1f}/{metrics['memory']['total_gb']:.1f} GB)"
        )
        print(
            f"💿 磁盘: {metrics['disk']['percent']}% (剩余 {metrics['disk']['free_gb']:.1f} GB)"
        )
