...
class BrainDecisionExecutor:
    ...
    def __init__(self):
        ...
        self.decision_log = Path(
            f"{config_helper.get_data_root()}/brain_decisions.json"
        )
    ...
    def brain_decide(self, state):
        ...
        # 决策1: 服务异常 -> 自动修复
        down_services = [
            name for name, healthy in state["services"].items() if not healthy
        ]
        if down_services:
            decisions.append(
                {
    ...
    def _optimize_learning(self):
        ...
        self.notifier.send(
            "学习优化", f"学习率: {current_rate:.2f} -> {new_rate:.2f}", "info"
        )
        brain.record_experience(
            agent="brain_decision",
            action="optimize_learning",
    ...
    def _explore_and_learn(self):
        ...
        try:
            # 尝试调用文档生成
            resp = requests.post(
                "http://smart_config.HOST:5008/md",
    ...
    def _cleanup_disk(self):
        ...
        # 清理旧备份
        subprocess.run(
            "find backups/clawsjoy -name '*.tar.gz' -mtime +30 -delete 2>/dev/null",
            shell=True,
        )
    ...
    def execute(self):
        ...
        print(f"\n📊 当前状态:")
        print(
            f"   🧠 大脑: {state['brain']['experiences']}经验, {state['brain']['success_rate']*100:.0f}%成功率"
        )
        print(f"   📡 服务: {sum(state['services'].values())}/4 正常")
        print(
            f"   💾 资源: CPU {state['performance']['cpu_percent']}% / 内存 {state['performance']['memory_percent']}% / 磁盘 {state['performance']['disk_percent']}%"
        )
