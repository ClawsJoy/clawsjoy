...
class IntelligenceServiceV4:
    ...
    def _init_services(self):
        ...
        self.predict = (
            predictor_module.predict
            if predictor_module and hasattr(predictor_module, "predict")
            else self._default_predict
        )
        ...
        self.decide = (
            ...
            if decision_module and hasattr(decision_module, "decide")
            else self._default_decide
        )
        ...
        self.learn = (
            ...
            if learner_module and hasattr(learner_module, "learn")
            else self._default_learn
        )
        ...
        self.run_loop = (
            loop_module.run
            if loop_module and hasattr(loop_module, "run")
            else self._default_loop
        )

        # 监控服务
        monitor_module = self.registry.get("success_monitor")
        self.monitor = (
            monitor_module.check_and_alert
            if monitor_module and hasattr(monitor_module, "check_and_alert")
            else self._default_monitor
        )
