#!/usr/bin/env python3
"""Code Completer - Code Completer 模块
...
class CodeCompleter:
    ...
    def load_history(self):
        """加载历史"""
        history_file = Path(f"{config_helper.get_data_root()}/completion_history.json")
        if history_file.exists():
            with open(history_file, "r") as f:
                data = json.load(f)
                self.context_memory = data.get("contexts", [])
                self.completion_stats = data.get(
                    "stats", defaultdict(lambda: {"used": 0, "accepted": 0})
                )
    ...
    def suggest_completion(self, partial_code, file_type="python"):
        ...
        for exp in experiences[-20:]:
            ...
            if partial_code.lower() in action.lower():
                suggestions.append(
                    {"completion": action, "type": "brain_memory", "confidence": 0.7}
                )
    ...
    def get_smart_suggestions(self, current_input):
        ...
        # 排序：先高置信度，再常用
        for s in suggestions:
            stats = self.completion_stats.get(
                s["completion"], {"used": 0, "accepted": 0}
            )
            s["popularity"] = stats["used"]

        suggestions.sort(
            key=lambda x: (x["confidence"], x.get("popularity", 0)), reverse=True
        )
