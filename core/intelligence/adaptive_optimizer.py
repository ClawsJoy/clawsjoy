...
class AdaptiveOptimizer:
    ...
    def load_params(self):
        ...
        param_file = Path(f"{config_helper.get_data_root()}/adaptive_params.json")
    ...
    def save_params(self):
        ...
        with open(f"{config_helper.get_data_root()}/adaptive_params.json", "w") as f:
    ...
    def optimize(self):
        ...
        # 自适应调整
        if rate < 40:
            self.params["quality_threshold"] = max(
                50, self.params["quality_threshold"] - 10
            )
            self.params["target_duration"] = max(
                30, self.params["target_duration"] - 10
            )
        elif rate > 80:
            self.params["quality_threshold"] = min(
                90, self.params["quality_threshold"] + 5
            )
            self.params["target_duration"] = min(
                120, self.params["target_duration"] + 10
            )
