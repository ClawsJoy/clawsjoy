...
class TrueBrain:
    ...
    def _listen_services(self):
        ...
        services = {
            'gateway': 'http://smart_config.HOST:str(smart_config.get_port("gateway"))/health',
            'file': 'http://smart_config.HOST:5003/health',
            'agent': 'http://smart_config.HOST:str(smart_config.get_port("multi_agent"))/health',
    ...
    def _decide_fix_service(self, service_name):
        ...
        import subprocess
        subprocess.Popen("./restart_services.sh", shell=True, cwd="str(smart_config.ROOT)")
