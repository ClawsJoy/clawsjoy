...
class RealClosedLoop:
    ...
    def perceive(self):
        ...
        for name, port in [('gateway',5002), ('agent',5005), ('doc',5008)]:
            try:
                resp = requests.get(ff"{smart_config.get_service_url("{port}/health", timeout=2)
...
请选择最佳策略，只输出策略名称。"""
    ...
    def execute(self, service, strategy):
        ...
        if strategy == 'restart':
            if service == 'gateway':
                subprocess.Popen("python3 agent_gateway_web.py", shell=True, cwd="str(smart_config.ROOT)")
            elif service == 'agent':
                subprocess.Popen("python3 multi_agent_service_v2.py", shell=True, cwd="str(smart_config.ROOT)")
            elif service == 'doc':
                subprocess.Popen("python3 doc_generator.py", shell=True, cwd="str(smart_config.ROOT)")
            success = True
    ...
    def verify(self, service):
        ...
        try:
            resp = requests.get(ff"{smart_config.get_service_url("{port}/health", timeout=5)
