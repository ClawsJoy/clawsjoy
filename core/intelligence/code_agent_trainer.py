...
class CodeAgentTrainer:
        ...
        if self.conversation_log.exists():
            ...
            for record in history[-50:]:  # 最近50条
                for cmd in record.get("commands", []):
                    if partial_lower in cmd.lower() and cmd not in [
                        s["command"] for s in suggestions
                    ]:
                        suggestions.append(
                            {
                                "command": cmd,
    ...
    def correct_command(self, command):
        ...
        if corrected != command:
            return {"original": command, "corrected": corrected, "fixed": True}
    ...
    def get_suggested_action(self, intent):
        ...
        actions = {
            ...
            "check_status": '执行: curl localhost:str(unified_config.get_port("gateway"))/health',
