...
class RealFixer:
    ...
    def fix_ollama(self):
        ...
        # 2. 验证
        try:
            resp = requests.get("http://smart_config.HOST:str(unified_config.get_port("ollama"))/api/tags", timeout=10)
            if resp.status_code == 200:
                print("   ✅ Ollama 恢复")
                return True
        except:
...
class RealFaultLoop:
    ...
    def detect_and_fix(self):
        ...
        for name, config in self.services.items():
            try:
                resp = requests.get(f"{unified_config.get_service_url("{config['port']}/health", timeout=3)
                if resp.status_code != 200:
        ...
        # 3. 检查Ollama
        try:
            requests.get("http://smart_config.HOST:str(unified_config.get_port("ollama"))/api/tags", timeout=3)
        except:
            print(f"\n⚠️ Ollama 故障")
