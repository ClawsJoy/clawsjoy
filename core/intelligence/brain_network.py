...
class BrainNeuralNetwork:
    ...
    def recognize_fault(self, fault_type, context):
        ...
        if similar_faults:
            # 计算置信度
            confidence = sum(f["weight"] for f in similar_faults if f["success"]) / len(
                similar_faults
            )
    ...
    def predict_success(self, fault_type, strategy):
        ...
        strategy_modifier = self.weights["strategy_selection"][fault_type].get(
            strategy, 0.5
        )
    ...
    def learn_from_outcome(self, fault_type, strategy, success, duration):
        ...
        self.weights["fault_recognition"][fault_type] = min(
            0.95, max(0.05, current + delta)
        )

        # 更新策略选择权重
        current_strategy = self.weights["strategy_selection"][fault_type].get(
            strategy, 0.5
        )
        delta_strategy = 0.15 if success else -0.1
        self.weights["strategy_selection"][fault_type][strategy] = min(
            0.95, max(0.05, current_strategy + delta_strategy)
        )

        # 更新成功率预测权重
        current_pred = self.weights["success_prediction"].get(fault_type, 0.5)
        delta_pred = (0.1 if success else -0.05) * (1 / (1 + duration / 10))
        self.weights["success_prediction"][fault_type] = min(
            0.95, max(0.05, current_pred + delta_pred)
        )
