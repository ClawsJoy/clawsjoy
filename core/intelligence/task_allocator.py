...
class SmartTaskAllocator:
    ...
    def get_agent_capabilities(self) -> dict:
        """获取所有Agent能力"""
        try:
            resp = requests.get("http://smart_config.HOST:str(smart_config.get_port("multi_agent"))/agents", timeout=3)
    ...
    def _execute_task(self, agent: str, task: str) -> dict:
        ...
        try:
            resp = requests.post("http://smart_config.HOST:str(smart_config.get_port("multi_agent"))/execute", 
