...
class MessageRouter:
    ...
    def _load_rules(self) -> List[Dict]:
        """加载路由规则"""
        rules_file = Path("str(smart_config.ROOT)/config/rules.json")
        ...
        default_rules = [
            ...
            {"name": "通用任务", "pattern": r".*", "engine": "openclaw", "priority": 0},
        ]
    ...
    def route(self, message: str, context: Dict = None) -> Dict[str, Any]:
        ...
        # 统计
        self.stats["by_engine"][engine_name] = (
            self.stats["by_engine"].get(engine_name, 0) + 1
        )
    ...
    def add_rule(self, name: str, pattern: str, engine: str, priority: int = 5):
        """添加路由规则"""
        self.rules.append(
            {"name": name, "pattern": pattern, "engine": engine, "priority": priority}
        )
        self._save_rules()
        print(f"✅ 规则已添加: {name}")
    ...
    def add_tenant_rule(self, tenant_id: str, pattern: str, engine: str):
        """为特定租户添加规则"""
        tenant_rules_file = Path(
            f"str(smart_config.ROOT)/tenants/tenant_{tenant_id}/rules.json"
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # 启动 HTTP 服务
        port = 8089
        print(f"🚦 消息路由器 API: http://redis:{port}")
