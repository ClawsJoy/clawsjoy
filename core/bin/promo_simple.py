...
PORT = 8108
IMAGE_DIR = Path("str(smart_config.ROOT)/web/images")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            per_image = max(3, min(8, 180 / len(images)))
            ...
            if video_path.exists() and video_path.stat().st_size > 0:
                self.send_json({"success": True, "video_url": f"/videos/{video_name}", "image_count": len(images), "duration": per_image * len(images)})
            else:
