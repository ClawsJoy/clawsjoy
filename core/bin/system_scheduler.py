...
class SystemScheduler:
    def __init__(self):
        self.log_file = Path("str(smart_config.ROOT)/logs/system.log")
    ...
    def run_health_check(self):
        ...
        # Chat API
        try:
            resp = requests.post("http://smart_config.HOST:18109/api/agent", json={"text": "ping"}, timeout=10)
        ...
        # Promo API
        try:
            resp = requests.post("http://smart_config.HOST:8108/api/promo/make", json={"city": "test"}, timeout=10)
        ...
        # Web
        try:
            resp = requests.get("http://smart_config.HOST:18083/", timeout=10)
    ...
    def run_cleanup(self):
        self.log("🧹 开始清理临时文件...")
        subprocess.run("rm -f str(smart_config.ROOT)/web/audio/tts_*.mp3 2>/dev/null", shell=True)
