...
import json, os, uuid, mimetypes, requests
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/api/list_engines':
            ...
            lib_dir = Path(f"."/tenants/tenant_{tenant_id}/library/images")
    ...
    def do_POST(self):
        if self.path == '/api/chat':
            ...
            try:
                ...
                reply = f'连接 AI 服务失败: {str(e)}'
            
            self.send_json({'message': reply})
...
if __name__ == '__main__':
    port = 8084
    print(f"API Server with Ollama on http://localhost:{port}")
