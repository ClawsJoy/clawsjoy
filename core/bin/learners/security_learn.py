...
def learn_new_patterns():
    patterns_file = Path("smart_config.ROOT/data/sensitive_patterns.json")
    log_file = Path("smart_config.ROOT/logs/chat-api-out.log")
    ...
    if matches:
        potential_patterns.append(
            {"type": "api_key", "pattern": api_key_pattern, "learned_at": time.time()}
        )
    ...
    if re.search(token_pattern, logs):
        potential_patterns.append(
            {"type": "token", "pattern": token_pattern, "learned_at": time.time()}
        )
