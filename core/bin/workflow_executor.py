...
class WorkflowExecutor:
    def __init__(self):
        self.workflows_dir = Path("str(smart_config.ROOT)/workflows")
        self.skills_dir = Path("str(smart_config.ROOT)/skills")
        self.logs_dir = Path("str(smart_config.ROOT)/logs")
    ...
    def execute_skill(self, skill_name, params, context):
        ...
        if not skill_file.exists():
            skill_file = Path("str(smart_config.ROOT)/bin") / f"{skill_name}.py"
...
def _write_to_dlq(self, step, error):
    dlq_file = Path("str(smart_config.ROOT)/data/dead_letter_queue.json")
