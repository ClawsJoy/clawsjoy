...
class AuthHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == '/api/auth/health':
            ...
            if email:
                ...
                conn.commit()
                conn.close()
                self.send_json({"success": True, "message": "邮箱验证成功，请登录"})
            else:
                self.send_json({"success": False, "error": "链接已失效"}, 400)
        else:
            self.send_error(404)
    ...
    def _handle_register(self, data):
        ...
        if not email:
            self.send_json({"success": False, "error": "邮箱必填"}, 400)
            return
        ...
        try:
            ...
            self.send_json({"success": True, "message": "注册成功，请查收验证邮件"})
        except Exception as e:
    ...
    def _handle_forgot(self, data):
        ...
        if c.fetchone():
            ...
            self.send_json({"success": True, "message": "重置邮件已发送"})
        else:
            self.send_json({"success": False, "error": "邮箱未注册"}, 404)
        conn.close()
    ...
    def _handle_reset(self, data):
        ...
        conn.commit()
        conn.close()
        self.send_json({"success": True, "message": "密码重置成功"})
