...
# ============ 配置 ============
SECRET_KEY = os.environ.get("CLAWSJOY_SECRET", "default-fallback-key-change-me")
DB_PATH = "str(smart_config.ROOT)/data/auth.db"
TOKEN_EXPIRY_DAYS = 7
...
FRONTEND_URL = os.environ.get("FRONTEND_URL", "http://redis:8082")
...
def init_db():
    Path("str(smart_config.ROOT)/data").mkdir(parents=True, exist_ok=True)
    ...
    if c.fetchone()[0] == 0:
        ...
        c.execute(
            "INSERT INTO users (username, password, tenant_id, role, email_verified) VALUES (?,?,?,?,?)",
            ("admin", pwd, "0", "admin", 1),
        )
        conn.commit()
    conn.close()
...
class AuthHandler(BaseHTTPRequestHandler):
    ...
    def _handle_login(self, data):
        ...
        c = conn.cursor()
        c.execute(
            "SELECT username, tenant_id, role, email_verified FROM users WHERE username=? AND password=?",
        ...
        if user:
            ...
            send_success(
                ...
                {
                    "token": token,
                    "user": {"name": user[0], "tenant_id": user[1], "role": user[2]},
                },
                "登录成功",
            )
        else:
    ...
    def _handle_register(self, data):
        ...
        try:
            c.execute(
                "INSERT INTO users (username, password, email, tenant_id, role) VALUES (?,?,?,?,?)",
                (
                    data.get("username"),
...
if __name__ == "__main__":
    init_db()
    port = 8092
    print(f"🔐 Auth API: http://redis:{port}")
