...
def monitoring_skills():
    ...
    stats = {cat: info["count"] for cat, info in categories.items()}
    stats["total"] = len(skill_loader.list_all())
...
def monitoring_tasks():
    ...
    for t in tasks[-limit:]:
        result.append({"raw": t[:100], "success": "成功" in t})
    return jsonify(result)
...
if __name__ == "__main__":
    port = smart_config.PORTS.get("web_monitor", 5021)
