...
GPU_QUEUE = Queue()
GPU_LOCK = Path("str(smart_config.ROOT)/tasks/gpu.lock")
TASKS_DIR = Path("str(smart_config.ROOT)/tasks")
...
class GPUManager:
    ...
    def process_queue(self):
        ...
        while True:
            ...
            # 获取待处理任务
            queue_files = sorted(
                TASKS_DIR.glob("gpu_queue_*.json"), key=lambda x: x.stat().st_mtime
            )
