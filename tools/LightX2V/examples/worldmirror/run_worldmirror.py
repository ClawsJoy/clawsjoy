...
DEFAULT_CONFIG_PATH = os.path.join(
    _REPO_ROOT, "configs/worldmirror/worldmirror_recon.json"
)
...
def build_parser():
    ...
    p.add_argument(
        ...
        type=str,
        default="/data/nvme1/models/HY-World-2.0",
        help="Local directory containing HY-WorldMirror-2.0 weights.",
    )
    p.add_argument(
        "--subfolder",
        type=str,
        default="HY-WorldMirror-2.0",
        help="Subfolder inside the model directory.",
    )
    p.add_argument(
        "--config_path",
        type=str,
        default=None,
        help="Optional training YAML; used with --ckpt_path.",
    )
    p.add_argument(
        "--ckpt_path",
        type=str,
        default=None,
        help="Optional .ckpt/.safetensors; used with --config_path.",
    )
    p.add_argument(
        "--lightx2v_config",
        type=str,
        default=DEFAULT_CONFIG_PATH,
        help="LightX2V JSON config (saving/mask/render defaults).",
    )
    ...
    p.add_argument(
        "--sky_mask_source", type=str, default=None, choices=["auto", "model", "onnx"]
    )
...
def main():
    ...
    from lightx2v.models.runners.worldmirror.worldmirror_runner import (  # noqa: F401  — registers "worldmirror"
        WorldMirrorRunner,
        _broadcast_string,
    )
    ...
    cuda_backend = None
    if is_distributed:
        cuda_backend = dist.distributed_c10d._get_default_group()._get_backend(
            torch.device("cuda")
        )
    ...
    try:
        ...
        while True:
            ...
            if rank == 0 and not (
                Path(new_input).is_dir() or Path(new_input).is_file()
            ):
