...
class LoraTrainer(BaseTrainer):
    def __init__(self, config):
        ...
        self.gradient_accumulation_iters = self.training_config[
    ...
    def setup(self, resume_ckpt_path=None):
        ...
        if resume_ckpt_path is not None:
            ...
            if os.path.exists(training_state_path):
                ...
                print(
                    f"Warning: training_state.pt not found in {resume_ckpt_path}. States not restored."
                )
    ...
    def compute_loss_on_sample(self, sample):
        with torch.no_grad():
            ...
            noisy_latent = self.noise_scheduler.add_noise(
                latent, noise, timestep_or_sigma
            )
        ...
        loss = torch.mean(
            ((prediction.float() - target.float()) ** 2).reshape(target.shape[0], -1),
            dim=1,
        )
        return loss.mean()
    ...
    def _resolve_resume(self):
        ...
        if ckpt_path is None:
            print(
                f"Auto-resume enabled but no checkpoint found in '{self.output_train_dir}'. Starting from scratch."
            )
        else:
            print(
                f"Auto-resuming from checkpoint: {ckpt_path} (iteration {current_iter})"
            )
    ...
    def train(self):
        ...
        progress = tqdm(
            total=max_train_iters, desc="Training iterations", initial=current_iter
        )
        if self.infer_every_iters:
        ...
        while current_iter < max_train_iters:
            for sample in self.dataloader_train:
                ...
                torch.nn.utils.clip_grad_norm_(
                    self.model.transformer.parameters(), max_grad_norm
                )
                self.optimizer.step()
                ...
                progress.set_postfix(
                    loss=running_loss, lr=self.lr_scheduler.get_last_lr()[0]
                )
                running_loss = 0.0
