...
class ControllerService(BaseService):
    def __init__(self):
        ...
        self._graceful_reclaim_timeout_seconds: float = float(
            os.getenv("DISAGG_RECLAIM_GRACEFUL_TIMEOUT_SECONDS", "30.0")
        )
        self._force_kill_wait_seconds: float = float(
            os.getenv("DISAGG_RECLAIM_FORCE_KILL_WAIT_SECONDS", "1.0")
        )
        self._instance_start_timeout_seconds: float = float(
            os.getenv("DISAGG_INSTANCE_START_TIMEOUT_SECONDS", "90.0")
        )
        self._sidecar_start_timeout_seconds: float = float(
            os.getenv("DISAGG_SIDECAR_START_TIMEOUT_SECONDS", "15.0")
        )
        self._sidecar_drain_idle_seconds: float = float(
            os.getenv("DISAGG_SIDECAR_DRAIN_IDLE_SECONDS", "1.0")
        )
        # <= 0 means wait indefinitely until sidecar pending queues are drained.
        self._sidecar_drain_timeout_seconds: float = float(
            os.getenv("DISAGG_SIDECAR_DRAIN_TIMEOUT_SECONDS", "0")
        )
        self._remote_proxy_start_timeout_seconds: float = float(
        ...
        self._metrics_output_json = Path(
            os.getenv(
                ...
                str(
                    ...
                    / "save_results"
                    / "disagg_controller_metrics.json"
                ),
            )
        )
    ...
    def _wait_for_tcp_port_state(
        self, host: str, port: int, should_be_open: bool, timeout_seconds: float
    ) -> bool:
    ...
    def _maybe_wrap_service_command_with_nsys(
        ...
        *,
        ...
        if nsys_bin is None:
            self.logger.warning(
                "DISAGG_ENABLE_NSYS is set but nsys is not available, skip profiling for %s rank=%s",
                instance_type,
                engine_rank,
            )
            return command
        ...
        if output_dir_raw:
            ...
            base_save_path = (
                instance_cfg.get("save_path")
                or (self._runtime_config or {}).get("save_path")
                or str(
                    Path(__file__).resolve().parents[3]
                    / "save_results"
                    / "wan22_i2v_dynamic.mp4"
                )
            )
    ...
    def _merge_request_metrics(
        self, existing: dict[str, Any] | None, update: dict[str, Any] | None
    ) -> dict[str, Any]:
        merged: dict[str, Any] = {}
    ...
    def _run_centralized_ok_server(
        self, stop_event: Event, bind_host: str, bind_port: int
    ):
        controller = self
    ...
    def _ensure_remote_instance_proxy(self, slot: dict[str, Any]):
        ...
        proxy_req_port = int(slot["proxy_req_port"])
        proxy_log_path = str(
            slot.get("proxy_log_path", f"{log_dir}/instance_proxy.log")
        )
        ...
        while time.time() < deadline:
            ...
            if isinstance(probe, dict) and probe.get("ok", False):
                self.logger.info(
                    "Remote instance proxy is ready on host=%s req_addr=%s",
                    slot.get("host"),
                    req_addr,
                )
                return
        ...
        raise RuntimeError(
            f"remote instance proxy failed to start on host={slot.get('host')} req_addr={req_addr}"
        )

    def _start_sidecar_process(
        self, instance_type: str, gpu_id: str | int, bind_host: str | None = None
    ) -> dict[str, Any]:
        host = str(bind_host or self._bootstrap_addr)
        ...
        if not ready:
            if process.poll() is None:
                ...
                try:
                    ...
                    process.kill()
            raise RuntimeError(
                f"sidecar server failed to start for {instance_type} gpu={gpu_id}"
            )
    ...
    def _launch_remote_instance(
        self, slot: dict[str, Any], instance_type: str, cmd: list[str], cuda_device: str
    ) -> tuple[dict[str, Any], dict[str, Any]]:
        if self._is_truthy(slot.get("use_remote_proxy", False)):
            return self._launch_remote_instance_via_proxy(
                slot, instance_type, cmd, cuda_device
            )
        ...
        service_cmd = _with_env(
            " ".join(shlex.quote(str(part)) for part in cmd_with_python),
            service_env_vars,
        )
    ...
    def _launch_remote_instance_via_proxy(
        self, slot: dict[str, Any], instance_type: str, cmd: list[str], cuda_device: str
    ) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        if not isinstance(reply, dict) or not reply.get("ok", False):
            raise RuntimeError(
                f"remote proxy failed to start instance on host={host}: {reply}"
            )
        ...
        if sidecar_pid <= 0 or service_pid <= 0:
            raise RuntimeError(
                f"remote proxy returned invalid pids for host={host}: {reply}"
            )
    ...
    def _stop_remote_pid(
        self, slot: dict[str, Any], pid: int, graceful_timeout_seconds: float
    ):
        if self._is_truthy(slot.get("use_remote_proxy", False)):
            req_addr = self._remote_proxy_req_addr(slot)
            ...
            reply = self._query_zmq(
                req_addr, payload, timeout_ms=(timeout_seconds + 3) * 1000
            )
            if isinstance(reply, dict) and reply.get("ok", False):
                return
            self.logger.warning(
        ...
        try:
            self._run_ssh_script(
                slot, script, timeout_seconds=float(timeout_seconds + 10), check=False
            )
        except Exception as exc:
            self.logger.warning(
                "Failed to stop remote pid=%s on host=%s: %s",
                pid,
                slot.get("host"),
                exc,
            )

    def _reclaim_sidecar_when_drained(
        self, instance_type: str, target_address: str, sidecar_meta: dict[str, Any]
    ):
        ...
        while True:
            ...
            if isinstance(reply, dict) and reply.get("ok", False):
                ...
                pending_transformer_request = int(
                    data.get("transformer_request_pool", 0)
                )
                pending_transformer_waiting = int(
                    data.get("transformer_waiting_pool", 0)
                )
                pending_transformer_active = int(
                    data.get("transformer_active_rooms", 0)
                )
                pending_active = (
                    pending_input_watch
                ...
                if (
                    pending_active == 0
                    and idle_seconds >= self._sidecar_drain_idle_seconds
                ):
                    break
        ...
        if config_path.is_file():
            if config_path.name.endswith("_controller.json"):
                candidate = config_path.with_name(
                    config_path.name.replace(
                        "_controller.json", f"_{instance_type}.json"
                    )
                )
            ...
            if config_path.name.endswith("_distill_controller.json"):
                candidate = config_path.with_name(
                    config_path.name.replace(
                        "_distill_controller.json", f"_distill_{instance_type}.json"
                    )
                )
                if candidate.is_file():
                    return str(candidate)
    ...
    def _load_warmup_duration_seconds(self, config: Mapping[str, Any]) -> float:
        ...
        if not stage_file.is_file():
            self.logger.warning(
                "workload stages config not found, skip warmup scale guard: %s",
                stage_file,
            )
            return 0.0
        ...
        try:
            ...
            self.logger.warning(
                "failed to load workload stages config %s: %s", stage_file, exc
            )
            return 0.0

        if not isinstance(loaded, list):
            self.logger.warning(
                "invalid workload stages config format (expect list): %s", stage_file
            )
            return 0.0
    ...
    def _sample_rdma_queue_pending(self) -> dict[str, int]:
        ...
        for service_type, rdma_buffer in buffer_by_service.items():
            ...
            try:
                ...
                self.logger.warning(
                    "Failed to sample RDMA pending count for %s: %s", service_type, exc
                )
    ...
    def _monitor_callback(self, results):
        ...
        autoscale_start_mono = float(
            monitor_runtime.get("autoscale_start_mono", time.monotonic())
        )
        ...
        scale_out_max_queue_threshold = int(
            monitor_runtime.get("scale_out_max_queue_threshold", 2)
        )
        scale_in_threshold = float(monitor_runtime.get("scale_in_threshold", 20.0))
        scale_cooldown_seconds = float(
            monitor_runtime.get("scale_cooldown_seconds", 30.0)
        )
        ...
        sample_ts = time.time()
        sample_ts_from_start_s = (
            sample_ts - self._controller_start_ts
        ...
        for item in results:
            ...
            try:
                queue_total_pending_int = (
                    int(queue_total_pending) if queue_total_pending is not None else -1
                )
            except (TypeError, ValueError):
            ...
            precompute_pending = self._calc_precompute_pending(
                service_type, queue_sizes
            )
        ...
        for service_type, metrics in service_metrics.items():
            if not metrics:
                continue
            avg_queue_total_pending = sum(
                int(metric.get("queue_total_pending", 0)) for metric in metrics
            ) / len(metrics)
            rdma_queue_pending = int(rdma_pending_by_service.get(service_type, 0))
            service_queue_scores[service_type] = float(rdma_queue_pending) + float(
                avg_queue_total_pending
            )

            precompute_values = [
                int(metric.get("precompute_pending", -1))
                for metric in metrics
                if int(metric.get("precompute_pending", -1)) >= 0
            ]
            if precompute_values:
                avg_precompute_pending = sum(precompute_values) / len(precompute_values)
                service_precompute_scores[service_type] = float(
                    rdma_queue_pending
                ) + float(avg_precompute_pending)
            else:
        ...
        for service_type, metrics in service_metrics.items():
            ...
            avg_gpu_utilization = sum(
                float(metric["gpu_utilization"]) for metric in metrics
            ) / len(metrics)
            avg_queue_total_pending = sum(
                int(metric.get("queue_total_pending", 0)) for metric in metrics
            ) / len(metrics)
            max_queue_total_pending = max(
                int(metric.get("queue_total_pending", -1)) for metric in metrics
            )
            rdma_queue_pending = int(rdma_pending_by_service.get(service_type, 0))
            current_queue_score = float(service_queue_scores.get(service_type, 0.0))
            current_precompute_score = float(
                service_precompute_scores.get(service_type, 0.0)
            )
            ...
            if (
                scale_out_triggered
                and now - float(last_scale_ts.get(service_type, 0.0))
            ...
            if low_precompute_pending >= 0:
                ...
                queues_empty_for_service = (
                    bool(low_metric.get("all_queues_empty", False))
                    and int(low_metric.get("queue_total_pending", -1)) == 0
                )

            blocked_by_queue_score = (
                current_precompute_score > 0.0
                and current_precompute_score >= max_precompute_score
            )
            ...
            if scale_in_triggered:
                try:
                    target_instance_address = self._instance_address_from_monitor_node(
                        low_monitor_address
                    )
        ...
        if scale_out_candidates:
            target = max(
                ...
                key=lambda item: (
                    item["score"],
                    item["max_queue_total_pending"],
                    item["avg_gpu_utilization"],
                ),
            )
            target_service = str(target["service_type"])
            if (
                float(target["now"]) - float(last_scale_ts.get(target_service, 0.0))
                < scale_cooldown_seconds
            ):
                return
            try:
    ...
    def _dump_controller_metrics(
        self,
        received_results: list[dict[str, Any]],
        batch_request_start_ts: float | None,
    ) -> Path:
        summary = {
    ...
    def _handle_decoder_result(
        ...
        *,
        ...
        if room is None:
            self.logger.warning(
                "Ignored decoder result without data_bootstrap_room: %s", result
            )
            return
        room = int(room)
        ...
        if message_type == "stage_metrics":
            ...
            if not isinstance(request_metrics, dict):
                self.logger.warning(
                    "Ignored stage metrics update without request_metrics: %s", result
                )
                return
            merged_metrics = self._merge_request_metrics(
                self._request_metrics_by_room.get(room), request_metrics
            )
        ...
        if room not in expected_rooms:
            self.logger.warning(
                "Ignored decoder result for unexpected room=%s: %s", room, result
            )
            return
        ...
        if isinstance(request_metrics, dict):
            merged_metrics = self._merge_request_metrics(
                stored_metrics, request_metrics
            )
            self._request_metrics_by_room[room] = merged_metrics
        ...
        if latency_summary is not None:
            ...
            self.logger.info(
                "Latency summary room=%s metrics=%s", room, latency_summary
            )
    ...
    def _init_gpu_pool(self, config: dict):
        disagg_cfg = (
            config.get("disagg_config")
            if isinstance(config.get("disagg_config"), dict)
            else {}
        )
        ...
        if isinstance(static_slots_raw, list) and static_slots_raw:
            default_workdir = str(
                disagg_cfg.get("remote_workdir", Path(__file__).resolve().parents[3])
            )
            default_python = str(
                disagg_cfg.get("remote_python_executable", sys.executable)
            )
            default_log_dir = str(
                disagg_cfg.get("remote_log_dir", "/tmp/lightx2v_disagg")
            )
            default_activate_cmd = str(
                disagg_cfg.get("remote_activate_cmd", "")
            ).strip()
            default_ssh_user = str(disagg_cfg.get("ssh_user", "")).strip()
            default_ssh_bin = str(
                disagg_cfg.get("ssh_bin", os.getenv("DISAGG_SSH_BIN", "ssh"))
            )
            default_use_remote_proxy = self._is_truthy(
                disagg_cfg.get("use_remote_proxy"),
                default=self._is_truthy(os.getenv("DISAGG_USE_REMOTE_PROXY"), False),
            )
            default_proxy_req_base_port = int(
                disagg_cfg.get("remote_proxy_req_base_port", 28000)
            )
            ...
            if isinstance(default_ssh_options_raw, str):
                ...
                default_ssh_options = [
                    str(opt) for opt in default_ssh_options_raw if str(opt).strip()
                ]
            else:
                default_ssh_options = []
            ...
            for index, raw_slot in enumerate(static_slots_raw):
                if not isinstance(raw_slot, dict):
                    raise ValueError(
                        f"invalid static_instance_slots[{index}] (expect object)"
                    )

                instance_type = str(raw_slot.get("instance_type", "")).strip().lower()
                if instance_type not in {"encoder", "transformer", "decoder"}:
                    raise ValueError(
                        f"invalid static_instance_slots[{index}].instance_type={instance_type!r}"
                    )

                host = str(raw_slot.get("host", "")).strip()
                if not host:
                    raise ValueError(
                        f"static_instance_slots[{index}].host cannot be empty"
                    )
                ...
                if isinstance(ssh_options_raw, str):
                    ...
                    ssh_options = [
                        str(opt) for opt in ssh_options_raw if str(opt).strip()
                    ]
                else:
                ...
                push_port = int(
                    raw_slot.get(
                        "sidecar_push_port", sidecar_base_port + engine_rank * 2
                    )
                )
                req_port = int(
                    raw_slot.get(
                        "sidecar_req_port", sidecar_base_port + engine_rank * 2 + 1
                    )
                )
                use_remote_proxy = self._is_truthy(
                    raw_slot.get("use_remote_proxy"), default=default_use_remote_proxy
                )
                proxy_req_port = int(
                    raw_slot.get(
                        "proxy_req_port", default_proxy_req_base_port + engine_rank
                    )
                )
                proxy_log_path = str(
                    raw_slot.get(
                        "proxy_log_path",
                        f"{default_log_dir}/instance_proxy_{engine_rank}.log",
                    )
                )
                ...
                self._static_instance_slots.append(
                    {
                        ...
                        "python_executable": str(
                            raw_slot.get("python_executable", default_python)
                        ),
                        "log_dir": str(raw_slot.get("log_dir", default_log_dir)),
                        "activate_cmd": str(
                            raw_slot.get("activate_cmd", default_activate_cmd)
                        ).strip(),
            ...
            self._free_slot_ids = {
                int(slot["slot_id"]) for slot in self._static_instance_slots
            }
            self.logger.info(
                "Static multi-node mode enabled with %s slots",
                len(self._static_instance_slots),
            )
            self._free_gpus = set()
            return
        ...
        with self._instance_lock:
            ...
            if use_static_slots:
                ...
                if selected_slot is None:
                    raise RuntimeError(
                        f"no idle static slot available for {instance_type}: all candidates cooling down or port is in use"
                    )
                ...
                if engine_rank is None:
                    raise RuntimeError(
                        f"no idle GPU available for {instance_type}: all candidates cooling down or port is in use"
                    )
            ...
            if not model_path or not config_json:
                raise RuntimeError(
                    "model_path and config_json are required to launch service subprocess"
                )
            service_config_json = self._resolve_service_config_json(
                str(config_json), instance_type
            )

            cmd = self._build_service_command(
                instance_type, engine_rank, instance_cfg, service_config_json
            )
            ...
            try:
                if (
                    ...
                    launch_mode = "remote"
                    process_meta, sidecar_meta = self._launch_remote_instance(
                        selected_slot, instance_type, cmd, cuda_device
                    )
                else:
                    sidecar_meta = self._start_sidecar_process(
                        instance_type, cuda_device, bind_host=host
                    )
                    env = os.environ.copy()
                ...
                monitor_port = MONITOR_POLLING_PORT + engine_rank
                if not self._wait_for_tcp_port_state(
                    host,
                    monitor_port,
                    should_be_open=True,
                    timeout_seconds=self._instance_start_timeout_seconds,
                ):
                    raise RuntimeError(
                        f"service {instance_type} rank={engine_rank} host={host} failed to expose monitor port {monitor_port}"
                    )
            except Exception:
                ...
                if (
                    launch_mode == "remote"
                    and selected_slot is not None
                    and process_meta is not None
                ):
                    remote_pid = process_meta.get("pid")
                    if isinstance(remote_pid, int) and remote_pid > 0:
                        self._stop_remote_pid(
                            selected_slot,
                            remote_pid,
                            self._graceful_reclaim_timeout_seconds,
                        )
                    ...
                    if launch_mode == "remote" and selected_slot is not None:
                        ...
                        if isinstance(sidecar_pid, int) and sidecar_pid > 0:
                            self._stop_remote_pid(
                                selected_slot,
                                sidecar_pid,
                                self._force_kill_wait_seconds,
                            )
                    else:
                        sidecar_process = sidecar_meta.get("process")
                        if (
                            sidecar_process is not None
                            and sidecar_process.poll() is None
                        ):
            ...
            self._managed_instances[instance_address] = {
                ...
                "slot_id": (
                    int(selected_slot["slot_id"]) if selected_slot is not None else None
                ),
                "static_slot": (
                    self._to_plain(selected_slot) if selected_slot is not None else None
                ),
            }
    ...
    def reclaim_instance(
        ...
        with self._instance_lock:
            ...
            if target_address is None:
                ...
                if not candidates:
                    raise RuntimeError(
                        f"no managed {instance_type} instance to reclaim"
                    )
            ...
            if meta.get("instance_type") != instance_type:
                raise RuntimeError(
                    f"instance type mismatch for {target_address}: expected={instance_type} got={meta.get('instance_type')}"
                )

            process = meta.get("process")
            process_meta = (
                meta.get("process_meta")
                if isinstance(meta.get("process_meta"), dict)
                else None
            )
            gpu_id = int(meta.get("gpu_id"))
            sidecar_meta = (
                meta.get("sidecar") if isinstance(meta.get("sidecar"), dict) else None
            )
            host = str(meta.get("host", self._bootstrap_addr))
            launch_mode = str(meta.get("launch_mode", "local"))
            static_slot = (
                meta.get("static_slot")
                if isinstance(meta.get("static_slot"), dict)
                else None
            )
            ...
            if launch_mode == "remote":
                if static_slot is None:
                    raise RuntimeError(
                        f"remote instance metadata missing static slot for {target_address}"
                    )

                remote_service_pid = None
                if process_meta is not None and isinstance(
                    process_meta.get("pid"), int
                ):
                ...
                if sidecar_meta is not None and isinstance(
                    sidecar_meta.get("pid"), int
                ):
                    self._stop_remote_pid(
                        static_slot,
                        int(sidecar_meta["pid"]),
                        self._force_kill_wait_seconds,
                    )
            else:
                if process is not None and process.poll() is None:
            ...
            if not self._wait_for_tcp_port_state(
                host, monitor_port, should_be_open=False, timeout_seconds=5.0
            ):
                self.logger.warning(
            ...
            if slot_id is not None and slot_id in range(
                len(self._static_instance_slots)
            ):
                self._free_slot_ids.add(slot_id)
                self._slot_reuse_block_until[slot_id] = (
                    time.time() + self._gpu_reuse_grace_seconds
                )
            else:
                self._free_gpus.add(gpu_id)
                self._gpu_reuse_block_until[gpu_id] = (
                    time.time() + self._gpu_reuse_grace_seconds
                )
    ...
    def _init_request_rdma_buffer(self, bootstrap_addr: str, config: dict):
        ...
        if not self._is_centralized_enabled():
            ...
            self.logger.info(
                "IS_CENTRALIZED enabled, skip controller request RDMA ring initialization"
            )
    ...
    def serve_rdma_dispatch_only(self, config: dict) -> None:
        ...
        bootstrap_addr = config.get(
            "data_bootstrap_addr", dc.get("bootstrap_addr", "127.0.0.1")
        )
        self._init_request_rdma_buffer(bootstrap_addr, config)
        self.logger.info(
            "RDMA dispatch rings ready on %s (Ctrl+C to exit).", bootstrap_addr
        )
        try:
            while True:
                time.sleep(3600.0)
            ...
            self.logger.info(
                "Controller serve_rdma_dispatch_only interrupted, exiting."
            )

    def _build_latency_summary(
        self, result: dict[str, Any], controller_recv_ts: float
    ) -> dict[str, float] | None:
        request_metrics = result.get("request_metrics")
        if not isinstance(request_metrics, dict):
            return None
        ...
        transformer_output_enqueued_ts = _as_float(
            transformer.get("output_enqueued_ts")
        )
        ...
        if centralized_mode:
            if encoder_recv_ts is not None:
                summary["controller_to_encoder_comm_delay_s"] = (
                    encoder_recv_ts - controller_send_ts
                )
            if encoder_recv_ts is not None and encoder_compute_start_ts is not None:
                summary["encoder_scheduling_delay_s"] = (
                    encoder_compute_start_ts - encoder_recv_ts
                )
            if (
                encoder_compute_start_ts is not None
                and encoder_compute_end_ts is not None
            ):
                summary["encoder_compute_delay_s"] = (
                    encoder_compute_end_ts - encoder_compute_start_ts
                )
            if (
                encoder_output_enqueued_ts is not None
                and transformer_recv_ts is not None
            ):
                summary["encoder_communication_delay_s"] = (
                    transformer_recv_ts - controller_send_ts
                )
            if (
                transformer_recv_ts is not None
                and transformer_compute_start_ts is not None
            ):
                summary["transformer_scheduling_delay_s"] = (
                    transformer_compute_start_ts - transformer_recv_ts
                )
            if (
                transformer_compute_start_ts is not None
                and transformer_compute_end_ts is not None
            ):
                summary["transformer_compute_delay_s"] = (
                    transformer_compute_end_ts - transformer_compute_start_ts
                )
            if transformer_recv_ts is not None:
                summary["transformer_communication_delay_s"] = (
                    transformer_recv_ts - controller_send_ts
                )
            if decoder_recv_ts is not None and decoder_compute_start_ts is not None:
                summary["decoder_scheduling_delay_s"] = (
                    decoder_compute_start_ts - decoder_recv_ts
                )
            if (
                decoder_compute_start_ts is not None
                and decoder_compute_end_ts is not None
            ):
                summary["decoder_compute_delay_s"] = (
                    decoder_compute_end_ts - decoder_compute_start_ts
                )
            if decoder_recv_ts is not None:
                summary["decoder_communication_delay_s"] = (
                    decoder_recv_ts - controller_send_ts
                )
            ...
            if all(key in summary for key in component_keys):
                summary["sum_of_components_s"] = sum(
                    summary[key] for key in component_keys
                )
        else:
            if encoder_recv_ts is not None:
                summary["controller_to_encoder_comm_delay_s"] = (
            ...
            if (
                ...
                summary["encoder_communication_delay_s"] = (
                    transformer_recv_ts - encoder_output_enqueued_ts
                )
            if (
                transformer_recv_ts is not None
                and transformer_compute_start_ts is not None
            ...
            if (
                transformer_output_enqueued_ts is not None
                and decoder_recv_ts is not None
            ):
                summary["transformer_communication_delay_s"] = (
                    decoder_recv_ts - transformer_output_enqueued_ts
                )
            if decoder_recv_ts is not None and decoder_compute_start_ts is not None:
                summary["decoder_scheduling_delay_s"] = (
                    decoder_compute_start_ts - decoder_recv_ts
            ...
            if decoder_output_enqueued_ts is not None:
                summary["decoder_communication_delay_s"] = (
                    controller_recv_ts - decoder_output_enqueued_ts
                )
    ...
    def send_request(self, config):
        ...
        if room is not None and isinstance(request_metrics, dict):
            self._request_metrics_by_room[room] = self._merge_request_metrics(
                None, request_metrics
            )
        ...
        if self._is_centralized_enabled():
            ...
            request_config["controller_control_host"] = request_config.get(
                "controller_result_host", self._bootstrap_addr
            )
            request_config["controller_control_port"] = int(
                request_config.get("controller_control_port", REQUEST_POLLING_PORT - 3)
            )
            ...
            for instance_type, target_address in (
                ...
                self.logger.info(
                    "Request dispatched to %s via ZMQ: target=%s",
                    instance_type,
                    target_address,
                )
            return
    ...
    def run(self, config):
        ...
        request_ingress_port = int(
            config.get(
                "controller_request_port",
                os.getenv("DISAGG_CONTROLLER_REQUEST_PORT", REQUEST_POLLING_PORT - 2),
            )
        )
        result_port = int(
            config.get("controller_result_port", REQUEST_POLLING_PORT - 1)
        )
        control_port = int(
            config.get("controller_control_port", REQUEST_POLLING_PORT - 3)
        )
        ...
        if centralized_mode:
            self.logger.info(
                "IS_CENTRALIZED enabled, controller will dispatch requests via ZMQ"
            )
        ...
        if centralized_mode:
            ...
            self.logger.info(
                "Centralized OK gate server started on %s:%s",
        ...
        load_from_user = str(os.getenv("LOAD_FROM_USER", "0")).strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
        }
        auto_request_count_raw = config.get(
            "request_count", os.getenv("DISAGG_AUTO_REQUEST_COUNT", "30")
        )
        try:
        ...
        try:
            ...
            if load_from_user:
                self.logger.info(
                    "LOAD_FROM_USER enabled, waiting workload configs on port=%s",
                    request_ingress_port,
                )
            else:
                self.logger.info(
            ...
            while True:
                if load_from_user:
                    ...
                    if not isinstance(workload_config, dict):
                        self.logger.warning(
                            "Ignored invalid workload config packet: %s",
                            workload_config,
                        )
                        continue

                    if (
                        workload_config.get("workload_end")
                        or workload_config.get("end")
                        or workload_config.get("stop")
                    ):
                        self.logger.info(
                            "Received workload end signal, stop accepting new configs."
                        )
                        break
                else:
                ...
                if base_save_path and not request_config.get("save_path"):
                    ...
                    request_config["save_path"] = str(
                        save_path.with_name(f"{save_path.stem}{room}{save_path.suffix}")
                    )
            ...
            for instance_type, address in reversed(list(self.started_instances)):
                try:
                    ...
                    self.logger.exception(
                        "Failed to reclaim %s instance address=%s",
                        instance_type,
                        address,
                    )
            ...
            try:
                ...
                self.logger.exception(
                    "Failed to write controller metrics to %s",
                    self._metrics_output_json,
                )
