...
class EncoderService(BaseService):
    def __init__(self, config: dict):
        ...
        self._request_server_ip = str(
            self.config.get("rdma_request_host", data_bootstrap_addr)
        )
        self._request_handshake_port = int(
            self.config.get("rdma_request_handshake_port", "5566")
        )
    ...
    def _ensure_request_buffer(self) -> bool:
        ...
        if self._request_rdma_client is None:
            self._request_rdma_client = RDMAClient(
                local_buffer_size=self._request_slot_size
            )
    ...
    def _produce_phase1_request_with_retry(self, room: int, payload: dict[str, Any]):
        ...
        retry_delay_s = max(
            0.01, float(os.getenv("RDMA_PHASE1_PRODUCE_RETRY_DELAY_S", "0.2"))
        )
        ...
        raise RuntimeError(
            f"Failed to produce phase1 RDMA request for room={room} after {retries} attempts"
        ) from last_exc
    ...
    def init(self, config):
        ...
        shared_slot_size = int(self.config.get("rdma_buffer_slot_size", 4096))
        self._request_server_ip = str(
            self.config.get("rdma_request_host", self._request_server_ip)
        )
        self._request_handshake_port = int(
            self.config.get("rdma_request_handshake_port", self._request_handshake_port)
        )
        self._request_slots = shared_slots
        self._request_slot_size = shared_slot_size
        self._phase1_server_ip = str(
        ...
        while self._phase1_rdma_buffer is None and time.time() < phase1_deadline:
            try:
                ...
                self.logger.exception(
                    "Failed to connect phase1 RDMA buffer, will retry"
                )
            if self._phase1_rdma_buffer is None:
                time.sleep(0.1)
        ...
        self.data_sender[data_bootstrap_room] = DataSender(
            self.data_mgr, data_bootstrap_addr, data_bootstrap_room
        )
    ...
    def _get_vae_encoder_output(
        self, first_frame: torch.Tensor, latent_h: int, latent_w: int
    ):
    ...
    def process(self, config):
        ...
        if config.get("enable_cfg", False):
            if negative_prompt is None:
                raise ValueError(
                    "negative_prompt is required in config when enable_cfg is True."
                )
        ...
        if task == "t2v":
            ...
            if self.image_encoder is not None:
                ...
                clip_encoder_out = (
                    self.image_encoder.visual([img]).squeeze(0).to(GET_DTYPE())
                )

            if self.vae_encoder is None:
                raise RuntimeError("VAE encoder is required but was not loaded.")

            latent_shape, latent_h, latent_w = self._compute_latent_shape_from_image(
                img
            )
        ...
        if self.data_mgr is not None and sender is not None:
            ...
            if config.get("enable_cfg", False):
                ...
                context_null_view = _buffer_view(
                    context_null_buf, GET_DTYPE(), tuple(context_null.shape)
                )
                ...
                if config.get("use_image_encoder", True):
                    ...
                    if image_encoder_output.get("clip_encoder_out") is not None:
                        clip_view = _buffer_view(
                            clip_buf,
                            GET_DTYPE(),
                            tuple(image_encoder_output["clip_encoder_out"].shape),
                        )
            ...
            meta = {
                ...
                "context_null_dtype": (
                    str(context_null.dtype) if context_null is not None else None
                ),
                "context_null_hash": _sha256_tensor(context_null),
                "clip_shape": (
                    list(clip_encoder_out.shape)
                    if clip_encoder_out is not None
                    else None
                ),
                "clip_dtype": (
                    str(clip_encoder_out.dtype)
                    if clip_encoder_out is not None
                    else None
                ),
                "clip_hash": _sha256_tensor(clip_encoder_out),
                "vae_shape": (
                    list(image_encoder_output["vae_encoder_out"].shape)
                    if image_encoder_output is not None
                    else None
                ),
                "vae_dtype": (
                    str(image_encoder_output["vae_encoder_out"].dtype)
                    if image_encoder_output is not None
                    else None
                ),
                "vae_hash": (
                    _sha256_tensor(image_encoder_output["vae_encoder_out"])
                    if image_encoder_output is not None
                    else None
                ),
            ...
            meta_buf = _buffer_view(
                room_buffers[buffer_index],
                torch.uint8,
                (room_buffers[buffer_index].numel(),),
            )
    ...
    def run(self, stop_event=None):
        ...
        while True:
            ...
            if self._centralized_request_mode:
                ...
                if self._request_rdma_buffer is None:
                    try:
                        ...
                        self.logger.exception(
                            "Failed to connect request RDMA buffer, will retry"
                        )

                if (
                    self._request_rdma_client is not None
                    and self._request_rdma_client.has_qp_error()
                ):
                    self.logger.warning(
                    ...
                    try:
                        ...
                        self.logger.exception(
                            "Failed to reconnect request RDMA buffer after QP error"
                        )
                ...
                if self._request_rdma_buffer is not None:
                    ...
                    if config is not None:
                        if (
                            ...
                            self.logger.warning(
                                "Ignored incomplete request packet from RDMA buffer: %s",
                                config,
                            )
                            continue
                        encoder_metrics = (
            ...
            if (
                ...
                self.logger.info(
                    "EncoderService received stop event, exiting request loop."
                )
                break
