...
class BaseGenerationService(ABC):
    ...
    async def _resolve_image_path(self, image_path: str) -> str:
        ...
        if image_path.startswith("http"):
            ...
            saved_path = save_base64_image(
                image_path, str(self.file_service.input_image_dir)
            )
    ...
    async def _process_image_path(
        self, image_path: str, task_data: Dict[str, Any]
    ) -> None:
        task_data["image_path"] = await self._resolve_image_path(image_path)

    async def _process_image_mask_path(
        self, image_mask_path: str, task_data: Dict[str, Any]
    ) -> None:
    ...
    def _pack_image_and_mask_as_dir(self, task_data: Dict[str, Any]) -> None:
        ...
        if not mask_file.exists() or not mask_file.is_file():
            raise RuntimeError(
                f"Invalid image_mask_path for mask mode: {image_mask_path}"
            )

        image_pair_dir = (
            self.file_service.input_image_dir / f"mask_pair_{uuid.uuid4().hex[:8]}"
        )
    ...
    async def _process_audio_path(
        self, audio_path: str, task_data: Dict[str, Any]
    ) -> None:
        if not audio_path:
            return
        ...
        if audio_path.startswith("http"):
            ...
            saved_path = save_base64_audio(
                audio_path, str(self.file_service.input_audio_dir)
            )
            task_data["audio_path"] = str(saved_path)
        else:
            task_data["audio_path"] = audio_path

    async def _process_talk_objects(
        self, talk_objects: list, task_data: Dict[str, Any]
    ) -> None:
        if not talk_objects:
            return
        ...
        for index, talk_object in enumerate(talk_objects):
            if talk_object.audio.startswith("http"):
                ...
                audio_path = save_base64_audio(
                    talk_object.audio, str(self.file_service.input_audio_dir)
                )
                task_data["talk_objects"][index]["audio"] = str(audio_path)
            else:
            ...
            if talk_object.mask.startswith("http"):
                ...
                mask_path = save_base64_image(
                    talk_object.mask, str(self.file_service.input_image_dir)
                )
                task_data["talk_objects"][index]["mask"] = str(mask_path)
            else:
    ...
    def _prepare_output_path(
        self, save_result_path: str, task_data: Dict[str, Any]
    ) -> None:
    ...
    async def generate_with_stop_event(self, message: Any, stop_event) -> Optional[Any]:
        try:
            ...
            if hasattr(message, "audio_path") and message.audio_path:
                ...
                logger.info(
                    f"Task {message.task_id} audio path: {task_data.get('audio_path')}"
                )
