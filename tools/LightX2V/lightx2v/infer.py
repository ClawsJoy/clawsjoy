...
from lightx2v.models.runners.hunyuan_video.hunyuan_video_15_distill_runner import (  # noqa: F401
...
from lightx2v.models.runners.wan.wan_vace_runner import (  # noqa: F401
    Wan22MoeVaceRunner,
    WanVaceRunner,
)
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[],
        help="Set supported tasks for the model",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="The path to input robot state file for Motus i2v inference.",
    )
    parser.add_argument(
        "--last_frame_path",
        type=str,
        default="",
        help="The path to last frame file for first-last-frame-to-video (flf2v) task",
    )
    parser.add_argument(
        "--audio_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="(worldmirror/recon) Path to a directory of images, a video file, or a single image.",
    )
    parser.add_argument(
        "--strict_output_path",
        type=str,
        default=None,
        help="(worldmirror/recon) If set, write outputs directly here instead of under save_result_path/<subdir>/<timestamp>/.",
    )
    parser.add_argument(
        "--prior_cam_path",
        type=str,
        default=None,
        help="(worldmirror/recon) Optional camera prior JSON (extrinsics + intrinsics).",
    )
    parser.add_argument(
        "--prior_depth_path",
        type=str,
        default=None,
        help="(worldmirror/recon) Optional depth prior directory (one .npy/.png per image).",
    )
    parser.add_argument(
        "--subfolder",
        type=str,
        default=None,
        help="(worldmirror/recon) Subfolder inside model_path containing weights. Overrides config.",
    )
    parser.add_argument(
        "--disable_heads",
        type=str,
        nargs="*",
        default=None,
        help="(worldmirror/recon) Heads to disable: any of camera depth normal points gs.",
    )
    parser.add_argument(
        "--enable_bf16",
        action="store_true",
        default=False,
        help="(worldmirror/recon) Run the WorldMirror model in bf16.",
    )
    parser.add_argument(
        "--save_rendered",
        action="store_true",
        default=False,
        help="(worldmirror/recon) Render an interpolated fly-through video from Gaussian splats.",
    )
    parser.add_argument(
        "--render_interp_per_pair",
        type=int,
        default=None,
        help="(worldmirror/recon) Interpolated frames per camera pair for --save_rendered.",
    )
    parser.add_argument(
        "--render_depth",
        action="store_true",
        default=False,
        help="(worldmirror/recon) Also render a depth video with --save_rendered.",
    )
    parser.add_argument(
        "--wm_config_path",
        type=str,
        default=None,
        help="(worldmirror/recon) Optional training YAML (pair with --wm_ckpt_path).",
    )
    parser.add_argument(
        "--wm_ckpt_path",
        type=str,
        default=None,
        help="(worldmirror/recon) Optional .ckpt/.safetensors (pair with --wm_config_path).",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The path to save action predictions for Motus.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[],
        help="Set return video or image shape",
    )
    parser.add_argument(
        "--target_video_length",
        type=int,
    ...
    parser.add_argument(
        "--sr_ratio", type=float, default=2.0, help="super resolution ratio for sr task"
    )
    parser.add_argument(
        "--num_iterations",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="(v2av) IC-LoRA reference-video conditioning strength in [0.0, 1.0]. 1.0 = full adherence to the control signal, 0.0 = ignore it.",
    )
    parser.add_argument(
        "--reference_video_frame_cap",
        type=int,
        default=None,
        help="(v2av) Maximum number of frames to read from the reference/control video. Defaults to the full clip.",
    )
    parser.add_argument(
        "--mux_audio_video_path",
        type=str,
        default=None,
        help="(v2av, optional) After saving, mux audio from this file into the output mp4 (ffmpeg). ",
    )
