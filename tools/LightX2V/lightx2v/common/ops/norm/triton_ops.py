...
def _fused_norm_3drope_kernel(
    ...
    var_hw = (
        tl.sum(xh1 * xh1) + tl.sum(xh2 * xh2) + tl.sum(xw1 * xw1) + tl.sum(xw2 * xw2)
    ) * (1.0 / HALF) + eps
...
def _fused_qk_norm_3drope_kernel(
    ...
    if pid < q_total:
        ...
        tl.store(
            q_ptr + base + HALF + QUARTER + EIGHTH + offs_e, new_xw2.to(tl.bfloat16)
        )
    else:
        k_pid = pid - q_total
        s = k_pid // k_num_heads
        ...
        tl.store(
            k_ptr + base + HALF + QUARTER + EIGHTH + offs_e, new_xw2.to(tl.bfloat16)
        )
