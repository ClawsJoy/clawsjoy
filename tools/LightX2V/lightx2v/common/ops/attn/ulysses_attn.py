...
class UlyssesAttnWeight(AttnWeightTemplate):
        ...
        **kwargs,
        ...
        if img_first:
            ...
            if len(cu_seqlens_qkv) == 3:
                txt_qkv_len = (
                    cu_seqlens_qkv[1] - slice_qkv_len
                )  # 文本查询、键和值的长度
        ...
        if txt_mask_len:
            cu_seqlens_kv = torch.cat(
                (
                    cu_seqlens_kv,
                    torch.tensor([txt_mask_len + global_img_seqlen], dtype=torch.int32),
                )
            )
        ...
        if use_qkv_fusion:
            ...
            img_qkv = torch.stack([img_q, img_k, img_v], dim=0).reshape(
                3, img_qkv_len, world_size, shard_heads, hidden_dims
            )
        ...
        if enable_head_parallel:
            assert (
                not is_gqa
            ), "GQA（q_heads != kv_heads）暂不支持 enable_head_parallel 模式"
            # head_parallel 路径下 q_shard_heads == kv_shard_heads
            if use_qkv_fusion:
                img_qkv = img_qkv.permute(
                    3, 2, 1, 0, 4
                ).contiguous()  # (shard_heads, world_size, img_qkv_len, 3, hidden_dims)
                output_qkv = torch.empty_like(img_qkv)
                if use_fp8_comm or use_fp4_comm:
                    if use_fp8_comm:
                        img_qkv_quant, img_qkv_scale = quant_fp8_vllm(
                            img_qkv.reshape(-1, hidden_dims)
                        )
                        img_qkv_quant = img_qkv_quant.reshape(
                            shard_heads, world_size, img_qkv_len, 3, hidden_dims
                        )
                        img_qkv_scale = img_qkv_scale.reshape(
                            shard_heads, world_size, img_qkv_len, 3, 1
                        )
                    else:
                        img_qkv_quant, img_qkv_scale = quant_fp4_sage3(
                            img_qkv.reshape(1, 1, -1, hidden_dims)
                        )
                        img_qkv_quant = img_qkv_quant.reshape(
                            shard_heads, world_size, img_qkv_len, 3, hidden_dims // 2
                        )
                        img_qkv_scale = img_qkv_scale.reshape(
                            shard_heads, world_size, img_qkv_len, 3, hidden_dims // 16
                        )
                    ...
                    for h in range(shard_heads):
                        work_quant = dist.all_to_all_single(
                            output_qkv_quant[h],
                            img_qkv_quant[h],
                            group=seq_p_group,
                            async_op=True,
                        )
                        work_scale = dist.all_to_all_single(
                            output_qkv_scale[h],
                            img_qkv_scale[h],
                            group=seq_p_group,
                            async_op=True,
                    ...
                    for h in range(shard_heads):
                        work = dist.all_to_all_single(
                            output_qkv[h], img_qkv[h], group=seq_p_group, async_op=True
                        )
                        comm_works.append(work)
            else:
                img_q = img_q.permute(
                    2, 1, 0, 3
                ).contiguous()  # (shard_heads, world_size, img_qkv_len, hidden_dims)
                ...
                if use_fp8_comm or use_fp4_comm:
                    if use_fp8_comm:
                        img_q_quant, img_q_scale = quant_fp8_vllm(
                            img_q.reshape(-1, hidden_dims)
                        )
                        img_k_quant, img_k_scale = quant_fp8_vllm(
                            img_k.reshape(-1, hidden_dims)
                        )
                        img_v_quant, img_v_scale = quant_fp8_vllm(
                            img_v.reshape(-1, hidden_dims)
                        )
                        img_q_quant = img_q_quant.reshape(
                            shard_heads, world_size, img_qkv_len, hidden_dims
                        )
                        img_k_quant = img_k_quant.reshape(
                            shard_heads, world_size, img_qkv_len, hidden_dims
                        )
                        img_v_quant = img_v_quant.reshape(
                            shard_heads, world_size, img_qkv_len, hidden_dims
                        )
                        img_q_scale = img_q_scale.reshape(
                            shard_heads, world_size, img_qkv_len, 1
                        )
                        img_k_scale = img_k_scale.reshape(
                            shard_heads, world_size, img_qkv_len, 1
                        )
                        img_v_scale = img_v_scale.reshape(
                            shard_heads, world_size, img_qkv_len, 1
                        )
                    else:
                        img_q_quant, img_q_scale = quant_fp4_sage3(
                            img_q.reshape(1, 1, -1, hidden_dims)
                        )
                        img_k_quant, img_k_scale = quant_fp4_sage3(
                            img_k.reshape(1, 1, -1, hidden_dims)
                        )
                        img_v_quant, img_v_scale = quant_fp4_sage3(
                            img_v.reshape(1, 1, -1, hidden_dims)
                        )
                        img_q_quant = img_q_quant.reshape(
                            shard_heads, world_size, img_qkv_len, hidden_dims // 2
                        )
                        img_k_quant = img_k_quant.reshape(
                            shard_heads, world_size, img_qkv_len, hidden_dims // 2
                        )
                        ...
                        img_q_scale = img_q_scale.reshape(
                            shard_heads, world_size, img_qkv_len, hidden_dims // 16
                        )
                        img_k_scale = img_k_scale.reshape(
                            shard_heads, world_size, img_qkv_len, hidden_dims // 16
                        )
                    ...
                    comm_scale_works = []
                    for h in range(shard_heads):
                        work_q_quant = dist.all_to_all_single(
                            output_q_quant[h],
                            img_q_quant[h],
                            group=seq_p_group,
                            async_op=True,
                        )
                        work_k_quant = dist.all_to_all_single(
                            output_k_quant[h],
                            img_k_quant[h],
                            group=seq_p_group,
                            async_op=True,
                        )
                        work_v_quant = dist.all_to_all_single(
                            output_v_quant[h],
                            img_v_quant[h],
                            group=seq_p_group,
                            async_op=True,
                        )
                        work_q_scale = dist.all_to_all_single(
                            output_q_scale[h],
                            img_q_scale[h],
                            group=seq_p_group,
                            async_op=True,
                        )
                        work_k_scale = dist.all_to_all_single(
                            output_k_scale[h],
                            img_k_scale[h],
                            group=seq_p_group,
                            async_op=True,
                        )
                        work_v_scale = dist.all_to_all_single(
                            output_v_scale[h],
                            img_v_scale[h],
                            group=seq_p_group,
                            async_op=True,
                    ...
                    comm_works = []
                    for h in range(shard_heads):
                        work_q = dist.all_to_all_single(
                            output_q[h], img_q[h], group=seq_p_group, async_op=True
                        )
                        work_k = dist.all_to_all_single(
                            output_k[h], img_k[h], group=seq_p_group, async_op=True
                        )
                        work_v = dist.all_to_all_single(
                            output_v[h], img_v[h], group=seq_p_group, async_op=True
                        )
            ...
            for h in range(shard_heads):
                if use_qkv_fusion:
                    if use_fp8_comm or use_fp4_comm:
                        ...
                        if use_fp8_comm:
                            output_qkv[h] = dequant_fp8_vllm(
                                output_qkv_quant[h], output_qkv_scale[h], original_dtype
                            )
                        else:
                            output_qkv[h] = dequant_fp4_sage3(
                                output_qkv_quant[h].reshape(1, 1, -1, hidden_dims // 2),
                                output_qkv_scale[h].reshape(
                                    1, 1, -1, hidden_dims // 16
                                ),
                            ).reshape(world_size, img_qkv_len, 3, hidden_dims)
                    else:
                        comm_works[h].wait()

                    qkv = (
                        output_qkv[h]
                        .reshape(global_img_seqlen, 3, single_head, hidden_dims)
                        .transpose(0, 1)
                    )
                    shard_img_q = qkv[
                        0
                    ]  # (global_img_seqlen, single_head, hidden_dims)
                    shard_img_k = qkv[1]
                    shard_img_v = qkv[2]
                else:
                    if use_fp8_comm or use_fp4_comm:
                        ...
                        if use_fp8_comm:
                            output_q[h] = dequant_fp8_vllm(
                                output_q_quant[h], output_q_scale[h], original_dtype
                            )
                            output_k[h] = dequant_fp8_vllm(
                                output_k_quant[h], output_k_scale[h], original_dtype
                            )
                            output_v[h] = dequant_fp8_vllm(
                                output_v_quant[h], output_v_scale[h], original_dtype
                            )
                        else:
                            output_q[h] = dequant_fp4_sage3(
                                output_q_quant[h].reshape(1, 1, -1, hidden_dims // 2),
                                output_q_scale[h].reshape(1, 1, -1, hidden_dims // 16),
                            ).reshape(world_size, img_qkv_len, hidden_dims)
                            output_k[h] = dequant_fp4_sage3(
                                output_k_quant[h].reshape(1, 1, -1, hidden_dims // 2),
                                output_k_scale[h].reshape(1, 1, -1, hidden_dims // 16),
                            ).reshape(world_size, img_qkv_len, hidden_dims)
                            output_v[h] = dequant_fp4_sage3(
                                output_v_quant[h].reshape(1, 1, -1, hidden_dims // 2),
                                output_v_scale[h].reshape(1, 1, -1, hidden_dims // 16),
                            ).reshape(world_size, img_qkv_len, hidden_dims)
                    else:
                        comm_works[3 * h].wait()
                    ...
                    shard_img_q = output_q[h].reshape(
                        global_img_seqlen, single_head, hidden_dims
                    )
                    shard_img_k = output_k[h].reshape(
                        global_img_seqlen, single_head, hidden_dims
                    )
                    shard_img_v = output_v[h].reshape(
                        global_img_seqlen, single_head, hidden_dims
                    )
                ...
                if q_only_img:
                    ...
                    shard_txt_k = txt_k[
                        :,
                        (cur_rank * shard_heads + h) : (cur_rank * shard_heads + h + 1),
                        :,
                    ]
                    shard_txt_v = txt_v[
                        :,
            ...
            if use_qkv_fusion:
                img_qkv = img_qkv.permute(
                    2, 1, 0, 3, 4
                ).contiguous()  # (world_size, img_qkv_len, 3, shard_heads, hidden_dims)
            else:
                img_q = img_q.permute(
                    1, 0, 2, 3
                ).contiguous()  # (world_size, img_qkv_len, q_shard_heads, hidden_dims)
                img_k = img_k.permute(
                    1, 0, 2, 3
                ).contiguous()  # (world_size, img_qkv_len, kv_shard_heads, hidden_dims)
            ...
            if use_qkv_fusion:
                if use_fp8_comm or use_fp4_comm:
                    if use_fp8_comm:
                        ...
                        img_qkv_quant = img_qkv_quant.reshape(
                            world_size, img_qkv_len, shard_heads, 3, hidden_dims
                        )
                        img_qkv_scale = img_qkv_scale.reshape(
                            world_size, img_qkv_len, shard_heads, 3, 1
                        )
                    else:
                        img_qkv_quant, img_qkv_scale = quant_fp4_sage3(
                            img_qkv.reshape(1, 1, -1, hidden_dims)
                        ...
                        img_qkv_quant = img_qkv_quant.reshape(
                            world_size, img_qkv_len, shard_heads, 3, hidden_dims // 2
                        )
                        img_qkv_scale = img_qkv_scale.reshape(
                            world_size, img_qkv_len, shard_heads, 3, hidden_dims // 16
                        )
                    output_qkv_quant = torch.empty_like(img_qkv_quant)
                    output_qkv_scale = torch.empty_like(img_qkv_scale)
                    dist.all_to_all_single(
                        output_qkv_quant, img_qkv_quant, group=seq_p_group
                    )
                    dist.all_to_all_single(
                        output_qkv_scale, img_qkv_scale, group=seq_p_group
                    )
                    if use_fp8_comm:
                        output_qkv = dequant_fp8_vllm(
                            output_qkv_quant, output_qkv_scale, original_dtype
                        )
                    else:
                        output_qkv = dequant_fp4_sage3(
                            output_qkv_quant.reshape(1, 1, -1, hidden_dims // 2),
                            output_qkv_scale.reshape(1, 1, -1, hidden_dims // 16),
                        )
                else:
                    output_qkv = torch.empty_like(img_qkv)
                    dist.all_to_all_single(output_qkv, img_qkv, group=seq_p_group)

                qkv = output_qkv.reshape(
                    global_img_seqlen, 3, shard_heads, hidden_dims
                ).transpose(0, 1)
                ...
                if use_fp8_comm or use_fp4_comm:
                    if use_fp8_comm:
                        ...
                        img_k_quant = img_k_quant.reshape(
                            world_size, img_qkv_len, kv_shard_heads, hidden_dims
                        )
                        img_v_quant = img_v_quant.reshape(
                            world_size, img_qkv_len, kv_shard_heads, hidden_dims
                        )
                        img_q_scale = img_q_scale.reshape(
                            world_size, img_qkv_len, q_shard_heads, 1
                        )
                        img_k_scale = img_k_scale.reshape(
                            world_size, img_qkv_len, kv_shard_heads, 1
                        )
                        img_v_scale = img_v_scale.reshape(
                            world_size, img_qkv_len, kv_shard_heads, 1
                        )
                        ...
                        img_q_quant = img_q_quant.reshape(
                            world_size, img_qkv_len, q_shard_heads, hidden_dims // 2
                        )
                        img_k_quant = img_k_quant.reshape(
                            world_size, img_qkv_len, kv_shard_heads, hidden_dims // 2
                        )
                        img_v_quant = img_v_quant.reshape(
                            world_size, img_qkv_len, kv_shard_heads, hidden_dims // 2
                        )
                        img_q_scale = img_q_scale.reshape(
                            world_size, img_qkv_len, q_shard_heads, hidden_dims // 16
                        )
                        img_k_scale = img_k_scale.reshape(
                            world_size, img_qkv_len, kv_shard_heads, hidden_dims // 16
                        )
                        img_v_scale = img_v_scale.reshape(
                            world_size, img_qkv_len, kv_shard_heads, hidden_dims // 16
                        )
                    ...
                    output_v_scale = torch.empty_like(img_v_scale)
                    dist.all_to_all_single(
                        output_q_quant, img_q_quant, group=seq_p_group
                    )
                    dist.all_to_all_single(
                        output_k_quant, img_k_quant, group=seq_p_group
                    )
                    dist.all_to_all_single(
                        output_v_quant, img_v_quant, group=seq_p_group
                    )
                    dist.all_to_all_single(
                        output_q_scale, img_q_scale, group=seq_p_group
                    )
                    dist.all_to_all_single(
                        output_k_scale, img_k_scale, group=seq_p_group
                    )
                    dist.all_to_all_single(
                        output_v_scale, img_v_scale, group=seq_p_group
                    )
                    if use_fp8_comm:
                        output_q = dequant_fp8_vllm(
                            output_q_quant, output_q_scale, original_dtype
                        )
                        output_k = dequant_fp8_vllm(
                            output_k_quant, output_k_scale, original_dtype
                        )
                        output_v = dequant_fp8_vllm(
                            output_v_quant, output_v_scale, original_dtype
                        )
                    else:
                        output_q = dequant_fp4_sage3(
                            output_q_quant.reshape(1, 1, -1, hidden_dims // 2),
                            output_q_scale.reshape(1, 1, -1, hidden_dims // 16),
                        )
                        output_k = dequant_fp4_sage3(
                            output_k_quant.reshape(1, 1, -1, hidden_dims // 2),
                            output_k_scale.reshape(1, 1, -1, hidden_dims // 16),
                        )
                        output_v = dequant_fp4_sage3(
                            output_v_quant.reshape(1, 1, -1, hidden_dims // 2),
                            output_v_scale.reshape(1, 1, -1, hidden_dims // 16),
                        )
                else:
                    output_q = torch.empty_like(img_q)
                    output_k = torch.empty_like(img_k)
                ...
                shard_img_q = output_q.reshape(
                    global_img_seqlen, q_shard_heads, hidden_dims
                )
                shard_img_k = output_k.reshape(
                    global_img_seqlen, kv_shard_heads, hidden_dims
                )
                shard_img_v = output_v.reshape(
                    global_img_seqlen, kv_shard_heads, hidden_dims
                )
            ...
            if q_only_img:
                ...
                shard_txt_k = txt_k[
                    :, cur_rank * kv_shard_heads : (cur_rank + 1) * kv_shard_heads, :
                ]
                shard_txt_v = txt_v[
                    :, cur_rank * kv_shard_heads : (cur_rank + 1) * kv_shard_heads, :
                ]
                ...
                shard_txt_q = txt_q[
                    :, cur_rank * q_shard_heads : (cur_rank + 1) * q_shard_heads, :
                ]
                shard_txt_k = txt_k[
                    :, cur_rank * kv_shard_heads : (cur_rank + 1) * kv_shard_heads, :
                ]
        ...
        if q_only_img:
            ...
            img_attn = self._reshape_img_attn(
                attn,
                world_size,
                shard_seqlen,
                q_shard_heads,
    ...
    def _reshape_img_attn(
        ...
        if use_fp8_comm:
            ...
            img_attn_quant, attn_scale = quant_fp8_vllm(
                img_attn.reshape(-1, original_shape[-1])
            )
            img_attn_quant = all2all_head2seq(
                img_attn_quant.reshape(original_shape), group=seq_p_group
            )
            attn_scale = all2all_head2seq(
                attn_scale.reshape(original_shape[0], original_shape[1], 1),
                group=seq_p_group,
            )
...
class Ulysses4090AttnWeight(AttnWeightTemplate):
    ...
    def load_balanced_all_to_all(self, shards, seq_p_group=None):
        ...
        for i, round_schedule in enumerate(self.rounds):
            ...
            if is_smaller_in_pair:
                # 当前进程是配对中的较小者，先发送后接收
                send_req = dist.isend(
                    shards[partner], dst=partner_global_rank, group=seq_p_group
                )
                recv_req = dist.irecv(
                    gathered_shards[partner], src=partner_global_rank, group=seq_p_group
                )
                send_req.wait()
                recv_req.wait()
            else:
    ...
    def apply(
        ...
        **kwargs,
        ...
        """
        assert (
            not enable_head_parallel
        ), "Ulysses-4090 can't support head parallel mode."
        assert not (
            use_fp8_comm and use_fp4_comm
        ), "use_fp8_comm and use_fp4_comm can't be enabled at the same time."
        ...
        if img_first:
            ...
            txt_q, txt_k, txt_v = (
                q[:txt_qkv_len, :, :].contiguous(),
                k[:txt_qkv_len, :, :].contiguous(),
                v[:txt_qkv_len, :, :].contiguous(),
            )
            img_q, img_k, img_v = (
                q[txt_qkv_len:, :, :].contiguous(),
                k[txt_qkv_len:, :, :].contiguous(),
        ...
        qkv_shards = [
            img_qkv[:, :, i * shard_heads : (i + 1) * shard_heads, :].contiguous()
        ...
        if use_fp8_comm:
            ...
            for i in range(world_size):
                qkv_quant, qkv_scale = quant_fp8_vllm(
                    qkv_shards[i].reshape(-1, hidden_dims)
                )
                if i == 0:
                    qkv_quant_bytes = qkv_quant.numel() * qkv_quant.element_size()
                    qkv_quant_dtype = qkv_quant.dtype
                    qkv_scale_dtype = qkv_scale.dtype
                qkv_quant_byte_tensors.append(
                    torch.cat(
                        [
                            qkv_quant.contiguous().reshape(-1).view(torch.uint8),
                            qkv_scale.contiguous().reshape(-1).view(torch.uint8),
                        ],
                        dim=0,
                    )
                )

            gathered_qkv_quant_byte_tensors = self.load_balanced_all_to_all(
                qkv_quant_byte_tensors, seq_p_group
            )
            ...
            for i in range(world_size):
                ...
                qkv_quant = (
                    qkv_quant_byte_tensor[:qkv_quant_bytes]
                    .view(qkv_quant_dtype)
                    .reshape(3, -1, hidden_dims)
                )
                qkv_scale = (
                    qkv_quant_byte_tensor[qkv_quant_bytes:]
                    .view(qkv_scale_dtype)
                    .reshape(3, -1, 1)
                )
                q_shards_new = dequant_fp8_vllm(
                    qkv_quant[0], qkv_scale[0], qkv_dtype
                ).reshape(-1, shard_heads, hidden_dims)
                k_shards_new = dequant_fp8_vllm(
                    qkv_quant[1], qkv_scale[1], qkv_dtype
                ).reshape(-1, shard_heads, hidden_dims)
                v_shards_new = dequant_fp8_vllm(
                    qkv_quant[2], qkv_scale[2], qkv_dtype
                ).reshape(-1, shard_heads, hidden_dims)
                gathered_q_shards.append(q_shards_new)
                gathered_k_shards.append(k_shards_new)
                gathered_v_shards.append(v_shards_new)
        else:
            gathered_qkv_byte_tensors = self.load_balanced_all_to_all(
                qkv_shards, seq_p_group
            )
            ...
            for i in range(world_size):
                qkv_tensor = (
                    gathered_qkv_byte_tensors[i]
                    .view(qkv_dtype)
                    .reshape(3, -1, shard_heads, hidden_dims)
                )
    ...
    def _reshape_img_attn(
        ...
        # 按序列维度切分成 N 份
        attn_shards = [
            img_attn[i * shard_seqlen : (i + 1) * shard_seqlen, :, :].contiguous()
            for i in range(world_size)
        ]
        ...
        if use_fp8_comm:
            ...
            for i in range(world_size):
                attn_quant, attn_scale = quant_fp8_vllm(
                    attn_shards[i].reshape(-1, hidden_dims)
                )
                if i == 0:
                ...
                attn_quant_byte_tensors.append(
                    torch.cat(
                        [
                            attn_quant.contiguous().reshape(-1).view(torch.uint8),
                            attn_scale.contiguous().reshape(-1).view(torch.uint8),
                        ],
                        dim=0,
                    )
                )

            gathered_attn_quant_byte_tensors = self.load_balanced_all_to_all(
                attn_quant_byte_tensors, seq_p_group
            )
            ...
            for i in range(world_size):
                ...
                attn_quant = (
                    attn_quant_byte_tensor[:attn_quant_bytes]
                    .view(attn_quant_dtype)
                    .reshape(-1, hidden_dims)
                )
                attn_scale = (
                    attn_quant_byte_tensor[attn_quant_bytes:]
                    .view(attn_scale_dtype)
                    .reshape(-1, 1)
                )
                attn_shards_new = dequant_fp8_vllm(
                    attn_quant, attn_scale, attn_dtype
                ).reshape(-1, shard_heads, hidden_dims)
                gathered_attn_shards.append(attn_shards_new)

        else:
            gathered_attn_shards = self.load_balanced_all_to_all(
                attn_shards, seq_p_group
            )
