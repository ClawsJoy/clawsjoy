...
from .svg_attn import (
    diagonal_band_mask_from_sparsity,
    get_attention_mask,
...
class SlaMaskGenerator(GeneralMaskGenerator):
    ...
    def __call__(self, q, k):
        ...
        sparse_map, lut, topk = get_block_map(
            ...
            BLKQ=self.q_block_size,
            BLKK=self.k_block_size,
        )
...
class NbhdMaskGenerator(GeneralMaskGenerator):
    ...
    def __init__(
        ...
        super().__init__(q_block_size, k_block_size, sparse_setting, attnmap_frame_num)
        self.coefficient = self.sparse_setting.get(
...
class SvgMaskGenerator(GeneralMaskGenerator):
    ...
    def __init__(
        ...
        super().__init__(q_block_size, k_block_size, sparse_setting, attnmap_frame_num)
        self.sample_mse_max_row = self.sparse_setting.get(
    ...
    def prepare_mask(self, q):
        ...
        if seqlen == SvgMaskGenerator.seqlen:
            return
        logger.info(
            f"SvgMaskGenerator: Preparing mask for seqlen={seqlen}, head_num={head_num}, head_dim={head_dim}"
        )
        frame_size = seqlen // self.attnmap_frame_num
        SvgMaskGenerator.attention_masks = [
            get_attention_mask(
                mask_name,
        ...
        mask = diagonal_band_mask_from_sparsity(
            block_num, block_num_per_frame, self.sparsity, device=q.device
        )
