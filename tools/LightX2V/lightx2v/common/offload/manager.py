...
class WeightAsyncStreamManager(object):
    ...
    def init_cpu_buffer(self, blocks_cpu_buffer=None, phases_cpu_buffer=None):
        ...
        if self.offload_granularity == "block":
            ...
            self.cpu_buffers = [
                blocks_cpu_buffer[i] for i in range(len(blocks_cpu_buffer))
            ]
        elif self.offload_granularity == "phase":
            assert phases_cpu_buffer is not None
            self.cpu_buffers = [
                phases_cpu_buffer[i] for i in range(len(phases_cpu_buffer))
            ]
        else:
    ...
    def init_cuda_buffer(self, blocks_cuda_buffer=None, phases_cuda_buffer=None):
        ...
        if self.offload_granularity == "block":
            ...
            self.cuda_buffers = [
                blocks_cuda_buffer[i] for i in range(len(blocks_cuda_buffer))
            ]
        elif self.offload_granularity == "phase":
            assert phases_cuda_buffer is not None
            self.cuda_buffers = [
                phases_cuda_buffer[i] for i in range(len(phases_cuda_buffer))
            ]
        else:
            raise NotImplementedError
    ...
    def init_first_buffer(self, blocks, adapter_block_idx=None):
        with torch_device_module.stream(self.init_stream):
            if hasattr(self, "cpu_buffers"):
                if self.offload_granularity == "block":
                    self.cuda_buffers[0].load_state_dict(
                        self.cpu_buffers[0].state_dict(), 0, adapter_block_idx
                    )
                else:
                    self.cuda_buffers[0].load_state_dict(
                        self.cpu_buffers[0][0].state_dict(), 0, adapter_block_idx
                    )
            else:
                if self.offload_granularity == "block":
                    self.cuda_buffers[0].load_state_dict(
                        blocks[0].state_dict(), 0, adapter_block_idx
                    )
                else:
                    self.cuda_buffers[0].load_state_dict(
                        blocks[0].compute_phases[0].state_dict(), 0, adapter_block_idx
                    )
        self._sync()
        ...
        with torch_device_module.stream(self.cuda_load_stream):
            if hasattr(self, "cpu_buffers"):
                self.cuda_buffers[phase_idx].load_state_dict(
                    self.cpu_buffers[0][phase_idx].state_dict(),
                    block_idx,
                    adapter_block_idx,
                )
            else:
    ...
    def start_prefetch_block(self, block_idx, adapter_block_idx=None):
        ...
        if self.offload_granularity == "block":
            future = self.executor.submit(
                self.cpu_buffers[1].load_state_dict_from_disk,
                block_idx,
                adapter_block_idx,
            )
