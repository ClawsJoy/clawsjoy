...
class EulerScheduler(WanScheduler):
    ...
    def step_pre(self, step_index):
        ...
        if self.config["model_cls"] == "wan2.2_audio":
            ...
            self.timestep_input = torch.cat(
                [
                    temp_ts,
                    temp_ts.new_ones(max_seq_len - temp_ts.size(0))
                    * self.timestep_input,
                ]
            ).unsqueeze(0)
    ...
    def prepare_latents(self, seed, latent_shape, dtype=torch.float32):
        ...
        if self.config["model_cls"] == "wan2.2_audio":
            ...
            if self.prev_latents is not None:
                self.latents = (
                    1.0 - self.mask
                ) * self.prev_latents + self.mask * self.latents
    ...
    def prepare(self, seed, latent_shape, infer_steps=None, image_encoder_output=None):
        ...
        timesteps = np.linspace(
            self.num_train_timesteps, 0, self.infer_steps + 1, dtype=np.float32
        )
    ...
    def step_post(self):
        ...
        sigma = self.unsqueeze_to_ndim(self.sigmas[self.step_index], sample.ndim).to(
            sample.device, sample.dtype
        )
        sigma_next = self.unsqueeze_to_ndim(
            self.sigmas[self.step_index + 1], sample.ndim
    ...
    def unsqueeze_to_ndim(self, in_tensor, tgt_n_dim):
        if in_tensor.ndim > tgt_n_dim:
            logger.warning(
                f"the given tensor of shape {in_tensor.shape} is expected to unsqueeze to {tgt_n_dim}, the original tensor will be returned"
            )
            return in_tensor
        if in_tensor.ndim < tgt_n_dim:
...
class ConsistencyModelScheduler(EulerScheduler):
    def step_post(self):
        ...
        x_t_next = x0 * (1 - sigma_next) + sigma_next * torch.randn(
            x0.shape, dtype=x0.dtype, device=x0.device, generator=self.generator
        )
        self.latents = x_t_next
        if self.config["model_cls"] == "wan2.2_audio" and self.prev_latents is not None:
            self.latents = (
