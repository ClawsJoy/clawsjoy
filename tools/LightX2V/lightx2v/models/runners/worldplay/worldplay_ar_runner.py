...
class WorldPlayARRunner(HunyuanVideo15Runner):
    ...
    def _run_input_encoder_local_i2v(self):
        ...
        if hasattr(self.input_info, "pose") and self.input_info.pose is not None:
            from lightx2v.models.networks.worldplay.pose_utils import (
                get_latent_num_from_pose,
            )

            latent_num = get_latent_num_from_pose(self.input_info.pose)
            vae_stride_t = self.config["vae_stride"][0]
            with self.config.temporarily_unlocked():
                self.config["target_video_length"] = latent_num * vae_stride_t - (
                    vae_stride_t - 1
                )
            logger.info(
                f"Auto-set target_video_length={self.config['target_video_length']} from pose ({latent_num} latent frames)"
            )
    ...
    def run_denoising_loop(self):
        ...
        for chunk_idx in range(total_chunks):
            ...
            if chunk_idx > 0:
                ...
                self._cache_context_kv(
                    chunk_idx,
                    full_viewmats,
                    full_Ks,
                    full_action,
                    full_cos_sin,
                    full_cond_latents_concat,
                    full_mask_concat,
                    H,
                    W,
                )
    ...
    def _cache_context_kv(
        ...
        self._set_chunk_cos_sin(
            full_cos_sin, context_start_frame, context_end_frame, H, W
        )
