...
class WanRunner(DisaggMixin, DefaultRunner):
    ...
    def load_image_encoder(self):
        ...
        if self.config["task"] in [
            ...
            # offload config
            clip_offload = self.config.get(
                "clip_cpu_offload", self.config.get("cpu_offload", False)
            )
            if clip_offload:
            ...
            if clip_quantized:
                ...
                clip_quantized_ckpt = find_torch_model_path(
                    self.config, "clip_quantized_ckpt", clip_model_name
                )
                ...
                clip_original_ckpt = find_torch_model_path(
                    self.config, "clip_original_ckpt", clip_model_name
                )
    ...
    def load_text_encoder(self):
        ...
        if t5_quantized:
            ...
            t5_quantized_ckpt = find_torch_model_path(
                self.config, "t5_quantized_ckpt", t5_model_name
            )
            ...
            t5_original_ckpt = find_torch_model_path(
                self.config, "t5_original_ckpt", t5_model_name
            )
    ...
    def load_vae_decoder(self):
        ...
        if self.config.get("use_tae", False):
            tae_path = find_torch_model_path(
                self.config, "tae_path", self.tiny_vae_name
            )
            vae_decoder = self.tiny_vae_cls(
                vae_path=tae_path,
                device=self.init_device,
    ...
    def init_scheduler(self):
        ...
        elif self.config.feature_caching in [
            "Tea",
            "Ada",
            "Custom",
            "FirstBlock",
        ...
        if self.config.get("changing_resolution", False):
            self.scheduler = WanScheduler4ChangingResolutionInterface(
                scheduler_class, self.config
            )
        else:
    ...
    def load_model(self):
        ...
        if disagg_mode == "encoder":
            ...
            if (
                self.config.get("disagg_config", {}).get("decoder_engine_rank")
                is not None
            ):
    ...
    def run_text_encoder(self, input_info):
        ...
        if self.config.get("enable_cfg", False) and self.config["cfg_parallel"]:
                    ...
                    [
                        torch.cat(
                            [
                                u,
                                u.new_zeros(
                                    self.config["text_len"] - u.size(0), u.size(1)
                                ),
                            ]
                        )
                        for u in context
            ...
            context = torch.stack(
                [
                    torch.cat(
                        [u, u.new_zeros(self.config["text_len"] - u.size(0), u.size(1))]
                    )
                    for u in context
                ]
            )
    ...
    def run_image_encoder(self, first_frame, last_frame=None):
        ...
        if last_frame is None:
            ...
            clip_encoder_out = (
                self.image_encoder.visual([first_frame, last_frame])
                .squeeze(0)
                .to(GET_DTYPE())
            )
        if self.config.get("lazy_load", False) or self.config.get(
    ...
    def run_vae_encoder(self, first_frame, last_frame=None):
        if self.config.get("resize_mode", None) is None:
            ...
            latent_h, latent_w = (
                ...
                self.input_info.latent_shape[-1],
            )
            world_size_h, world_size_w = None, None
        ...
        if self.config.get("changing_resolution", False):
            ...
            for i in range(len(self.config["resolution_rate"])):
                ...
                vae_encode_out_list.append(
                    self.get_vae_encoder_output(
                        first_frame,
                        latent_h_tmp,
                        latent_w_tmp,
    ...
    def get_vae_encoder_output(
        ...
        if last_frame is not None:
            vae_input = torch.concat(
                [
                    ...
                    torch.nn.functional.interpolate(
                        last_frame.cpu(), size=(h, w), mode="bicubic"
                    ).transpose(0, 1),
                ],
                dim=1,
            ).to(AI_DEVICE)
        ...
        vae_encoder_out = self.vae_encoder.encode(
            vae_input.unsqueeze(0).to(GET_DTYPE()),
            world_size_h=world_size_h,
            world_size_w=world_size_w,
        )
...
class MultiModelStruct:
    def __init__(self, model_list, config, boundary=0.875, num_train_timesteps=1000):
        ...
        logger.info(
            f"boundary: {self.boundary}, boundary_timestep: {self.boundary_timestep}"
        )
    ...
    def get_current_model_index(self):
        if (
            ...
            logger.info(
                f"using - HIGH - noise model at step_index {self.scheduler.step_index + 1}"
            )
            self.scheduler.sample_guide_scale = self.config["sample_guide_scale"][0]
            if (
                self.config.get("cpu_offload", False)
                and self.config.get("offload_granularity", "block") == "model"
            ):
            ...
            logger.info(
                f"using - LOW - noise model at step_index {self.scheduler.step_index + 1}"
            )
...
class Wan22MoeRunner(WanRunner):
    ...
    def load_transformer(self):
        ...
        if not self.config.get("lazy_load", False) and not self.config.get(
            ...
            model_struct = MultiModelStruct(
                [None, None], self.config, self.config["boundary"]
            )
    ...
    def switch_lora(
        ...
        if high_lora_path is not None:
            if self.model.model[0] is not None and hasattr(
                self.model.model[0], "_update_lora"
            ):
                logger.info(
                    f"Switching high_noise_model LoRA to: {high_lora_path} with strength={high_lora_strength}"
                )
                self.model.model[0]._update_lora(high_lora_path, high_lora_strength)

        if low_lora_path is not None:
            if self.model.model[1] is not None and hasattr(
                self.model.model[1], "_update_lora"
            ):
                logger.info(
                    f"Switching low_noise_model LoRA to: {low_lora_path} with strength={low_lora_strength}"
                )
...
class Wan22DenseRunner(WanRunner):
    ...
    def _resolve_wan22_vae_paths(self):
        ...
        decoder_path = self.config.get("vae_path") or find_torch_model_path(
            self.config, filename=decoder_filename
        )
        ...
        return {
            ...
            "lightvae_pruning_rate": self.config.get(
                "lightvae_pruning_rate", default_pruning_rate
            ),
...
class LingbotRunner(Wan22MoeRunner):
    ...
    def _se3_inverse(T: torch.Tensor) -> torch.Tensor:
        ...
        out = torch.eye(4, device=T.device, dtype=T.dtype)[None].repeat(
            T.shape[0], 1, 1
        )
        out[:, :3, :3] = rot_inv
        out[:, :3, 3:] = trans_inv
    ...
    def _build_lingbot_dit_cond_dict(self, action_path: str) -> dict:
        ...
        if not (os.path.isfile(poses_path) and os.path.isfile(intrinsics_path)):
            logger.warning(
                "lingbot action path missing poses.npy or intrinsics.npy: {}",
                action_path,
            )
            return {}
        ...
        if Ks_np.shape[0] > 1:
            ...
            ks_idx = np.clip(
                np.round(np.linspace(0, Ks_np.shape[0] - 1, lat_f)).astype(np.int64),
                0,
                Ks_np.shape[0] - 1,
            )
            Ks_np = Ks_np[ks_idx]
        ...
        if self.control_type == "act" and os.path.isfile(action_file):
            ...
            if action_np.shape[0] != lat_f:
                idx = np.clip(
                    np.round(np.linspace(0, action_np.shape[0] - 1, lat_f)).astype(
                        np.int64
                    ),
                    0,
                    action_np.shape[0] - 1,
        ...
        plucker_pix = self._build_plucker_embedding(
            c2ws, Ks, height, width, only_rays_d=action_tensor is not None
        )
        ...
        c2ws_plucker_tokens = plucker_pix.reshape(
            lat_f * lat_h * lat_w, base_c * pix_c1 * pix_c2
        )
        plucker = (
            c2ws_plucker_tokens.unsqueeze(0)
            .reshape(1, lat_f, lat_h, lat_w, -1)
            .permute(0, 4, 1, 2, 3)
            .contiguous()
        )
        ...
        if action_tensor is not None:
            ...
            action_pix = action_tensor[:, None, None, None, None, :].repeat(
                1, lat_h, pix_c1, lat_w, pix_c2, 1
            )
            ...
            action_tokens = action_pix.reshape(
                lat_f * lat_h * lat_w, action_c * pix_c1 * pix_c2
            )
            action_emb = (
                action_tokens.unsqueeze(0)
                .reshape(1, lat_f, lat_h, lat_w, -1)
                .permute(0, 4, 1, 2, 3)
                .contiguous()
            )
        ...
        logger.info(
            "[lingbot] c2ws_plucker_emb returned batch-less shape={}",
            tuple(plucker_no_batch.shape),
        )
    ...
    def get_encoder_output_i2v(
        ...
        out = super().get_encoder_output_i2v(
            clip_encoder_out, vae_encoder_out, text_encoder_output, img
        )
        # `infer.py` passes `--action_path` CLI arg; runner also historically used `pose` as a generic path.
        action_path = getattr(self.input_info, "action_path", "") or getattr(
        ...
        if action_path:
            ...
            out["image_encoder_output"]["dit_cond_dict"] = (
                self._build_lingbot_dit_cond_dict(action_path)
            )
        return out
