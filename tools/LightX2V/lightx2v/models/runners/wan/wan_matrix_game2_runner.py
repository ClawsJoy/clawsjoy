...
from lightx2v.models.input_encoders.hf.wan.matrix_game2.conditions import (
...
def get_wanx_vae_wrapper(model_path, weight_dtype):
    vae = WanMtxg2VAE(pretrained_path=os.path.join(model_path, "Wan2.1_VAE.pth")).to(
        weight_dtype
    )
    clip = CLIPModel(
...
class WanSFMtxg2Runner(WanSFRunner):
    ...
    def _run_input_encoder_local_i2v(self):
        ...
        padding_video = torch.zeros_like(image).repeat(
            1, 1, 4 * (self.config["num_output_frames"] - 1), 1, 1
        )
        img_cond = torch.concat([image, padding_video], dim=2)
        tiler_kwargs = {"tiled": True, "tile_size": [44, 80], "tile_stride": [23, 38]}
        img_cond = self.image_encoder.encode(
            img_cond, device=self.device, **tiler_kwargs
        ).to(self.device)
    ...
    def init_run_segment(self, segment_idx):
        ...
        if self.config["streaming"]:
            self.inputs["current_actions"] = get_current_action(
