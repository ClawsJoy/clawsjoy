...
class LightLLMServiceTextEncoder:
    ...
    def __init__(self, config, device=None):
        ...
        # LightLLM 服务配置
        self.service_url = config.get("service_url", "http://smart_config.HOST:8010")
    ...
    def _test_service_connection(self):
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(
                    f"✓ Successfully connected to LightLLM service at {self.service_url}"
                )
            else:
                logger.warning(
                    f"⚠ LightLLM service returned status code: {response.status_code}"
                )
        except requests.exceptions.RequestException as e:
            logger.error(f"✗ Failed to connect to LightLLM service: {e}")
            logger.error(
                f"  Please ensure the service is running at {self.service_url}"
            )
            logger.error(
                f"  Start with: python -m lightllm.server.api_server --return_input_hidden_states ..."
            )
    ...
    def _call_service(
        ...
        if images is not None and len(images) > 0:
            ...
            logger.debug(
                f"Found {image_token_count} image tokens in prompt, have {len(images)} images"
            )

            # 确保图像数量与 prompt 中的图像 token 数量匹配
            if image_token_count != len(images):
                logger.warning(
                    f"Image token count ({image_token_count}) != image count ({len(images)}), adjusting to match prompt"
                )
                # 如果 prompt 中有多个图像 token，但只提供了 1 个图像，重复使用该图像
                if len(images) == 1 and image_token_count > 1:
                    images = images * image_token_count
                    logger.debug(
                        f"Repeated image {image_token_count} times to match prompt"
                    )
                elif image_token_count == 0:
                    logger.warning(
                        "No image tokens found in prompt, skipping image transmission"
                    )
                    images = []
            ...
            for idx, img in enumerate(images):
                ...
                logger.debug(
                    f"Encoded image {idx + 1}/{len(images)}: {len(img_str)} chars base64 (JPEG)"
                )
        ...
        for attempt in range(self.retry_times):
            try:
                logger.debug(
                    f"Calling LightLLM service (attempt {attempt + 1}/{self.retry_times})..."
                )
                response = requests.post(
                    f"{self.service_url}/generate", json=payload, timeout=self.timeout
                )
                ...
                if hasattr(e, "response") and e.response is not None:
                    try:
                        ...
                        logger.warning(f"⚠ Attempt {attempt + 1} failed: {last_error}")
                        logger.debug(
                            f"  Response text: {e.response.text[:200] if hasattr(e.response, 'text') else 'N/A'}"
                        )
                else:
                    logger.warning(f"⚠ Attempt {attempt + 1} failed: {last_error}")
        ...
        # 所有重试都失败
        raise RuntimeError(
            f"Failed to call LightLLM service after {self.retry_times} attempts. Last error: {last_error}"
        )
    ...
    def infer(self, text: List[str], image_list: Optional[List] = None) -> Tuple:
        ...
        logger.debug(
            f"  Image count: {len(condition_image_list) if condition_image_list else 0}"
        )
        logger.debug(
            f"  Base image prompt: {base_img_prompt[:50] if base_img_prompt else 'None'}..."
        )
        ...
        if self.use_shm and "shm_hidden_states_name" in result:
            ...
            try:
                ...
                hidden_states_np = shm_client.read_hidden_states(
                    shm_name, shape, dtype=np.uint8
                )
                hidden_states = torch.from_numpy(hidden_states_np).to(
                    device=self.device, non_blocking=True
                )
                logger.debug(f"✓ Read hidden states from shared memory: {shm_name}")
            except Exception as e:
                logger.warning(
                    f"Failed to read from shared memory '{shm_name}': {e}, falling back to HTTP mode"
                )
                ...
                if "hidden_states_base64" in result:
                    ...
                    hidden_states_np = np.frombuffer(
            ...
            if isinstance(hidden_states_data, list):
                # 列表格式：检查是否是扁平列表或嵌套列表
                if len(hidden_states_data) > 0 and isinstance(
                    hidden_states_data[0], list
                ):
                ...
                hidden_states = torch.from_numpy(hidden_states_data).to(
                    device=self.device, non_blocking=True
                )
            else:
                # 其他格式
            ...
            raise ValueError(
                f"LightLLM service response missing 'hidden_states' or 'hidden_states_base64'. Response keys: {result.keys()}"
            )
        ...
        logger.debug(
            f"Converted hidden states: shape={hidden_states.shape}, dtype={hidden_states.dtype}, device={hidden_states.device}"
        )
        ...
        attention_mask = torch.ones(
            hidden_states.shape[0], seq_len, dtype=torch.long, device=self.device
        )
        ...
        logger.info(
            f"✓ LightLLM service inference complete: prompt_embeds shape={prompt_embeds.shape}"
        )
