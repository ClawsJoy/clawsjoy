...
class Qwen3Model_TextEncoder:
    def __init__(self, config):
        ...
        self.cpu_offload = config.get(
            "qwen3_cpu_offload", config.get("cpu_offload", False)
        )
    ...
    def load(self):
        if self.qwen3_quantized:
            ...
            self.text_encoder = Qwen3Model.from_pretrained(
                self.config["qwen3_quantized_ckpt"],
            ...
            if self.cpu_offload:
                self.text_encoder = Qwen3Model.from_pretrained(
                    qwen3_original_ckpt, torch_dtype=GET_DTYPE(), device_map="cpu"
                )
            else:
                self.text_encoder = Qwen3Model.from_pretrained(
                    qwen3_original_ckpt, torch_dtype=GET_DTYPE(), device_map=AI_DEVICE
                )

        qwen3_tokenizer_path = self.config.get(
            "qwen3_tokenizer_path", os.path.join(self.config["model_path"], "tokenizer")
        )
