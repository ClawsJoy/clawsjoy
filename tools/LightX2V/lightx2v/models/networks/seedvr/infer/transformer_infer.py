...
from lightx2v.models.networks.seedvr.utils.ops import (
...
class SeedVRTransformerInfer(BaseTransformerInfer):
    def __init__(self, config):
        ...
        self.rope = (
            get_na_rope(rope_type=self.rope_type, dim=self.rope_dim)
            if self.rope_type
            else None
        )
    ...
    def _attn_forward(self, block_weight, vid, txt, vid_shape, txt_shape, cache):
        ...
        qkv_vid = gather_seq_scatter_heads_qkv(
            qkv_vid, seq_dim=0, qkv_shape=vid_shape, cache=cache.namespace("vid")
        )
        qkv_txt = gather_seq_scatter_heads_qkv(
            qkv_txt, seq_dim=0, qkv_shape=txt_shape, cache=cache.namespace("txt")
        )
    ...
    def _attn_window_forward(self, block_weight, vid, txt, vid_shape, txt_shape, cache):
        ...
        qkv_vid_win = rearrange(
            qkv_vid_win, "l (o h d) -> l o h d", o=3, d=self.head_dim
        )
        qkv_txt = rearrange(qkv_txt, "l (o h d) -> l o h d", o=3, d=self.head_dim)
    ...
    def _infer_block(self, block_weight, vid, txt, vid_shape, txt_shape, emb, cache):
        ...
        if block_weight.window is not None and block_weight.window_method is not None:
            vid_attn, txt_attn = self._attn_window_forward(
                block_weight, vid_attn, txt_attn, vid_shape, txt_shape, cache
            )
        else:
            vid_attn, txt_attn = self._attn_forward(
                block_weight, vid_attn, txt_attn, vid_shape, txt_shape, cache
            )
        ...
        if self.mlp_type == "swiglu":
            vid_mlp = self._get_branch(block_weight, "mlp_proj_out", "vid").apply(
                F.silu(
                    self._get_branch(block_weight, "mlp_proj_in_gate", "vid").apply(
                        vid_mlp
                    )
                )
            ...
            if not block_weight.vid_only:
                txt_mlp = self._get_branch(block_weight, "mlp_proj_out", "txt").apply(
                    F.silu(
                        self._get_branch(block_weight, "mlp_proj_in_gate", "txt").apply(
                            txt_mlp
                        )
                    )
                    * self._get_branch(block_weight, "mlp_proj_in", "txt").apply(
                        txt_mlp
                    )
                )
        else:
    ...
    def infer(self, block_weights, pre_infer_out):
        ...
        for block_weight in block_weights:
            vid, txt, vid_shape, txt_shape = self._infer_block(
                block_weight, vid, txt, vid_shape, txt_shape, emb, cache
            )
