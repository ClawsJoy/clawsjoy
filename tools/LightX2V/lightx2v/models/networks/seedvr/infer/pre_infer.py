...
class SeedVRPreInfer:
    ...
    def _patch_in(self, weights, vid, vid_shape, cache: Cache):
        if not hasattr(weights, "vid_in_proj"):
            raise RuntimeError(
                "SeedVR pre-weights missing vid_in_proj. Check ckpt keys for vid_in.proj.*"
            )
    ...
    def infer(
        self, weights, vid, txt, vid_shape, txt_shape, timestep, disable_cache=False
    ):
        ...
        # Time embedding
        emb = self._time_embedding(
            weights, timestep, device=vid.device, dtype=vid.dtype
        )
