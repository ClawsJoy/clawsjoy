...
class NeoppPreInfer:
    def __init__(self, config):
        ...
        self.cos_cached_x, self.sin_cached_x = precompute_rope_freqs_sincos(
            self.rope_dim_part,
            config["vision_config"]["max_position_embeddings_vision"],
            base=config["vision_config"]["rope_theta_vision"],
            device=None,
        )
    ...
    def infer(self, weights):
        ...
        image_input_reshaped = image_input.view(
            image_input.shape[0] * image_input.shape[1], -1
        )

        image_embeds = self.extract_feature(
            weights, image_input_reshaped, grid_hw=self.scheduler.grid_hw
        ).view(1, token_h * token_w, -1)

        t_expanded = t.expand(token_h * token_w)
        timestep_embeddings = self.timestep_embedder(weights, t_expanded).view(
            1, token_h * token_w, -1
        )
        if self.add_noise_scale_embedding:
            noise_scale_tensor = torch.full_like(
                t_expanded, self.scheduler.noise_scale / self.noise_scale_max_value
            )
            noise_embeddings = self.noise_scale_embedder(
                weights, noise_scale_tensor
            ).view(1, token_h * token_w, -1)
    ...
    def extract_feature(
        self, weights, pixel_values: torch.FloatTensor, grid_hw=None
    ) -> torch.Tensor:
        ...
        patch_embeds = F.gelu(
            weights.vision_model_mot_gen_patch_embedding.apply(pixel_values)
        ...
        for i in range(grid_hw.shape[0]):
            ...
            patches_per_img = weights.vision_model_mot_gen_dense_embedding.apply(
