...
from lightx2v.models.networks.longcat_image.infer.offload.transformer_infer import (
    LongCatImageOffloadTransformerInfer,
)
...
class LongCatImageTransformerModel(BaseTransformerModel):
    ...
    def __init__(self, model_path, config, device):
        ...
        if self.config["seq_parallel"]:
            raise NotImplementedError(
                "Sequence parallel is not implemented for LongCatImageTransformerModel"
            )
    ...
    def _infer_cond_uncond(self, latents_input, prompt_embeds, infer_condition=True):
        ...
        noise_pred = self.post_infer.infer(
            self.post_weight, hidden_states, pre_infer_out.temb
        )
    ...
    def _seq_parallel_pre_process(self, pre_infer_out):
        raise NotImplementedError(
            "Sequence parallel pre-process is not implemented for LongCatImageTransformerModel"
        )

    @torch.no_grad()
    def _seq_parallel_post_process(self, x):
        raise NotImplementedError(
            "Sequence parallel post-process is not implemented for LongCatImageTransformerModel"
        )
    ...
    def infer(self, inputs):
        ...
        if self.config.get("enable_cfg", True):
            ...
            if use_cfg_parallel:
                ...
                noise_pred = self.scheduler.apply_cfg(
