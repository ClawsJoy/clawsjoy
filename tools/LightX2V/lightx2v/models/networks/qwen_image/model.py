...
from lightx2v.models.networks.qwen_image.infer.offload.transformer_infer import (
...
from lightx2v.models.networks.qwen_image.weights.post_weights import (
...
class QwenImageTransformerModel(BaseTransformerModel):
    ...
    def _init_infer_class(self):
        if self.config["feature_caching"] == "NoCaching":
            self.transformer_infer_class = (
                ...
                else QwenImageOffloadTransformerInfer
            )
        else:
    ...
    def _infer_cond_uncond(self, latents_input, prompt_embeds, infer_condition=True):
        ...
        noise_pred = self.post_infer.infer(
            self.post_weight, hidden_states, pre_infer_out.temb_txt_silu
        )
    ...
    def infer(self, inputs):
        ...
        if self.config["task"] == "i2i":
            image_latents = torch.cat(
                [item["image_latents"] for item in inputs["image_encoder_output"]],
                dim=1,
            )
