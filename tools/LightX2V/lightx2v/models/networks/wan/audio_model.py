...
from lightx2v.models.networks.wan.infer.audio.transformer_infer import (
    WanAudioTransformerInfer,
)
from lightx2v.models.networks.wan.model import WanModel
from lightx2v.models.networks.wan.weights.audio.transformer_weights import (
    WanAudioTransformerWeights,
)
...
class WanAudioModel(WanModel):
    ...
    def _load_adapter_ckpt(self):
        if self.config.get("adapter_model_path", None) is None:
            if self.config.get("adapter_quantized", False):
                if self.config.get("adapter_quant_scheme", None) in [
                    "fp8",
                    "fp8-q8f",
                    "fp8-vllm",
                    "fp8-sgl",
                ...
                elif self.config.get("adapter_quant_scheme", None) in [
                    "int8",
                    "int8-q8f",
                    "int8-vllm",
                    "int8-torchao",
                    ...
                    raise ValueError(
                        f"Unsupported quant_scheme: {self.config.get('adapter_quant_scheme', None)}"
                    )
            else:
                adapter_model_name = "audio_adapter_model.safetensors"
            self.config["adapter_model_path"] = os.path.join(
                self.config["model_path"], adapter_model_name
            )

        if self.config.get("dummy_model", False):
            from lightx2v.models.networks.base_model import (
            ...
            logger.info(
                f"[DummyModel] Generating random adapter weights on device={dummy_device}"
            )
            tensors_meta = BaseTransformerModel._read_safetensors_metadata(
            ...
            for key, meta in tensors_meta.items():
                ...
                adapter_weights_dict[key] = (
                    torch.randn(shape, dtype=dtype, device=dummy_device)
                    if dtype.is_floating_point
                    else torch.zeros(shape, dtype=dtype, device=dummy_device)
                )
            return adapter_weights_dict

        adapter_offload = self.config.get("cpu_offload", False)
        load_from_rank0 = self.config.get("load_from_rank0", False)
        adapter_weights_dict = load_weights(
            self.config["adapter_model_path"],
            cpu_offload=adapter_offload,
            remove_key="audio",
        ...
        if not adapter_offload:
            if not dist.is_initialized() or not load_from_rank0:
                for key in adapter_weights_dict:
                    adapter_weights_dict[key] = adapter_weights_dict[key].to(
    ...
    def start_compile(self, shape, audio_num, with_mask=True):
        ...
        logger.info(
            f"[Compile] Compile shape: {shape}, audio_num:{audio_num}, graph_name: {graph_name}"
        )
        ...
        new_inputs["text_encoder_output"]["context"] = torch.randn(
            1, 512, 4096, dtype=torch.bfloat16
        ).cuda()
        new_inputs["text_encoder_output"]["context_null"] = torch.randn(
            1, 512, 4096, dtype=torch.bfloat16
        ).cuda()

        new_inputs["image_encoder_output"] = {}
        new_inputs["image_encoder_output"]["clip_encoder_out"] = torch.randn(
            257, 1280, dtype=torch.bfloat16
        ).cuda()
        new_inputs["image_encoder_output"]["vae_encoder_out"] = torch.randn(
            16, 1, latents_h, latents_w, dtype=torch.bfloat16
        ).cuda()

        new_inputs["audio_encoder_output"] = torch.randn(
            audio_num, latents_length, 128, 1024, dtype=torch.bfloat16
        ).cuda()
        if with_mask:
            new_inputs["person_mask_latens"] = torch.zeros(
                audio_num, 1, (latents_h // 2), (latents_w // 2), dtype=torch.int8
            ).cuda()
        else:
        ...
        new_inputs["previmg_encoder_output"]["prev_latents"] = torch.randn(
            16, latents_length, latents_h, latents_w, dtype=torch.bfloat16
        ).cuda()
        new_inputs["previmg_encoder_output"]["prev_mask"] = torch.randn(
            4, latents_length, latents_h, latents_w, dtype=torch.bfloat16
        ).cuda()

        self.scheduler.latents = torch.randn(
            16, latents_length, latents_h, latents_w, dtype=torch.bfloat16
        ).cuda()
        self.scheduler.timestep_input = torch.tensor(
            [600.0], dtype=torch.float32
        ).cuda()
        self.scheduler.audio_adapter_t_emb = torch.randn(
            1, 3, 5120, dtype=torch.bfloat16
        ).cuda()
    ...
    def select_graph_for_compile(self, input_info):
        logger.info(
            f"target_h, target_w : {input_info.target_shape[0]}, {input_info.target_shape[1]}, audio_num: {input_info.audio_num}"
        )
        graph_name = self.get_graph_name(
            input_info.target_shape,
            input_info.audio_num,
            with_mask=input_info.with_mask,
        )
    ...
    def _seq_parallel_pre_process(self, pre_infer_out):
        ...
        if person_mask_latens is not None:
            pre_infer_out.adapter_args["person_mask_latens"] = torch.chunk(
                person_mask_latens, world_size, dim=1
            )[cur_rank]
