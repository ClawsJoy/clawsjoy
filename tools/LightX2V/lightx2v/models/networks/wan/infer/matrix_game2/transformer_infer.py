...
class WanMtxg2TransformerInfer(WanSFTransformerInfer):
    def __init__(self, config):
        ...
        self.vae_time_compression_ratio = config["action_config"][
        ...
        self.freqs_cos, self.freqs_sin = self._get_rotary_pos_embed(
            7500,
            self.patch_size[1],
            self.patch_size[2],
            64,
    ...
    def _get_rotary_pos_embed(
        ...
        if isinstance(self.patch_size, int):
            assert all(
                s % self.patch_size == 0 for s in latents_size
    ...
    def _infer_action_attn(self, phase, q, k, v):
        ...
        if q.dim() == 3:
            out = attn_mod.apply(
                q, k, v, max_seqlen_q=q.size(0), max_seqlen_kv=k.size(0)
            )
    ...
    def infer_action_model(
        ...
        cond3 = (
            ...
            == current_start + self.num_frame_per_chunk
        )
        ...
        mouse_offload = (
            msc is not None
            and getattr(msc, "_kv_offload", False)
            and self.enable_mouse
            and mouse_condition is not None
            and is_causal
        )
        if mouse_offload:
    ...
    def _infer_action_model_with_kvcache(
        ...
        if self.enable_mouse and mouse_condition is not None:
            ...
            if is_causal:
                mouse_condition = mouse_condition[
                    ...
                    * (N_feats - self.num_frame_per_chunk - self.windows_size)
                    + pad_t :,
                    :,
                ]
                group_mouse = [
                    ...
                    for i in range(self.num_frame_per_chunk)
                ]
            else:
                group_mouse = [
                    mouse_condition[
            ...
            group_mouse = group_mouse.unsqueeze(-1).expand(
                B, self.num_frame_per_chunk, pad_t, C, S
            )
            group_mouse = group_mouse.permute(0, 4, 1, 2, 3).reshape(
                B * S, self.num_frame_per_chunk, pad_t * C
            )

            group_mouse = torch.cat([hidden_states, group_mouse], dim=-1)

            group_mouse = torch.nn.functional.linear(
                group_mouse, phase.mouse_mlp_0.weight.T, phase.mouse_mlp_0.bias
            )
            group_mouse = torch.nn.functional.gelu(group_mouse, approximate="tanh")
            group_mouse = torch.nn.functional.linear(
                group_mouse, phase.mouse_mlp_2.weight.T, phase.mouse_mlp_2.bias
            )
            group_mouse = torch.nn.functional.layer_norm(
                group_mouse,
                (group_mouse.shape[-1],),
                phase.mouse_mlp_3.weight.T,
                phase.mouse_mlp_3.bias,
                1e-5,
            )
            ...
            if is_causal:
                ...
                if current_end > g and num_new_tokens + l_ > kv_cache_size:
                    ...
                    action_mouse_kv_cache.roll_window(
                        li, sink_tokens, num_evicted_tokens
                    )
                    local_end_index = l_ + current_end - g - num_evicted_tokens
                else:
                    local_end_index = l_ + current_end - g
                local_start_index = local_end_index - num_new_tokens
                action_mouse_kv_cache.store_kv(
                    k, v, local_start_index, local_end_index, li
                )
        ...
        if self.enable_keyboard and keyboard_condition is not None:
            ...
            if is_causal:
                ...
                keyboard_condition = phase.keyboard_embed_2.apply(
                    keyboard_condition
                ).unsqueeze(0)
                group_keyboard = [
                    keyboard_condition[
            ...
            if use_rope_keyboard:
                ...
                if is_causal:
                    ...
                    if current_end > g and num_new_tokens + l_ > kv_cache_size:
                        num_evicted_tokens = num_new_tokens + l_ - kv_cache_size
                        action_keyboard_kv_cache.roll_window(
                            li, sink_tokens, num_evicted_tokens
                        )
                        local_end_index = l_ + current_end - g - num_evicted_tokens
                    else:
                    ...
                    local_start_index = local_end_index - num_new_tokens
                    assert k.shape[0] == S
                    action_keyboard_kv_cache.store_kv(
                        k[:1][0], v[:1][0], local_start_index, local_end_index, li
                    )
                    action_keyboard_kv_cache.set_ends(li, current_end, local_end_index)
                    attn_s = max(0, local_end_index - max_attention_size)
                    slice_k = (
                        action_keyboard_kv_cache.k_cache(li, attn_s, local_end_index)
                        .unsqueeze(0)
                        .expand(S, -1, -1, -1)
                    )
                    slice_v = (
                        action_keyboard_kv_cache.v_cache(li, attn_s, local_end_index)
                        .unsqueeze(0)
                        .expand(S, -1, -1, -1)
                    )
                ...
                if is_causal:
                    ...
                    action_keyboard_kv_cache.store_kv(
                        k_wr, v_wr, local_start_index, local_end_index, li
                    )
                    action_keyboard_kv_cache.set_ends(li, current_end, local_end_index)
                    attn_s = max(0, local_end_index - max_attention_size)
                    attn = self._infer_action_attn(
                        phase,
                        q,
                        action_keyboard_kv_cache.k_cache(
                            li, attn_s, local_end_index
                        ).unsqueeze(0),
                        action_keyboard_kv_cache.v_cache(
                            li, attn_s, local_end_index
                        ).unsqueeze(0),
                    )
                else:
