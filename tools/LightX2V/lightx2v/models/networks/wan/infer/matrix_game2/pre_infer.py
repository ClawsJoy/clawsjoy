...
def cond_current(
    ...
    new_cond["cond_concat"] = conditional_dict["image_encoder_output"]["cond_concat"][
        :, :, current_start_frame : current_start_frame + num_frame_per_chunk
    ]
    new_cond["visual_context"] = conditional_dict["image_encoder_output"][
        "visual_context"
    ]
    if replace:
        ...
        if mode != "templerun":
            conditional_dict["text_encoder_output"]["mouse_cond"][
                :, -last_frame_num + final_frame : final_frame
            ] = replace["mouse"][None, None, :].repeat(1, last_frame_num, 1)
        conditional_dict["text_encoder_output"]["keyboard_cond"][
            :, -last_frame_num + final_frame : final_frame
        ] = replace["keyboard"][None, None, :].repeat(1, last_frame_num, 1)
    if mode != "templerun":
        new_cond["mouse_cond"] = conditional_dict["text_encoder_output"]["mouse_cond"][
            :, : 1 + 4 * (current_start_frame + num_frame_per_chunk - 1)
        ]
    new_cond["keyboard_cond"] = conditional_dict["text_encoder_output"][
        "keyboard_cond"
    ][:, : 1 + 4 * (current_start_frame + num_frame_per_chunk - 1)]
...
class WanMtxg2PreInfer(WanSFPreInfer):
    ...
    def infer(self, weights, inputs, kv_start=0, kv_end=0):
        ...
        if self.config["streaming"]:
            ...
            current_conditional_dict, _ = cond_current(
                inputs,
                current_start_frame,
            ...
            current_conditional_dict = cond_current(
                inputs,
                current_start_frame,
                self.scheduler.num_frame_per_chunk,
                mode=self.config["mode"],
        ...
        grid_sizes_t, grid_sizes_h, grid_sizes_w = torch.tensor(
        ...
        embed_tmp = sinusoidal_embedding_1d(self.freq_dim, t.flatten()).type_as(
            x
        )  # torch.Size([3, 256])
