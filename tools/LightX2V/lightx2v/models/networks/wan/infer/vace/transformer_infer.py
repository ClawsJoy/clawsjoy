...
class WanVaceTransformerInfer(WanOffloadTransformerInfer):
    def __init__(self, config):
        ...
        self.vace_blocks_mapping = {
            orig_idx: seq_idx
            for seq_idx, orig_idx in enumerate(self.config["vace_layers"])
        }

    def infer(self, weights, pre_infer_out):
        self.cos_sin = pre_infer_out.cos_sin
        self.reset_infer_states()
        pre_infer_out.c = self.vace_pre_process(
            weights.vace_patch_embedding, pre_infer_out.vace_context
        )

        # In seq parallel mode, chunk c to match pre_infer_out.x
        if self.config.get("seq_parallel", False):
            pre_infer_out.c = self._chunk_c_for_seq_parallel(
                pre_infer_out.c, pre_infer_out.x
            )
    ...
    def vace_pre_process(self, patch_embedding, vace_context):
        c = patch_embedding.apply(
            vace_context.unsqueeze(0).to(self.sensitive_layer_dtype)
        )
    ...
    def infer_vace_blocks(self, vace_blocks, pre_infer_out):
        ...
        if hasattr(self, "offload_manager"):
            self.offload_manager.init_cuda_buffer(
                ...
                self.vace_offload_phase_cuda_buffers,
            )
        self.infer_func(vace_blocks, pre_infer_out.c, pre_infer_out)
        self.infer_state = "base"
        if hasattr(self, "offload_manager"):
            self.offload_manager.init_cuda_buffer(
                self.offload_block_cuda_buffers, self.offload_phase_cuda_buffers
            )
    ...
    def post_process(self, x, y, c_gate_msa, pre_infer_out):
        ...
        if self.infer_state == "base" and self.block_idx in self.vace_blocks_mapping:
            ...
            x = x + pre_infer_out.adapter_args["hints"][
                hint_idx
            ] * pre_infer_out.adapter_args.get("context_scale", 1.0)
        return x
