...
class WanLingbotPreInfer(WanPreInfer):
    def _build_lingbot_conditional_dict(
        ...
        cam = (
            cam.permute(0, 2, 4, 6, 1, 3, 5, 7)
            .contiguous()
            .reshape(b, -1, c * p0 * p1 * p2)
        )
        ...
        cam_hidden = weights.c2ws_hidden_states_layer2.apply(
            torch.nn.functional.silu(
                weights.c2ws_hidden_states_layer1.apply(cam.squeeze(0))
            )
        )
    ...
    def infer(self, weights, inputs, kv_start=0, kv_end=0):
        ...
        out.conditional_dict = self._build_lingbot_conditional_dict(
            weights, inputs, out.x
        )
        return out
