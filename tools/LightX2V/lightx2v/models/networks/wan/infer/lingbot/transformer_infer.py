...
class WanLingbotTransformerInfer(WanTransformerInfer):
    ...
    def infer_cross_attn(
        self, phase, x, context, y_out, gate_msa, block, conditional_dict
    ):
        ...
        cam_hidden = block.cam_injector_layer2.apply(
            torch.nn.functional.silu(block.cam_injector_layer1.apply(cam))
        )
