...
def _causal_rope_apply_kernel(
    ...
    freq_row = tl.where(
        col_offsets < t_dim,
        start_frame + t,
        tl.where(col_offsets < t_dim + h_dim, y, z),
    )
    ...
    rotated_real = x_real.to(tl.float32) * cos_vals.to(tl.float32) - x_imag.to(
        tl.float32
    ) * sin_vals.to(tl.float32)
    rotated_imag = x_real.to(tl.float32) * sin_vals.to(tl.float32) + x_imag.to(
        tl.float32
    ) * cos_vals.to(tl.float32)
...
class _CausalRoPEApplyTriton(torch.autograd.Function):
    ...
    def forward(ctx, x, grid_sizes, freqs, start_frame):
        ...
        grid_sizes = grid_sizes.to(
            device=x.device, dtype=torch.int64, non_blocking=True
        ).contiguous()
