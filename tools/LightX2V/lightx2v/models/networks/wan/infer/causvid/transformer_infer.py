...
class WanTransformerInferCausVid(WanOffloadTransformerInfer):
    ...
    def _init_kv_cache(self, dtype, device):
        ...
        for _ in range(self.blocks_num):
            kv_cache.append(
                {
                    "k": torch.zeros(
                        [kv_size, self.num_heads, self.head_dim],
                        dtype=dtype,
                        device=device,
                    ),
                    "v": torch.zeros(
    ...
    def _init_crossattn_cache(self, dtype, device):
        ...
        for _ in range(self.blocks_num):
            crossattn_cache.append(
                {
                    "k": torch.zeros(
                        [self.text_len, self.num_heads, self.head_dim],
                        dtype=dtype,
                        device=device,
                    ),
                    "v": torch.zeros(
    ...
    def infer_self_attn(
        ...
        q = weights.self_attn_norm_q.apply(weights.self_attn_q.apply(norm1_out)).view(
            s, n, d
        )
        k = weights.self_attn_norm_k.apply(weights.self_attn_k.apply(norm1_out)).view(
            s, n, d
        )
        ...
        if not self.parallel_attention:
            ...
            raise NotImplementedError(
                "Parallel attention is not implemented for causvid inference"
            )
        ...
        cu_seqlens_q, cu_seqlens_k = self._calculate_q_k_len(
            q=q, k_lens=torch.tensor([kv_end], dtype=torch.int32)
        )
    ...
    def infer_cross_attn(self, weights, x, context, block_idx):
        ...
        q = weights.cross_attn_norm_q.apply(weights.cross_attn_q.apply(norm3_out)).view(
            -1, n, d
        )
        if not self.crossattn_cache[block_idx]["is_init"]:
            k = weights.cross_attn_norm_k.apply(
                weights.cross_attn_k.apply(context)
            ).view(-1, n, d)
        ...
        cu_seqlens_q, cu_seqlens_k = self._calculate_q_k_len(
            q, k_lens=torch.tensor([k.size(0)], dtype=torch.int32)
        )
        ...
        if self.task in ["i2v", "s2v", "rs2v"]:
            k_img = weights.cross_attn_norm_k_img.apply(
    ...
    def infer_ffn(self, weights, x, embed0):
        ...
        y = weights.ffn_0.apply(
            norm2_out * (1 + embed0[4].squeeze(0)) + embed0[3].squeeze(0)
        )
    ...
    def infer_block(
        ...
        if embed0.dim() == 3:
            modulation = weights.compute_phases[0].modulation.tensor.unsqueeze(
                2
            )  # 1, 6, 1, dim
            ...
            embed0 = (weights.compute_phases[0].modulation.tensor + embed0).chunk(
                6, dim=1
            )
