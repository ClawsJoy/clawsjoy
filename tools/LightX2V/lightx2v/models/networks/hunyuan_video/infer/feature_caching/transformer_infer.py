...
class HunyuanTransformerInferTeaCaching(HunyuanVideo15OffloadTransformerInfer):
    ...
    def calculate_should_calc(self, img, vec, block):
        ...
        img_mod1_shift, img_mod1_scale, _, _, _, _ = img_mod_layer.apply(vec_).chunk(
            6, dim=-1
        )
        inp = inp.squeeze(0)
        normed_inp = torch.nn.functional.layer_norm(
            inp, (inp.shape[1],), None, None, 1e-6
        )
        ...
        if (
            ...
            or self.scheduler.step_index == self.scheduler.infer_steps - 1
        ):
            should_calc = True
            ...
            if self.scheduler.infer_condition:
                self.accumulated_rel_l1_distance_odd += rescale_func(
                    (
                        (modulated_inp - self.previous_modulated_input_odd).abs().mean()
                        / self.previous_modulated_input_odd.abs().mean()
                    )
                    .cpu()
                    .item()
                )
                ...
                self.accumulated_rel_l1_distance_even += rescale_func(
                    (
                        (modulated_inp - self.previous_modulated_input_even)
                        .abs()
                        .mean()
                        / self.previous_modulated_input_even.abs().mean()
                    )
                    .cpu()
                    .item()
                )
    ...
    def infer(self, weights, infer_module_out):
        should_calc = self.calculate_should_calc(
            infer_module_out.img, infer_module_out.vec, weights.double_blocks[0]
        )
        if not should_calc:
            if self.scheduler.infer_condition:
    ...
    def clear(self):
        ...
        if self.previous_modulated_input_even is not None:
            self.previous_modulated_input_even = (
                self.previous_modulated_input_even.cpu()
            )
