...
if torch.cuda.is_available() and torch.cuda.get_device_capability(0) in [
    (8, 9),
    (12, 0),
]:
    try:
...
def sage_attn_no_pad_v3(
    ...
    output_unpad = (
        sageattn3_blackwell(
            query_unpad.unsqueeze(0).transpose(1, 2),
            key_unpad.unsqueeze(0).transpose(1, 2),
            value_unpad.unsqueeze(0).transpose(1, 2),
        )
        .transpose(1, 2)
        .squeeze(0)
    )
