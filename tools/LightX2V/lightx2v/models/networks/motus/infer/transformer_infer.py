...
class MotusTransformerInfer(BaseTransformerInfer):
    def __init__(self, model, config):
        ...
        self.modulate_func = (
            ...
            if config.get("modulate_type", "triton") == "triton"
            else modulate
        )
    ...
    def _get_cu_seqlens(self, cache_name, batch, seq_len, device, attn_type):
        ...
        if (
            cu_seqlens is None
            or int(cu_seqlens[-1].item()) != expected_total
            or cu_seqlens.device != device
        ):
            tensor = torch.arange(
                0, expected_total + seq_len, seq_len, dtype=torch.int32
            )
    ...
    def _apply_gate(self, x, y, gate):
        out_dtype = x.dtype
        if self.sensitive_layer_dtype != self.infer_dtype:
            out = x.to(self.sensitive_layer_dtype) + y.to(
    ...
    def _apply_video_rope(self, q, k, cos_sin_cache):
        ...
        if q.shape[0] == 1:
            q_out, k_out = self.apply_rope_func(
                q.squeeze(0), k.squeeze(0), cos_sin_cache
            )
    ...
    def _infer_joint_attention(
        ...
        norm_video = self._modulate(
            video_self_phase.norm1.apply(video_tokens), v_mod[1], v_mod[0]
        )
        norm_action = self._modulate(
            action_block.norm1.apply(action_tokens), a_mod[1], a_mod[0]
        )
        ...
        video_q = video_self_phase.self_attn_norm_q.apply(
            apply_mm(video_self_phase.self_attn_q, norm_video)
        ).view(
            batch,
            video_len,
            self.num_heads,
        ...
        video_k = video_self_phase.self_attn_norm_k.apply(
            apply_mm(video_self_phase.self_attn_k, norm_video)
        ).view(
            batch,
            video_len,
            self.num_heads,
        ...
        video_v = apply_mm(video_self_phase.self_attn_v, norm_video).view(
            batch, video_len, self.num_heads, self.head_dim
        )
        video_q, video_k = self._apply_video_rope(
            video_q, video_k, pre_infer_out.cos_sin
        )

        action_q, action_k, action_v = action_block.wan_action_qkv.apply(norm_action)
        action_q = action_block.wan_action_norm_q.apply(action_q.flatten(-2)).view(
            batch, action_len, self.num_heads, self.head_dim
        )
        action_k = action_block.wan_action_norm_k.apply(action_k.flatten(-2)).view(
            batch, action_len, self.num_heads, self.head_dim
        )

        und_q, und_k, und_v = und_block.wan_und_qkv.apply(norm_und)
        und_q = und_block.wan_und_norm_q.apply(und_q.flatten(-2)).view(
            batch, und_len, self.num_heads, self.head_dim
        )
        und_k = und_block.wan_und_norm_k.apply(und_k.flatten(-2)).view(
            batch, und_len, self.num_heads, self.head_dim
        )
        ...
        cu_seqlens = self._get_cu_seqlens(
            "joint_attn_cu_seqlens_q",
            batch,
            total_len,
            q_all.device,
            self.self_joint_attn_type,
        )
        ...
        action_out = apply_mm(
            ...
            attn_out[:, video_len : video_len + action_len, :],
        )
        und_out = apply_mm(
            und_block.wan_und_o, attn_out[:, video_len + action_len :, :]
        )
    ...
    def _infer_cross_attention(self, video_tokens, processed_t5_context, layer_idx):
        ...
        q = cross_phase.cross_attn_norm_q.apply(
            apply_mm(cross_phase.cross_attn_q, norm_video)
        ).view(
            batch,
            q_len,
            self.num_heads,
        ...
        k = cross_phase.cross_attn_norm_k.apply(
            apply_mm(cross_phase.cross_attn_k, processed_t5_context)
        ).view(
            batch,
            ctx_len,
            self.num_heads,
        ...
        v = apply_mm(cross_phase.cross_attn_v, processed_t5_context).view(
            batch, ctx_len, self.num_heads, self.head_dim
        )

        cu_seqlens_q = self._get_cu_seqlens(
            "cross_attn_cu_seqlens_q", batch, q_len, q.device, self.cross_attn_type
        )
        cu_seqlens_kv = self._get_cu_seqlens(
            "cross_attn_cu_seqlens_kv", batch, ctx_len, k.device, self.cross_attn_type
        )
        attn_out = self._apply_attention_kernel(
            self.cross_attn,
            q,
    ...
    def _infer_video_ffn(self, video_tokens, video_adaln_modulation, layer_idx):
        ...
        ffn_input = self._modulate(
            ffn_phase.norm2.apply(video_tokens), v_mod[4], v_mod[3]
        )
    ...
    def _infer_action_ffn(self, action_tokens, action_adaln_modulation, layer_idx):
        ...
        a_mod = action_adaln_modulation
        ffn_input = self._modulate(
            action_block.norm2.apply(action_tokens), a_mod[4], a_mod[3]
        )
    ...
    def infer(self, weights, pre_infer_out):
        ...
        video_tokens = self.model.video_backbone.prepare_input(
            self.scheduler.video_latents.to(self.model.dtype)
        )
        action_tokens = self._prepare_action_tokens(
            pre_infer_out, self.scheduler.action_latents
        )

        timestep_scaled = (
            (timestep * 1000).expand(action_tokens.shape[0]).to(self.model.dtype)
        )
        video_head_time_emb, video_adaln_params = (
            self.model.video_backbone.get_time_embedding(
                timestep_scaled, video_tokens.shape[1]
            )
        )
        action_head_time_emb, action_adaln_params = (
            self.model.action_backbone.get_time_embedding(
                timestep_scaled, action_tokens.shape[1]
            )
        )
        ...
        hidden_states = self.infer_main_blocks(
            range(len(weights.video.blocks)), hidden_states, pre_infer_out, infer_state
        )
    ...
    def infer_without_offload(self, blocks, hidden_states, pre_infer_out, infer_state):
        for block in blocks:
            hidden_states = self.infer_block(
                block, hidden_states, pre_infer_out, infer_state
            )
    ...
    def infer_block(self, block, hidden_states, pre_infer_out, infer_state):
        ...
        video_adaln_modulation = self.model.video_backbone.compute_adaln_modulation(
            infer_state["video_adaln_params"], block
        )
        action_adaln_modulation = self.model.action_backbone.compute_adaln_modulation(
            infer_state["action_adaln_params"], block
        )
        ...
        video_tokens = self._infer_cross_attention(
            video_tokens, infer_state["processed_t5_context"], block
        )
        action_tokens = self._infer_action_ffn(
            action_tokens, action_adaln_modulation, block
        )
        und_tokens = self._infer_und_ffn(und_tokens, block)
        video_tokens = self._infer_video_ffn(
            video_tokens, video_adaln_modulation, block
        )
        return video_tokens, action_tokens, und_tokens
    ...
    def infer_non_blocks(self, hidden_states, infer_state):
        ...
        action_pred_full = self.model.action_backbone.apply_output(
            action_tokens, infer_state["action_head_time_emb"]
        )
        num_regs = self.model.action_backbone.config.num_registers
        action_velocity = (
            action_pred_full[:, 1:-num_regs, :]
            if num_regs > 0
            else action_pred_full[:, 1:, :]
        )
