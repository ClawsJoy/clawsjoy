...
def build_s3_client(args: argparse.Namespace):
    if boto3 is None or BotoConfig is None:
        raise RuntimeError(
            "boto3/botocore is required. Please install boto3 or aioboto3."
        )
...
def generate_presigned_pair(
    s3_client, bucket: str, object_key: str, expires_in: int
) -> tuple[str, str]:
...
def build_sync_payload(
    args: argparse.Namespace, presigned_url: str = ""
) -> Dict[str, Any]:
...
def run_client_upload_flow(
    ...
    put_url, get_url = generate_presigned_pair(
        s3_client, bucket, object_key, args.presign_expires
    )

    payload = build_sync_payload(args, presigned_url="")
    t0 = time.perf_counter()
    response = call_sync(
        args.url, payload, args.timeout_seconds, args.poll_interval_seconds
    )
    t1 = time.perf_counter()
    if response.status_code != 200:
        raise RuntimeError(
            f"[client_upload] sync failed ({response.status_code}): {response.text}"
        )
    image_bytes = response.content
    t2 = time.perf_counter()
    upload_resp = requests.put(
        put_url, data=image_bytes, timeout=args.upload_timeout_seconds
    )
    t3 = time.perf_counter()
    if upload_resp.status_code not in (200, 201, 204):
        raise RuntimeError(
            f"[client_upload] upload failed ({upload_resp.status_code}): {upload_resp.text}"
        )
    ...
    if args.verify_download:
        ...
        if check_resp.status_code != 200:
            raise RuntimeError(
                f"[client_upload] download verify failed ({check_resp.status_code}): {check_resp.text}"
            )
...
def run_server_upload_flow(
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"[server_upload] sync failed ({response.status_code}): {response.text}"
        )

    content_type = response.headers.get("content-type", "")
    if "application/json" not in content_type:
        raise RuntimeError(
            f"[server_upload] expected JSON but got content-type={content_type!r}"
        )
    body = response.json()
    if not body.get("uploaded_to_presigned_url"):
        raise RuntimeError(
            f"[server_upload] upload flag is false, response={json.dumps(body, ensure_ascii=False)}"
        )
    ...
    if args.verify_download:
        ...
        if check_resp.status_code != 200:
            raise RuntimeError(
                f"[server_upload] download verify failed ({check_resp.status_code}): {check_resp.text}"
            )
...
def print_summary(label: str, values: List[float]) -> None:
    print(
        f"{label}: avg={statistics.mean(values):.2f} ms, p50={percentile(values, 0.5):.2f} ms, p90={percentile(values, 0.9):.2f} ms, min={min(values):.2f} ms, max={max(values):.2f} ms"
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Benchmark sync latency: client upload to S3 vs x2v server upload to S3."
    )
    parser.add_argument(
        "--url",
        type=str,
        default="http://smart_config.HOST:8000",
        help="x2v server base url",
    )
    parser.add_argument("--runs", type=int, default=5, help="Benchmark rounds")
    parser.add_argument(
        "--warmup_runs", type=int, default=0, help="Warmup rounds before measurement"
    )
    parser.add_argument(
        "--order",
        type=str,
        default="alternate",
        choices=["alternate", "client_first", "server_first"],
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Target shape, e.g. 1536 2752",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--verify_download",
        action="store_true",
        help="Verify uploaded object with GET presigned URL",
    )
    ...
    print(
        f"Start benchmark: warmup={args.warmup_runs}, runs={args.runs}, order={args.order}"
    )
    for idx in range(total_rounds):
        ...
        if args.order == "client_first":
            ...
            flow_order = (
                ["client", "server"] if (idx % 2 == 0) else ["server", "client"]
            )
        ...
        for flow in flow_order:
            if flow == "client":
                one_round_client = run_client_upload_flow(
                    args, s3_client, bucket, client_key
                )
                print(
                    "[client_upload] "
                ...
                one_round_server = run_server_upload_flow(
                    args, s3_client, bucket, server_key
                )
                print(
                    f"[server_upload] total={one_round_server['sync_total_ms']:.2f} ms"
                )
