...
def build_presigned_url(args: argparse.Namespace) -> tuple[str, Optional[str]]:
    ...
    if not object_key:
        key_prefix = (
            args.s3_key_prefix or os.getenv("S3_BASE_PATH", "lightx2v/sync")
        ).strip("/")
...
def download_uploaded_image(
    download_url: str, output_path: str, timeout_seconds: int
) -> Path:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Call /v1/tasks/image/sync for T2I with presigned_url upload."
    )
    parser.add_argument(
        "--url",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional URL to download result image for verification",
    )
    parser.add_argument(
        "--download_output",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=120,
        help="Timeout for download verification",
    )
    ...
    if target_shape is not None and len(target_shape) < 2:
        raise ValueError(
            "--target_shape must provide at least 2 integers, e.g. --target_shape 1536 2752"
        )
    ...
    download_url = (
        args.download_url or resolved_download_url or result.get("presigned_url")
    )
    if download_url:
