...
def build_wan21_components(
    ...
    with gr.Column(elem_classes=["diffusion-model-group"]) as wan21_row:
        with gr.Row():
            ...
            with gr.Column(scale=1, min_width=150):
                ...
                dit_download_btn = gr.Button(
                    "📥 下载", visible=dit_btn_visible, size="sm", variant="secondary"
                )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                lora_path_input = gr.Dropdown(
                    ...
                    value=(
                        lora_choices_init[0]
                        if lora_choices_init and lora_choices_init[0]
                        else ""
                    ),
                    allow_custom_value=True,
                    visible=False,
    ...
    def update_lora_choices(model_path_val):
        ...
        return gr.update(
            ...
            value=lora_choices[0] if lora_choices and lora_choices[0] else "",
        )
...
def build_wan22_components(
    ...
    with gr.Row(visible=False, elem_classes=["wan22-row"]) as wan22_row:
        with gr.Column(scale=1):
            high_noise_choices_init = get_high_noise_choices(
                model_path, "wan2.2", "i2v"
            )
            ...
            high_noise_download_btn = gr.Button(
                "📥 下载",
                visible=high_noise_btn_visible,
                size="sm",
                variant="secondary",
            )
        ...
        with gr.Column(scale=1):
            ...
            low_noise_download_btn = gr.Button(
                "📥 下载", visible=low_noise_btn_visible, size="sm", variant="secondary"
            )
...
def build_qwen_image_components(
    ...
    def get_dit_choices_func(model_path_val):
        model_type_val = (
            model_type_input.value
            if hasattr(model_type_input, "value")
    ...
    # 初始化时检查模型状态，确定下载按钮的初始可见性
    def get_initial_download_btn_visibility(
        choices, model_path_val, model_type_val, base_category
    ):
        if not choices:
            return False
    ...
    dit_btn_visible = get_initial_download_btn_visibility(
        qwen_image_dit_choices_init, model_path, initial_model_type, "dit"
    )
    vae_btn_visible = get_initial_download_btn_visibility(
        qwen_image_vae_choices_init, model_path, initial_model_type, "vae"
    )
    scheduler_btn_visible = get_initial_download_btn_visibility(
        qwen_image_scheduler_choices_init, model_path, initial_model_type, "scheduler"
    )
        ...
        with gr.Row():
            with gr.Column(scale=5):
                qwen_image_dit_path_input = gr.Dropdown(
                    ...
                    value=(
                        qwen_image_dit_choices_init[0]
                        if qwen_image_dit_choices_init
                        else ""
                    ),
                    allow_custom_value=True,
                )
            with gr.Column(scale=1, min_width=150):
                qwen_image_dit_download_btn = gr.Button(
                    t("download", lang),
                    visible=dit_btn_visible,
                    size="sm",
                    variant="secondary",
                )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                qwen_image_vae_path_input = gr.Dropdown(
                    ...
                    value=(
                        qwen_image_vae_choices_init[0]
                        if qwen_image_vae_choices_init
                        else ""
                    ),
                    allow_custom_value=True,
                )
                qwen_image_vae_download_btn = gr.Button(
                    t("download", lang),
            ...
            with gr.Column(scale=1):
                qwen_image_scheduler_path_input = gr.Dropdown(
                    ...
                    value=(
                        qwen_image_scheduler_choices_init[0]
                        if qwen_image_scheduler_choices_init
                        else ""
                    ),
                    allow_custom_value=True,
                )
                qwen_image_scheduler_download_btn = gr.Button(
                    t("download", lang),
                    visible=scheduler_btn_visible,
                    size="sm",
                    variant="secondary",
                )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                qwen25vl_encoder_path_input = gr.Dropdown(
                    ...
                    value=(
                        qwen25vl_encoder_choices_init[0]
                ...
                qwen25vl_encoder_download_btn = gr.Button(
                    t("download", lang),
                    visible=qwen25vl_btn_visible,
                    size="sm",
                    variant="secondary",
                )
    ...
    def download_qwen_image_vae(
        ...
        get_choices = (
            get_qwen_image_2512_vae_choices
            if model_type_val == "Qwen-Image-2512"
            else get_qwen_image_vae_choices
        )
        return download_model_handler(
            model_path_val,
            model_name,
    ...
    def download_qwen_image_scheduler(
        ...
        category = (
            ...
            if model_type_val == "Qwen-Image-2512"
            else "qwen_image_scheduler"
        )
        get_choices = (
            get_qwen_image_2512_scheduler_choices
            if model_type_val == "Qwen-Image-2512"
            else get_qwen_image_scheduler_choices
        )
        return download_model_handler(
            model_path_val,
            model_name,
    ...
    def download_qwen25vl_encoder(
        ...
        return download_model_handler(
            ...
            "qwen25vl_encoder",
            download_source_val,
            get_choices_func=get_qwen25vl_encoder_choices,
            progress=progress,
        )
    ...
    qwen_image_dit_download_btn.click(
        ...
        outputs=[
            qwen_image_dit_download_status,
    ...
    qwen_image_vae_download_btn.click(
        ...
        outputs=[
            qwen_image_vae_download_status,
    ...
    qwen_image_scheduler_download_btn.click(
        ...
        outputs=[
            qwen_image_scheduler_download_status,
...
def build_z_image_turbo_components(
    model_path, model_path_input, download_source_input, model_type_input, lang="zh"
):
    ...
    z_image_turbo_scheduler_choices_init = get_z_image_turbo_scheduler_choices(
        model_path
    )
    ...
    # Z-Image-Turbo 模型配置
    with gr.Column(
        elem_classes=["diffusion-model-group"], visible=False
    ) as z_image_turbo_components_group:
        # Diffusion 模型
        with gr.Row():
            with gr.Column(scale=5):
                z_image_turbo_dit_path_input = gr.Dropdown(
                    ...
                    value=(
                        z_image_turbo_dit_choices_init[0]
            ...
            with gr.Column(scale=1, min_width=150):
                ...
                if z_image_turbo_dit_choices_init:
                    ...
                    initial_dit_btn_visible = not check_model_exists(
                        model_path, actual_name
                    )

                z_image_turbo_dit_download_btn = gr.Button(
                    t("download", lang),
                    visible=initial_dit_btn_visible,
                    size="sm",
                    variant="secondary",
                )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                z_image_turbo_vae_path_input = gr.Dropdown(
                    ...
                    value=(
                        z_image_turbo_vae_choices_init[0]
                ...
                if z_image_turbo_vae_choices_init:
                    ...
                    initial_vae_btn_visible = not check_model_exists(
                        model_path, actual_name
                    )

                z_image_turbo_vae_download_btn = gr.Button(
                    t("download", lang),
                    visible=initial_vae_btn_visible,
                    size="sm",
                    variant="secondary",
                )
            ...
            with gr.Column(scale=1):
                z_image_turbo_scheduler_path_input = gr.Dropdown(
                    ...
                    value=(
                        z_image_turbo_scheduler_choices_init[0]
                ...
                if z_image_turbo_scheduler_choices_init:
                    ...
                    initial_scheduler_btn_visible = not check_model_exists(
                        model_path, actual_name
                    )

                z_image_turbo_scheduler_download_btn = gr.Button(
                    t("download", lang),
                    visible=initial_scheduler_btn_visible,
                    size="sm",
                    variant="secondary",
                )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                if qwen3_encoder_choices_init:
                    ...
                    initial_qwen3_btn_visible = not (
                        os.path.exists(repo_path) and os.path.isdir(repo_path)
                    )

                qwen3_encoder_download_btn = gr.Button(
                    t("download", lang),
                    visible=initial_qwen3_btn_visible,
                    size="sm",
                    variant="secondary",
                )
    ...
    # 绑定下载按钮事件
    def download_z_image_turbo_dit(
        model_path_val, model_name, download_source_val, progress=gr.Progress()
    ):
        return download_model_handler(
            model_path_val,
            ...
            "z_image_turbo_dit",
            download_source_val,
            get_choices_func=get_z_image_turbo_dit_choices,
            progress=progress,
        )

    def download_z_image_turbo_vae(
        model_path_val, model_name, download_source_val, progress=gr.Progress()
    ):
        return download_model_handler(
            model_path_val,
            ...
            "z_image_turbo_vae",
            download_source_val,
            get_choices_func=get_z_image_turbo_vae_choices,
            progress=progress,
        )

    def download_z_image_turbo_scheduler(
        model_path_val, model_name, download_source_val, progress=gr.Progress()
    ):
        return download_model_handler(
            model_path_val,
            ...
            download_source_val,
            get_choices_func=get_z_image_turbo_scheduler_choices,
            progress=progress,
        )

    def download_qwen3_encoder(
        model_path_val, model_name, download_source_val, progress=gr.Progress()
    ):
        return download_model_handler(
            model_path_val,
            ...
            "qwen3_encoder",
            download_source_val,
            get_choices_func=get_qwen3_encoder_choices,
            progress=progress,
        )
    ...
    def update_scheduler_status(model_path_val, model_name):
        return update_model_status(
            model_path_val, model_name, "z_image_turbo_scheduler"
        )
    ...
    z_image_turbo_dit_download_btn.click(
        ...
        outputs=[
            z_image_turbo_dit_download_status,
    ...
    z_image_turbo_vae_download_btn.click(
        ...
        outputs=[
            z_image_turbo_vae_download_status,
    ...
    z_image_turbo_scheduler_download_btn.click(
        ...
        outputs=[
            z_image_turbo_scheduler_download_status,
