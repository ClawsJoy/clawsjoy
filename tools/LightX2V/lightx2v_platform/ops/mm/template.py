...
class MMWeightQuantTemplate(MMWeightTemplate):
    ...
    def _load_cuda_buffers(self, weight_dict):
        ...
        self.weight_cuda_buffer, self.weight_scale_cuda_buffer = (
    ...
    def _load_default_tensors(self, weight_dict):
        if not self.lazy_load:
            self.weight, self.weight_scale, self.pin_weight, self.pin_weight_scale = (
    ...
    def load_mxfp4(self, weight_dict):
        if self.config.get("weight_auto_quant", False):
            ...
            if device.type == "cpu":
                ...
                self.pin_weight_scale = torch.empty(
                    weight_scale_shape, pin_memory=True, dtype=weight_scale_dtype
                )
    ...
    def load_nvfp4(self, weight_dict):
        ...
        if device.type == "cpu":
            ...
            self.pin_input_global_scale = torch.empty(
                input_global_scale_shape,
                pin_memory=True,
                dtype=input_global_scale_dtype,
            )
            self.pin_input_global_scale.copy_(input_global_scale)

            alpha_shape = alpha.shape
            alpha_dtype = alpha.dtype
            self.pin_alpha = torch.empty(
                alpha_shape, pin_memory=True, dtype=alpha_dtype
            )
            ...
            if self.create_cuda_buffer:
                ...
                if device.type == "cpu":
                    ...
                    self.pin_bias = torch.empty(
                        bias_shape, pin_memory=True, dtype=bias_dtype
                    )
    ...
    def load_state_dict(self, destination, block_index, adapter_block_index=None):
        if self.is_post_adapter:
            ...
            weight_scale_name = re.sub(
                r"\.\d+", lambda m: f".{block_index}", self.weight_scale_name, count=1
            )
