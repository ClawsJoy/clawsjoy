...
class FixedButler:
    ...
    def _load_memory(self) -> Dict:
        if self.memory_file.exists():
            with open(self.memory_file, "r") as f:
                return json.load(f)
        return {"name": None, "preferences": {}, "history": [], "conversation_count": 0}
    ...
    def _extract_preference(self, text: str) -> Optional[str]:
        """精确提取偏好"""
        patterns = [r"喜欢[\s]*([^，。！？]{2,10})", r"偏好[\s]*([^，。！？]{2,10})"]
        for p in patterns:
            match = re.search(p, text)
            if match:
    ...
    def _fast_response(self, user_input: str) -> Optional[tuple]:
        ...
        if "技能" in lower:
            return (
                "系统有20+原子技能，包括图像生成、视频制作、任务调度等。想了解哪个？",
                "list_skills",
            )
...
if __name__ == "__main__":
    ...
    for msg in tests:
        ...
        print(
            f"   📝 记住: 名字={result['memory']['name']}, 偏好={result['memory']['prefs']}"
        )
