...
class LifeCycleConfigDrivenV2:
    ...
    def __init__(self, user_id: str = "default"):
        ...
        print(
            f"📊 配置: 反思间隔={self.reflection_interval}, 梦境间隔={self.dreaming_interval}"
        )
    ...
    def _act(self, perception: Dict) -> Tuple[str, str]:
        ...
        actions = {
            "greeting": (
                f"你好{f'，{name}' if name else ''}！有什么可以帮你的？",
                "greeting",
            ),
            "self_intro": (
                f"你好，{perception.get('value')}！很高兴认识你",
                "self_intro",
            ),
            "preference": (
                f"好的，我记住你喜欢{perception.get('value')}",
                "preference",
            ),
            "ask_name": (
                f"当然记得！你是{name}呀" if name else "你还没告诉我名字呢",
                "query_name",
            ),
            "ask_preference": (
                f"你喜欢{', '.join(prefs)}" if prefs else "你还没告诉我你的偏好呢",
                "query_pref",
            ),
            "list_agents": (
...
回复要求：简洁，1-2句话。"""
    ...
    def _dream(self):
        ...
        if len(self.dreaming_data["long_term"]) > self.long_term_max:
            self.dreaming_data["long_term"] = self.dreaming_data["long_term"][
                -self.long_term_max :
            ]
...
if __name__ == "__main__":
    ...
    for msg in tests:
        ...
        print(
            f"   [感知: {result['perception']}, 梦境: {result['dream_cycles']}次, 耗时: {result['time_ms']}ms]"
        )
